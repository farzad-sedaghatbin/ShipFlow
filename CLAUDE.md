...
| 2026-09-01 | **A hill-chart scope's task set has two independent, non-exclusive linkage mechanisms — `HillChartPoint.linkedTask`'s subtask subtree (the auto-created Scope-Task Bridge) and `Task.scope` (any task explicitly linked via the "Link to Scope" picker) — and `ScopeProgressService` must aggregate over BOTH when computing a scope's position.** Fixed after Farzad reported a scope's hill-chart % looked pinned to whichever task was first created for it and didn't move after later edits, and that completing one subtask sometimes snapped the whole scope to "Done" (position 100) even with other linked work still open. | `calculatePositionFromTask`/`calculateSuggestedPosition` only ever averaged `linkedTask`'s direct children via `findByParentTaskIdNotDeleted` — `TaskRepository.findByScopeId`/`findByScopeIdAndStatus` existed but were never called from the progress path, so any task linked to the scope afterward via `Task.scope` (already shown as "linked" in `PitchHillChart.tsx`'s Scope Summary panel) was invisible to the average. That also explains the "jumps to Done" symptom: if the narrow calculation's only visible task happened to be one lone completed subtask, average(DONE)=100 pulled the whole scope to the hill's Done end regardless of other `Task.scope`-linked work still in progress. Fix: `ScopeProgressService` now unions `linkedTask`'s subtasks with `taskRepository.findByScopeIdNotDeleted(scopeId)` (new soft-delete-aware query, mirroring `findByParentTaskIdNotDeleted`'s pattern), deduplicated by task ID. `HillChartService#toDTO`'s "suggested position" preview had the identical bug via a direct `calculatePositionFromTask(linkedTask)` call bypassing the scope-aware path — fixed to go through `calculateSuggestedPosition(HillChartPoint)` instead. Any future scope-progress code must go through `ScopeProgressService`'s scope-aware entry points, not `calculatePositionFromTask(Task)` directly (that one-arg overload is intentionally kept subtree-only, for use only at scope-creation time before a `HillChartPoint` exists to look up `Task.scope` links through). |

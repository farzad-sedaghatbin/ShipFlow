...
## Current Milestone: v1.16.0 — "ShipFlow Agent"

**Current version**: v1.14.1 (released 2026-09-10)
...
> **Note on the v1.13.0 release**: the S63/S64/S65 session map below is the planned milestone content, but the actual `v1.13.0` tag also bundles several unplanned fixes that landed in the same release-cutting session and were folded into the same version rather than each getting its own patch release: a sign-in activity audit trail, three public-hosting security hardening fixes (self-registration privilege escalation, default-admin resurrection, docker-compose port exposure), an opt-in gate for the login page's demo-credentials hint, a Retro Board merge-visibility/unmerge fix, and two integration bugs (an `ApiKeyService` NPE, a stale test mock) found only after all of the above were merged together and fixed directly on `main` via their own PR before tagging. See `CHANGELOG.md`'s `[1.13.0]` entry for the full detail on each.

> **Note on the v1.14.0 release and the resulting version shift (2026-09-09)**: v1.14.0 shipped its planned S66–S68 content (cycle-summary reliability, RAG hardening, unified prompt context) plus the open-core licensing addition (see the note below) — and then, in the same release-cutting session, absorbed an unplanned Scrum-parity feedback batch from a tester (Sara Jahani): terminology/mis-gating bug fixes, consistent back-navigation, per-tab project isolation, Scrum-native reports (burnup/sprint/release), an Active Sprint Board (Scrum previously had none), AI task-suggestion + Q&A parity for Scrum, a new Project Agreements feature, a design analysis for configurable methodology structure (not implemented — deferred), and a Task Test Cases panel fix. None of this was part of the planned S66–S68 scope, but rather than spin up a separate v1.14.x patch release for it (mirroring how v1.13.0's release-cutting session folded in its own unplanned fixes), it was folded into the same v1.14.0 tag. **Consequence**: this consumed the v1.14.0 version number for meaningfully more than its original planned scope, so every future milestone originally numbered v1.15.0–v1.20.0 in the Future milestones overview below is now v1.16.0–v1.21.0 — shifted up by one, with session numbering (S69 onward) unchanged. See `CHANGELOG.md`'s `[1.14.0]` entry for the full detail on each item.

ShipFlow is **methodology-agnostic** — supports Shape Up + Kanban + Scrum (v1.1.0 shipped). Competitor migration tooling (v1.2.0) shipped. MCP Server Admin & API Keys (v1.3.0) shipped. Enterprise Auth & UX Depth (v1.4.0) shipped. AI Copilot v2 — AI Pitch Writer, Retrospective Summarizer, Proactive Dashboard Insights (v1.5.0) shipped. MCP Ecosystem (v1.6.0) shipped. Workflow Automations — trigger/action engine, 20 templates, full UI (v1.7.0) shipped. Custom Fields & Advanced RBAC, Wiki / Docs Space, and pluggable Object Storage (v1.8.0) shipped. Production-Grade Self-Hosting — Helm chart, OpenTelemetry, Grafana, audit export, and air-gapped AI mode (v1.9.0) shipped. Wiki references (Pitch/Task links), drag-and-drop task reordering, and target-release for standalone tasks (v1.10.0) shipped. Mobile PWA — offline support, responsive audit, Web Push, passkey auth (v1.11.0) shipped. Plugin SDK completion, GitLab + Azure DevOps MCP client integrations (v1.12.0) shipped. Live Presence & Truth — SSE foundation rework, presence indicators + conflict-safe editing, public-page accuracy sweep, public-API hardening (v1.13.0) shipped. AI You Can Trust — cycle-summary reliability, RAG hardening, unified prompt context, open-core licensing, and an unplanned Scrum-parity feedback batch (v1.14.0) shipped. Next: ShipFlow Agent — @mentionable AI teammate, agent actions via MCP write-tools, agent loops (v1.16.0).

> **Note on the v1.14.0 licensing addition (2026-09-07)**: ShipFlow began moving to an open-core model mid-milestone — this public repository stays fully open source as the **Community Edition** (10-active-user / 5-enabled-automation caps), and an uploadable signed licence file unlocks commercial features and lifts those caps. This shipped as its own PR (`feat(license): licence keys, feature gating and Community Edition limits`), not one of the planned S66–S68 sessions, the same way v1.13.0's release-cutting session folded several unplanned fixes into that tag. No existing feature is gated behind a licence yet — this PR is plumbing only (`@RequiresLicensedFeature` + `LicenseFeatureAspect`, unused in production code). See `30-licensing.md` and the Architectural Decisions Log for the full contract.

> **Roadmap revision (2026-08-23)**: v1.13.0 was previously planned as "Collaborative Editing — real-time CRDT co-editing on pitches and retrospectives". A gap/market/competitor analysis re-scoped it: CRDT co-editing is a from-scratch multi-milestone build (no WebSocket transport, no CRDT library, pitches are plain Markdown textareas), no competitor's 2026 story is co-editing (Linear/Atlassian/Asana are all shipping **AI agents that do work inside the tool**), and adoption — not collaboration depth — is ShipFlow's bottleneck. The replacement v1.13–v1.21 roadmap (renumbered 2026-09-09, see the versioning note above) is in the Future milestones overview below; full CRDT co-editing survives only as a demand-gated wiki-only slot in the final Knowledge & Docs Graph milestone (BlockNote's first-party Yjs path). See `ROADMAP.md` and the plan file `~/.claude/plans/i-blive-the-plan-velvety-river.md` for the full rationale.
...
### v1.13.0 session map (shipped 2026-09-06)

| Session | Task | Status |
|---------|------|--------|
| S63 | SSE foundation rework — `NotificationSseManager` reworked to a `Set<SseEmitter>` per user (a second tab/device no longer evicts the first) with Redis pub/sub fan-out (`SseRedisConfig`, `shipflow:sse:notifications`) so delivery works correctly in a multi-replica deployment; `useNotificationStream` reconnect now backs off exponentially with jitter and re-reads the auth token per attempt instead of a flat 5s retry with a stale token | ✅ done |
| S64 | Live presence indicators + conflict-safe editing — presence avatar stack (Redis-backed, ephemeral) on Pitch/Retro/Wiki via the S63 SSE transport; `Pitch`/`RetroItem`/`WikiPage` gain a JPA `@Version` column, updates carry `expectedVersion`, a mismatch returns 409 resolved by a keep-mine/discard-and-reload dialog instead of silent last-write-wins; Pitch/Wiki live-refresh for current viewers on save | ✅ done |
| S65 | Public-page truth sweep (a handful of stale dates/missing entries on `ReleaseNotes.tsx`/`PublicRoadmap.tsx`, not the sprawling rewrite the task name implies — `ROADMAP.md`/`COMPETITOR_ANALYSIS.md` were already accurate) + `/api/v1/public/**` hardening: API keys can now be restricted to one project (`ApiKey.restrictedToProjectId`, enforced by `PublicApiAuthorizationService`), and the prefix gained its own rate-limit bucket where it previously had none | ✅ done |

### v1.14.0 session map (shipped 2026-09-09)

| Session | Task | Status |
|---------|------|--------|
| S66 | Cycle-summary reliability — no hallucinated/conversational persistence, invalidation | ✅ done |
| S67 | RAG hardening — structured cycle context in Q&A widget, honest confidence, help-guide indexing | ✅ done |
| S68 | Unified prompt context — `ProjectSnapshotService` into Risk Advisor/Q&A/Test Gen, per-feature model config, fix dead Create-Selected | ✅ done |
| — | **Out-of-plan**: open-core licensing (licence keys, `@RequiresLicensedFeature` gating plumbing, Community Edition user/automation limits) — see the licensing note above | ✅ done |
| — | **Out-of-plan, unplanned Scrum-parity feedback batch** (tester Sara Jahani) — see the versioning note above and `CHANGELOG.md`'s `[1.14.0]` entry for full detail: Scrum terminology/mis-gating bug fixes, consistent back-navigation + per-tab project isolation, Scrum-native reports (burnup/sprint/release), Active Sprint Board (Scrum previously had none), AI task-suggestion + Q&A parity for Scrum, Project Agreements feature, configurable-methodology-structure design analysis (not implemented, deferred), Task Test Cases panel fix | ✅ done |

### Future milestones overview

> **Renumbered 2026-09-09** — see the versioning note above. Session numbers (S69 onward) are unchanged; only the version label each range ships under moved up by one.
...
| v1.13.0 ✅ shipped 2026-09-06 | Live Presence & Truth — SSE foundation rework (Redis pub/sub fan-out for `NotificationSseManager`, multi-emitter-per-user, `useNotificationStream` reconnect fix), presence indicators + optimistic-lock conflict dialogs on pitch/retro/wiki + live refresh, public-page truth sweep + `/api/v1/public/**` hardening. See the v1.13.0 session map above for the S63–S65 breakdown and the versioning note above for the additional unplanned fixes bundled into this tag | S63–S65 |
| v1.14.0 ✅ shipped 2026-09-09 | AI You Can Trust — cycle-summary reliability, RAG hardening, unified prompt context (S66–S68), plus open-core licensing and an unplanned Scrum-parity feedback batch bundled into the same tag. See the v1.14.0 session map above and the versioning note above for the full breakdown | S66–S68 |
| v1.16.0 (current) | **ShipFlow Agent** (flagship) — S69: @mentionable agent in comment threads (RAG + structured context, RBAC-scoped agent identity); S70: agent actions via existing MCP write-tool layer with propose→confirm; S71: agent loops (standup digest to Slack/Teams, cycle-health check, betting-table prep) as Workflow Automation action types. Unique wedge: runs fully self-hosted/air-gapped via Ollama | S69–S71 |
| v1.17.0 | Time to Value — S72: project/pitch templates + clone project; S73: tokenized public share links (pitch, hill chart, roadmap; revocable, noindex); S74: guest access (single-project external stakeholders) | S72–S74 |
| v1.18.0 | Dev Loop — S75: GitHub Issues two-way sync; S76: branch/PR status automation (branch→In Progress, PR→In Review, merge→Done); S77: GitLab repo parity (`GitLabRepository` entity, repo picker, Wise Architecture wiring) + Org-Settings tab for GitLab/ADO tokens | S75–S77 |
| v1.19.0 | Stakeholder View — S78: cross-project portfolio rollup + velocity/appetite-accuracy trends; S79: capacity & timesheet rollups over existing `WorkLog` data; S80: goals (OKR-lite) linked to Initiatives/Pitches | S78–S80 |
| v1.20.0 | Platform Trust — S81: OAuth 2.0 for MCP (GAP 11, plan in `MCP_SERVER_MILESTONE.md`); S82: credential encryption at rest (AES-GCM `AttributeConverter` over isolated secret columns); S83: MCP completeness (prompt templates, resource URIs, missing tools) | S81–S83 |
| v1.21.0 | Knowledge & Docs Graph — S84–S85: Knowledge Center providers (GitHub, Confluence, Notion, Google Drive — SPI ready); S86: wiki-only co-editing via BlockNote's first-party Yjs path, **demand-gated** (skip if still no multi-user usage) | S84–S86 |

Dropped / not scheduled: full CRDT co-editing on pitches & retros (no document model, no demand evidence — revisit only after the v1.21 gate), plugin dynamic runtime loading / marketplace registry (sandboxing risk, deliberately out of scope), native app-store mobile app (PWA holds).
...
7. Write unit or integration tests (JaCoCo gate: ≥ 50% line coverage — see the v1.13.1 note in the Architectural Decisions Log before assuming "80%"). **The full suite must be at 0 failures** — if your change breaks existing tests (e.g. serialization format change, renamed field), fix those tests in the same commit before pushing.
...
**Coverage gate**: 50% line coverage enforced by JaCoCo (lowered from a documented-but-never-actually-enforced 80% in v1.13.1 — see the Architectural Decisions Log). Tests must pass before any PR is merged.
...
### Gate a feature behind a commercial licence (v1.14.0 open-core)

ShipFlow's open-core licensing lives entirely in the `license` package (`LicenseService`, `LicenseLimits`, `LicenseFeatureAspect`). To make an existing or new feature commercial-only:

1. Pick a feature name matching (or add to) a licence payload's `features` array convention, e.g. `"sso"`, `"ldap"`, `"white-label"` — see `LicensePayload`'s Javadoc for the current list.
2. Annotate the Spring-bean **service** method (never a controller method directly — this is `@Aspect`-proxied, so it only intercepts calls that go through the Spring proxy, i.e. from a different bean) with `@RequiresLicensedFeature("your-feature-name")`.
3. That's it — `LicenseFeatureAspect` throws `LicenseFeatureException` (mapped to HTTP 403, messageKey `license.feature.required`) when `LicenseService.hasFeature(name)` is false. No manual `if (!licenseService.hasFeature(...))` checks needed at call sites.
4. To gate a **count/limit** instead of a binary feature (like the existing 10-user / 5-automation Community caps), extend `LicenseLimits` with a new cap method rather than hand-rolling a second enforcement mechanism — see `UserService.assertSeatAvailable()` / `WorkflowAutomationService.assertAutomationCanBeEnabled()` for the reference pattern (always check the cap BEFORE the state transition that would exceed it, never react by undoing/deactivating something that already exists).
5. Update the frontend: the Organization Settings → Licence tab already surfaces `features[]` from `GET /api/license/status` — if the gated feature has its own settings UI, disable/hide the relevant control based on that response rather than letting the request round-trip into a 403.
6. As of v1.14.0, **no shipped feature is gated this way yet** — the annotation and aspect exist as plumbing for the first real commercial feature to use.
...
| 12 | Tests: ≥ 50% line coverage (v1.13.1 baseline — see Architectural Decisions Log); **0 failures in the full suite** — fix any tests broken by your change (serialization, renamed fields, etc.) in the same PR | `src/test/` |
...
| 2026-09-01 | **A hill-chart scope's position must be averaged over EVERY task associated with it — the linked/root task's own status, its subtasks, AND any tasks linked via the independent `Task.scope` field — never a subset.** Two rounds of the same underlying bug class, both reported by Farzad on the same real production pitch (`ship.kixian.com/pitches/55/hill-chart`, deploys straight from `main`, no version tag needed). | **Round 1**: `calculatePositionFromTask`/`calculateSuggestedPosition` only averaged `linkedTask`'s direct children via `findByParentTaskIdNotDeleted` — `TaskRepository.findByScopeId`/`findByScopeIdAndStatus` existed but were never called from the progress path, so a task linked to the scope afterward via `Task.scope` (the "Link to Scope" picker, shown as "linked" in `PitchHillChart.tsx`'s Scope Summary panel) was invisible to the average; editing/adding such a task after scope creation never moved the hill chart. Fixed by unioning `linkedTask`'s subtasks with `taskRepository.findByScopeIdNotDeleted(scopeId)` (new soft-delete-aware query), deduplicated by task ID; also fixed `HillChartService#toDTO`'s "suggested position" preview, which bypassed the scope-aware path via a direct `calculatePositionFromTask(linkedTask)` call. **Round 2 (found live, same day, testing round 1's fix on the real pitch)**: still broken — scope "Event Expense Tracking..." showed 100% while its linked task "3. Money Overview" was BACKLOG and its one subtask "Design" was DONE. Root cause: the linked/root task's *own* status was dropped from the average entirely the instant it had ANY subtasks — `calculatePositionFromTask` computed `average(subtasks only)`, never including the task itself. `average(one DONE subtask) = 100`, regardless of the parent's real status. This is a common real usage pattern here: a root task tracking a whole feature (BACKLOG until all facets — design, backend, mobile, QA — are done) with only a "Design" subtask marked DONE early. Confirmed with Farzad (not obvious which behavior was "correct" — could have gone either way) that the task's own status should count as one of the averaged points, not be dropped: `average(BACKLOG=0, DONE=100) = 50%`. Fixed: `calculatePositionFromTask(Task)` and `collectScopeTasks(HillChartPoint)` now always add the task/linked-task itself to the list being averaged, before adding its subtasks / `Task.scope`-linked tasks. **Any future hill-chart progress code**: always go through `ScopeProgressService`'s scope-aware entry points (`calculateSuggestedPosition(Long)`/`(HillChartPoint)`), never hand-roll a subtask-only average — and remember the averaged set is `{task itself} ∪ {its subtasks} ∪ {Task.scope-linked tasks}`, deduplicated by ID, not any one of those alone. When testing a fix here, verify against real production data if available (`ship.kixian.com`) — the H2 test suite's synthetic scopes didn't happen to exercise a BACKLOG-root-with-one-DONE-subtask case, so round 2 shipped past `./mvnw verify` clean the first time. |
| 2026-09-06 | **`UserService.createUser`'s new registration-hardening checks (forced `app.auth.default-role`, `app.auth.public-registration` gate) apply ONLY to a non-admin caller — an authenticated ADMIN hitting `POST /api/auth/register` is exempt from both, exactly like the pre-existing `isCallerAdmin()` carve-out that already gated auto-project-assignment on this same endpoint.** Found while closing a public-hosting security report ("anyone can self-register as ADMIN"): `UserManagement.tsx`'s "Add User" feature has no dedicated backend endpoint — it POSTs to this exact `/api/auth/register` while authenticated, relying on that pre-existing admin check to get behavior (project auto-assignment) a genuine public caller doesn't. | A literal reading of "ignore the client role" and "gate registration when disabled" would have also broken admin-invoked user creation — on a production instance (where `app.auth.public-registration` defaults `false`), admins would have been unable to create MANAGER/READONLY/ADMIN teammate accounts at all, and even with registration open, every admin-created user would have silently gotten the default role instead of the one picked in the UI. Both new checks reuse the same `isCallerAdmin()` result (computed once in `createUser`, passed into `assignProjectsToNewUser` to avoid a second `findByUsername` lookup) rather than introducing a second admin-detection mechanism. Any future change to this endpoint's authorization must preserve that an authenticated ADMIN caller is a distinct, fully-trusted path — not merely "public registration with extra steps." Related: `DefaultAdminInitializer` was separately fixed the same day so the seeded `admin`/`admin123` account is created only when `userRepository.count() == 0` (a fresh database) and never re-created/re-activated afterwards (the old gate, `existsByUsername("admin")`, let a renamed or deleted admin account resurrect on the next restart); a `WARN` now logs every startup for as long as an "admin"-named account still matches the default password via `PasswordEncoder`. And `docker-compose.yml`/`docker-compose.monitoring.yml`/`docker-compose.ollama.yml` now bind Postgres/Qdrant/Prometheus/Grafana/Ollama to `127.0.0.1` (Redis was already loopback-only in `docker-compose.yml`) — the app's own port is unchanged and still needs a reverse proxy in front per `SECURITY.md`. **Frontend note for future sessions**: there is no public self-registration page in this codebase to put the 403 handling on — `Login.tsx` is sign-in-only (its own `login.*` i18n namespace has no counterpart to the vestigial top-level `auth.signUp`/`auth.noAccount`/`auth.hasAccount`/`auth.confirmPassword` keys, which belong to a registration UI that apparently existed at some point but is gone from the current `Login.tsx`), and `authService.register` has zero callers — the only frontend code that hits this endpoint is User Management's authenticated-admin "Add User" flow (`UserManagement.tsx`, exempt from this gate). So the 403 is instead handled centrally in `services/api.ts`'s shared response interceptor (special-cased on `messageKey === "auth.registration.disabled"`, same pattern as the existing 401/`AUTH_ENDPOINTS` carve-out), which covers every current and future caller without inventing a new public page as part of a security-hardening PR. If a real public sign-up page is wanted, that's a separate product decision. |
| 2026-09-07 | **JaCoCo's 80% line-coverage gate is now genuinely enforced for the first time — and set to 50%, not 80%, because real (now-measured) coverage is ~51%.** The gate had been silently non-functional since it was introduced: `maven-surefire-plugin`'s `<argLine>` was hardcoded to just `-Dnet.bytebuddy.experimental=true`, clobbering (not extending) the `argLine` property `jacoco-maven-plugin`'s `prepare-agent` execution injects to attach its instrumentation javaagent — so the agent never attached, `jacoco.exec` stayed empty, and JaCoCo's `check` goal silently skips evaluation against missing execution data instead of failing loudly. `BUILD SUCCESS` never meant "≥80% covered"; it meant "coverage was never checked." Found and fixed in the v1.13.1 audit pass (`@{argLine}` now composes instead of overwriting). | Fixing the argLine bug alone, with the threshold left at 80%, would have made `./mvnw verify` fail outright on `main` — real coverage is 51%, and closing a 29-point gap is many sessions of test-writing, not a patch-release fix. Farzad chose (explicitly, when asked) to set the gate to today's honest baseline (0.50, just under the real number) rather than either reverting the argLine fix (leaves the gate silently broken again) or blocking the release to write a large volume of tests first. **This is a real, intentional lowering of the enforced bar, not a bug** — any future session should ratchet the JaCoCo `<minimum>` in `backend/pom.xml` back up incrementally as real test coverage grows, never relax it further, and should not be surprised that "the coverage gate" now means 50% rather than the 80% still mentioned in older PRs/commit history from before this date. |
| 2026-09-07 | **v1.14.0 open-core licensing: `LicenseService`/`LicenseLimits`/`LicenseFeatureAspect` (package `license`) verify a licence file's Ed25519 signature over the RAW `payload` JSON node as received — never a re-derived serialization of the `LicensePayload` POJO — and a bad/missing/unverifiable licence always resolves to `MISSING` (Community Edition), logged, never a thrown exception.** `LicenseService.seatLimit()` returns the licensed seat count from the payload itself (0 if none); `LicenseLimits.activeUserCap()`/`enabledAutomationCap()` are the separate, deliberately different concept of the *enforced* cap after applying Community-fallback-or-licensed-buffer logic — callers needing "what's actually allowed right now" always go through `LicenseLimits`, never `LicenseService.seatLimit()` directly. This repo's actual `LICENSE` file (MIT) was deliberately left unchanged — the task's "moving to Elastic License 2.0" framing was treated as business-model *context* for why this feature exists, not a literal instruction to swap the repository's legal licence text, which is a distinct decision for Farzad to make explicitly, not something to fold into a feature PR. | Signing/verifying the raw `JsonNode` (not the POJO) means a future licence-issuer-side field addition never breaks verification on an older ShipFlow build, matching `LicensePayload`'s own `@JsonIgnoreProperties(ignoreUnknown = true)` intent — round-tripping through the POJO first would silently normalize away exactly the bytes the signature was computed over. Splitting "what the licence says" (`LicenseService`) from "what's currently enforced" (`LicenseLimits`) keeps the community-vs-licensed policy (10 users / 5 automations vs `seats × 1.1` / unlimited) in one place rather than duplicated at each of `UserService`/`WorkflowAutomationService`/`LicenseController`'s three call sites. **Mockito gotcha for future sessions** (see the existing "Mockito Service Mock Rule"): this PR added a new `LicenseLimits` constructor dependency to both `UserService` and `WorkflowAutomationService`; `UserServiceTest`/`WorkflowAutomationServiceTest` now carry a `@Mock private LicenseLimits licenseLimits;` field with a lenient default stub in `setUp()` (`activeUserCap()` → `Integer.MAX_VALUE`, `automationsUnlimited()` → `true`) specifically so the ~30 pre-existing, licensing-unrelated tests in those two classes keep passing — Mockito's un-stubbed default for `int` is `0` and for `boolean` is `false`, which would otherwise make every existing `createUser`/`create automation` test spuriously hit the Community cap. Any future test that constructs either service via `@InjectMocks` must add this same mock+stub, not just a bare `@Mock`. Separately, every `@SpringBootTest`-based test elsewhere in the suite that does NOT `@MockBean` `LicenseService`/`UserService`/`WorkflowAutomationService` boots the REAL `LicenseService`, which reads the committed `src/test/resources/license/test-license.json` (signed with a keypair whose public key is in `application-test.properties`; seats 100000, all features, expires 2099) — this is why the seat/automation caps don't spuriously break any other pre-existing integration test in the repo. |
| 2026-09-07 | **`PresenceService`'s Redis calls (v1.13.0's live-presence feature) must never be allowed to propagate an exception out of `heartbeat`/`leave`/`getViewers` — presence is ephemeral/best-effort, and `getViewers` is called synchronously, in-transaction, from `PitchSseService`/`WikiSseService`'s live-refresh broadcast during an ordinary pitch/wiki save.** Found because `main`'s own CI `build-and-test` job had been failing since the v1.13.0 tag (2026-09-06) and nobody had investigated it — `PitchControllerIntegrationTest`/`WikiControllerTest` both failed with `RedisConnectionFailureException: Unable to connect to Redis` bubbling all the way up through `updatePitch`/wiki-page-update into a 500, because CI's `application-test.properties` deliberately runs with no Redis (`spring.cache.type=simple`, `management.health.redis.enabled=false` — see that file's own comments), exactly the condition `PresenceService` didn't handle. Local `./mvnw verify` runs never caught this because local dev has a real Redis running via `docker compose`, masking the bug. | `NotificationSseManager` already established the right pattern for this exact class of dependency — every Redis call wrapped in try/catch, logging a warning and degrading gracefully (a `@Nullable RedisTemplate` for "not configured at all", a caught exception for "configured but unreachable") — specifically because SSE/presence infrastructure is fan-out/best-effort, not data any primary request path should depend on succeeding. `PresenceService` was added later (v1.13.0 S64) and didn't follow that precedent: it calls `StringRedisTemplate` directly with zero exception handling. Fixed by wrapping each method's Redis calls in try/catch (log.warn + safe fallback: no-op for `heartbeat`/`leave`, `List.of()` for `getViewers`) — same shape as `NotificationSseManager`, not a new pattern. **Any future addition to `PresenceService`, or any new service that calls Redis directly (not through the already-guarded `NotificationSseManager`), must follow this same never-let-it-throw discipline** — grep for `StringRedisTemplate`/`RedisTemplate` usages outside `NotificationSseManager` before assuming a new Redis-backed feature is safe to call from a primary save path. |

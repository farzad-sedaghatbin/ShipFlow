...
**Current version**: v1.11.0 (released 2026-07-28)
...
> **Scoping note (2026-08-04):** v1.6.0/S42 already built the plugin SPI (`RiskCalculatorPlugin`/`ReportGeneratorPlugin`/`IntegrationProviderPlugin`), `PluginRegistry`'s Spring-bean auto-discovery, and the admin/public list+enable/disable endpoints — S60 does **not** redo any of that. What S42 left unbuilt despite `plugin-sdk/README.md` documenting it: the `shipflow-plugin-archetype` Maven archetype module doesn't exist (only a hand-written `plugin-sdk/sample-plugin/` example does), there's no distributable `shipflow-plugin-api` jar (the SPI interfaces live inside the monolithic `backend` module), and there's no runtime JAR-loading/classloader — "drop a JAR into `plugins/`" is aspirational prose with no implementation behind it. See the Architectural Decisions Log entries below for the two scope calls this session map is built on.

| Session | Task | Status |
|---------|------|--------|
| S60 | Plugin SDK completion — real `shipflow-plugin-archetype` Maven archetype module (`mvn archetype:generate` actually works), a real distributable `shipflow-plugin-api` jar (SPI interfaces extracted out of `backend`, `provided`-scope dependency for plugin projects), a browsable plugin catalog UI extending `PluginAdminController`/`PublicPluginController`. Plugins still compile into the app (no dynamic runtime loading — see decisions log). Correct `plugin-sdk/README.md`'s now-inaccurate runtime-loading claim as part of this session. | planned |
| S61 | GitLab MCP client provider — `GitLabMcpConfig` in `McpConfig` (env-var gated, mirrors `GitHubMcpConfig`), `GitLabMcpProvider implements McpClientService` following `GitHubMcpProvider`'s JSON-RPC `tools/call` pattern exactly, PAT/OAuth token via the existing org-settings integration-secret convention. | planned |
| S62 | Azure DevOps MCP client provider — same shape as S61 (`AzureDevOpsMcpConfig`, `AzureDevOpsMcpProvider implements McpClientService`), adapted for Azure DevOps' REST API/auth model. | planned |
...
| 2026-08-04 | v1.12.0's GitLab/Azure DevOps integrations use the **MCP client pattern** (`McpClientService`, like `GitHubMcpProvider`/`FigmaMcpProvider`), not the S42 plugin SPI (`IntegrationProviderPlugin`) | GitLab and Azure DevOps are direct analogs of the one existing similar integration (GitHub) — consistency beats novelty here. `IntegrationProviderPlugin` has zero real implementations and no test coverage beyond the registry mechanics, so building on it first would mean hardening an unproven interface at the same time as shipping the actual integrations. Unifying the plugin SPI and MCP-client registration into one shared abstraction was considered and explicitly deferred — a real architecture project of its own, not bundled into two new provider integrations. |
| 2026-08-04 | v1.12.0's "plugin registry" stays **compile-time** (real Maven archetype + real `shipflow-plugin-api` jar + a browsable catalog UI) — no dynamic runtime JAR loading from a `plugins/` directory, despite `plugin-sdk/README.md` documenting that capability | Runtime JAR loading means executing third-party code inside the main process with no sandboxing/signing story — a real arbitrary-code-execution surface that would need its own security design before it's safe to ship, not something to bolt onto a milestone themed around developer-experience scaffolding. The lighter scope still closes the actual gap (the archetype and API jar are documented but don't exist — see the v1.12.0 session map's scoping note) without taking on that risk. Revisit dynamic loading as its own future milestone if a real marketplace (external, third-party-authored plugins) becomes a product priority. |

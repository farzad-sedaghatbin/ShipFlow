...
import { Sparkles, Loader2, Wand2, RotateCcw, CheckCircle2, AlertCircle } from 'lucide-react';
...
interface SuggestedSprintTasksPanelProps {
  cycleId: number;
...
export function SuggestedSprintTasksPanel({ cycleId, onTasksCreated }: SuggestedSprintTasksPanelProps) {
  ...
  const { data: status } = useQuery({
    queryKey: ['sprint-task-suggestions-status'],
    queryFn: sprintTaskSuggestionService.getStatus,
  });
  ...
  const handleGenerate = async () => {
    ...
    setErrorMessage('');
    try {
      const response = await sprintTaskSuggestionService.generate(cycleId);
  ...
  const handleCreateSelected = async () => {
    ...
    setCreating(true);
    try {
      const res = await taskService.bulkCreate({ cycleId, tasks: tasksToCreate });
      ...
      <Dialog open={dialogOpen} onOpenChange={handleOpenChange}>
        <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>{t('suggestedSprintTasksPanel.dialogSubtitle')}</DialogDescription>
          </DialogHeader>
          ...
          {state === 'loading' && (
            <div className="flex flex-col items-center gap-3 text-center py-8" aria-live="polite">
              ...
              <p className="text-sm text-muted-foreground">{t('suggestedSprintTasksPanel.loading')}</p>
            </div>
          )}
                          ...
                          <div className="flex-1 min-w-0 space-y-1.5">
                            <div className="flex items-center gap-2 flex-wrap">
                              ...
                              <Badge variant="outline">
                                {t('suggestedSprintTasksPanel.sourceSprint')}
                              </Badge>

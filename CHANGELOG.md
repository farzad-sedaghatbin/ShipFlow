...
- **Compare before restore**: Choosing a revision in the history panel now opens a **side-by-side diff** (current vs. that revision) with added/removed/changed lines highlighted and a title diff, so you confirm the change before restoring. New `GET /wiki/pages/{id}/revisions/{revision}` returns a single revision's full content; the diff is computed client-side per BlockNote block (`wikiDiff.ts`, LCS-based) since the stored `contentText` is space-joined.

...
- **Sharing a filtered Bug Reports view could silently drop the active project**: `pitch`/`cycle`/`release` filters are project-scoped IDs, but the "Copy shareable link" URL never encoded *which* project they belonged to — opening the link applied those IDs against whatever project happened to be active for whoever opened it (often a different one, especially in a fresh tab/session), returning zero results with no obvious explanation, and making project-scoped dropdowns (Pitch, Cycle, Release) appear to have silently lost their selection even though the raw filter values were technically still in the URL. `BugReportsPage`'s URL-sync effect now also writes `project=<id>`, reusing the exact deep-link param `ProjectContext` already reads on load for "open project in new tab" — no new mechanism, just wiring an existing one into this page's share link.

...
class VectorStoreProviderTypeTest {
  ...
  void fromConfigValue_shouldReturnCorrectType() {
    assertEquals(
        VectorStoreProviderType.IN_MEMORY, VectorStoreProviderType.fromConfigValue("in-memory"));
    ...
    assertEquals(
        VectorStoreProviderType.PINECONE, VectorStoreProviderType.fromConfigValue("pinecone"));
    assertEquals(
        VectorStoreProviderType.WEAVIATE, VectorStoreProviderType.fromConfigValue("weaviate"));
  }
  ...
  void fromConfigValue_shouldHandleWhitespace() {
    assertEquals(
        VectorStoreProviderType.QDRANT, VectorStoreProviderType.fromConfigValue("  qdrant  "));
    assertEquals(
        VectorStoreProviderType.CHROMA, VectorStoreProviderType.fromConfigValue(" chroma "));
  }

  @Test
  void fromConfigValue_shouldThrowExceptionForNullOrEmpty() {
    assertThrows(
        IllegalArgumentException.class, () -> VectorStoreProviderType.fromConfigValue(null));
    assertThrows(IllegalArgumentException.class, () -> VectorStoreProviderType.fromConfigValue(""));
    assertThrows(
        IllegalArgumentException.class, () -> VectorStoreProviderType.fromConfigValue("   "));
  }
  ...
  void getDisplayName_shouldReturnCorrectName() {
    assertEquals(
        "In-Memory (Development/Testing)", VectorStoreProviderType.IN_MEMORY.getDisplayName());
    assertEquals(
        "Qdrant (Production Recommended)", VectorStoreProviderType.QDRANT.getDisplayName());

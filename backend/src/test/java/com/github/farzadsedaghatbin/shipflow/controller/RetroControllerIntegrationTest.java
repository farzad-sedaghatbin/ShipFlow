...
class RetroControllerIntegrationTest {
  ...
  void setUp() {
    ...
    testProject = projectRepository.save(Project.builder().name("Test Project").projectKey("RETRO").isActive(true)
        .enableRetrospectives(true).createdAt(LocalDateTime.now()).build());
  ...
  class RetroCrudTests {
    ...
    void createRetroSucceeds() throws Exception {
      CreateRetroRequest request = CreateRetroRequest.builder().title("Test Retrospective").notes("Test notes")
          .cycleId(testCycle.getId()).projectId(testProject.getId()).build();

      mockMvc.perform(post("/api/retros").contentType(MediaType.APPLICATION_JSON)
    ...
    void getRetroByIdSucceeds() throws Exception {
      Retrospective retro = retroRepository
          .save(Retrospective.builder().title("Test Retro").status(RetroStatus.DRAFT).cycle(testCycle)
              .project(testProject).createdAt(LocalDateTime.now()).build());

      mockMvc.perform(get("/api/retros/" + retro.getId())).andExpect(status().isOk())
          .andExpect(jsonPath("$.title", is("Test Retro"))).andExpect(jsonPath("$.status", is("DRAFT")));
    }

    @Test
    @DisplayName("Get retrospectives by project succeeds")
    void getRetrosByProjectSucceeds() throws Exception {
      retroRepository.save(Retrospective.builder().title("Retro 1").status(RetroStatus.DRAFT).cycle(testCycle)
          .project(testProject).createdAt(LocalDateTime.now()).build());

      retroRepository.save(Retrospective.builder().title("Retro 2").status(RetroStatus.OPEN).cycle(testCycle)
          .project(testProject).createdAt(LocalDateTime.now()).build());

      mockMvc.perform(get("/api/retros/project/" + testProject.getId())).andExpect(status().isOk())
    ...
    void updateRetroSucceeds() throws Exception {
      Retrospective retro = retroRepository
          .save(Retrospective.builder().title("Original Title").status(RetroStatus.DRAFT).cycle(testCycle)
              .project(testProject).createdAt(LocalDateTime.now()).build());

      UpdateRetroRequest updateRequest = UpdateRetroRequest.builder().title("Updated Title")
          .notes("Updated notes").build();

      mockMvc.perform(put("/api/retros/" + retro.getId()).contentType(MediaType.APPLICATION_JSON)
    ...
    void deleteRetroSucceeds() throws Exception {
      Retrospective retro = retroRepository
          .save(Retrospective.builder().title("To Delete").status(RetroStatus.DRAFT).cycle(testCycle)
              .project(testProject).createdAt(LocalDateTime.now()).build());
  ...
  class RetroStatusTests {
    ...
    void openRetroSucceeds() throws Exception {
      Retrospective retro = retroRepository
          .save(Retrospective.builder().title("Draft Retro").status(RetroStatus.DRAFT).cycle(testCycle)
              .project(testProject).createdAt(LocalDateTime.now()).build());

      mockMvc.perform(post("/api/retros/" + retro.getId() + "/open")).andExpect(status().isOk())
    ...
    void closeRetroSucceeds() throws Exception {
      Retrospective retro = retroRepository
          .save(Retrospective.builder().title("Open Retro").status(RetroStatus.OPEN).cycle(testCycle)
              .project(testProject).createdAt(LocalDateTime.now()).build());

      mockMvc.perform(post("/api/retros/" + retro.getId() + "/close")).andExpect(status().isOk())
          .andExpect(jsonPath("$.status", is("CLOSED"))).andExpect(jsonPath("$.closedAt", notNullValue()));
    }
    ...
    void cannotOpenClosedRetro() throws Exception {
      Retrospective retro = retroRepository
          .save(Retrospective.builder().title("Closed Retro").status(RetroStatus.CLOSED).cycle(testCycle)
              .project(testProject).createdAt(LocalDateTime.now()).closedAt(LocalDateTime.now()).build());

      mockMvc.perform(post("/api/retros/" + retro.getId() + "/open")).andExpect(status().is5xxServerError());
    }
  }
  ...
  class RetroItemTests {
    ...
    @BeforeEach
    void setUpRetro() {
      openRetro = retroRepository.save(Retrospective.builder().title("Open Retro").status(RetroStatus.OPEN)
    ...
    void addItemSucceeds() throws Exception {
      CreateRetroItemRequest request = CreateRetroItemRequest.builder().content("Something went well")
          .columnType(RetroColumnType.WENT_WELL).retrospectiveId(openRetro.getId()).build();

      mockMvc.perform(post("/api/retros/items").contentType(MediaType.APPLICATION_JSON)
          .content(objectMapper.writeValueAsString(request))).andExpect(status().isCreated())
    ...
    void addAnonymousItemSucceeds() throws Exception {
      CreateRetroItemRequest request = CreateRetroItemRequest.builder()
          .content("Sensitive feedback that should be anonymous").columnType(RetroColumnType.DID_NOT_GO_WELL)
          .retrospectiveId(openRetro.getId()).isAnonymous(true).build();
      ...
      mockMvc.perform(post("/api/retros/items").contentType(MediaType.APPLICATION_JSON)
          ...
          .andExpect(jsonPath("$.isAnonymous", is(true))).andExpect(jsonPath("$.author").doesNotExist());
    }
    ...
    void addNonAnonymousItemShowsAuthor() throws Exception {
      CreateRetroItemRequest request = CreateRetroItemRequest.builder()
          .content("Public feedback with attribution").columnType(RetroColumnType.WENT_WELL)
          .retrospectiveId(openRetro.getId()).isAnonymous(false).build();

      mockMvc.perform(post("/api/retros/items").contentType(MediaType.APPLICATION_JSON)
          .content(objectMapper.writeValueAsString(request))).andExpect(status().isCreated())
          .andExpect(jsonPath("$.content", is("Public feedback with attribution")))
          .andExpect(jsonPath("$.isAnonymous", is(false))).andExpect(jsonPath("$.authorId", notNullValue()))
    ...
    void getItemsSucceeds() throws Exception {
      retroItemRepository.save(RetroItem.builder().content("Item 1").columnType(RetroColumnType.WENT_WELL)
          .retrospective(openRetro).createdAt(LocalDateTime.now()).build());

      retroItemRepository.save(RetroItem.builder().content("Item 2").columnType(RetroColumnType.TRY_NEXT)
          .retrospective(openRetro).createdAt(LocalDateTime.now()).build());

      mockMvc.perform(get("/api/retros/" + openRetro.getId() + "/items")).andExpect(status().isOk())
          .andExpect(jsonPath("$", hasSize(2)));
    }
    ...
    void updateItemSucceeds() throws Exception {
      RetroItem item = retroItemRepository
          .save(RetroItem.builder().content("Original").columnType(RetroColumnType.ACTIONS)
              .retrospective(openRetro).createdAt(LocalDateTime.now()).build());

      mockMvc.perform(put("/api/retros/items/" + item.getId()).contentType(MediaType.APPLICATION_JSON)
          .content(objectMapper.writeValueAsString(Map.of("content", "Updated")))).andExpect(status().isOk())
    ...
    void deleteItemSucceeds() throws Exception {
      RetroItem item = retroItemRepository
          .save(RetroItem.builder().content("To delete").columnType(RetroColumnType.DID_NOT_GO_WELL)
              .retrospective(openRetro).createdAt(LocalDateTime.now()).build());

      mockMvc.perform(delete("/api/retros/items/" + item.getId())).andExpect(status().isNoContent());
    }
  ...
  class CycleRetroStatusTests {
    ...
    void getCycleStatusNoRetros() throws Exception {
      mockMvc.perform(get("/api/retros/cycle/" + testCycle.getId() + "/status")).andExpect(status().isOk())
          .andExpect(jsonPath("$.totalRetros", is(0))).andExpect(jsonPath("$.closedRetros", is(0)))
    ...
    void getCycleStatusWithClosedRetro() throws Exception {
      ...
      mockMvc.perform(get("/api/retros/cycle/" + testCycle.getId() + "/status")).andExpect(status().isOk())
          .andExpect(jsonPath("$.totalRetros", is(1))).andExpect(jsonPath("$.closedRetros", is(1)))
    ...
    void canCloseCycleEndpoint() throws Exception {
      ...
      mockMvc.perform(get("/api/retros/cycle/" + testCycle.getId() + "/can-close")).andExpect(status().isOk())
  ...
  class ProjectSettingTests {
    ...
    void checkRetrosEnabled() throws Exception {
      mockMvc.perform(get("/api/retros/project/" + testProject.getId() + "/enabled")).andExpect(status().isOk())
    ...
    void disableRetros() throws Exception {
      mockMvc.perform(put("/api/retros/project/" + testProject.getId() + "/enabled")
  ...
  class AuthorizationTests {
    ...
    void nonAdminCannotDelete() throws Exception {
      Retrospective retro = retroRepository
          .save(Retrospective.builder().title("Cannot Delete").status(RetroStatus.DRAFT).cycle(testCycle)
              .project(testProject).createdAt(LocalDateTime.now()).build());
    ...
    void nonAdminCannotClose() throws Exception {
      Retrospective retro = retroRepository
          .save(Retrospective.builder().title("Cannot Close").status(RetroStatus.OPEN).cycle(testCycle)
              .project(testProject).createdAt(LocalDateTime.now()).build());

      mockMvc.perform(post("/api/retros/" + retro.getId() + "/close")).andExpect(status().isForbidden());
    }

...
/**
 * Unit tests for SlackIntegrationController
 * Tests REST API endpoints for Slack integration using MockMvc
 */
...
class SlackIntegrationControllerTest {
    ...
    void createChannelConfig_ShouldReturn201() throws Exception {
        ...
        SlackChannelConfigDTO response = SlackChannelConfigDTO.builder()
                .id(1L)
                .slackConfigId(1L)
                .channelName("engineering")
                .notifyTaskAssigned(true)
        ...
        // When & Then
        mockMvc.perform(post("/api/slack/configurations/1/channels")
                        .with(csrf())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(request)))
                .andExpect(status().isCreated())
    ...
    void getChannelConfigs_ShouldReturnList() throws Exception {
        // Given
        SlackChannelConfigDTO config = SlackChannelConfigDTO.builder()
                .id(1L)
                .slackConfigId(1L)
                .channelName("engineering")
                .notifyTaskAssigned(true)
    ...
    void sendTestNotification_ShouldReturn200() throws Exception {
        // Given
        TestSlackNotificationRequest request = TestSlackNotificationRequest.builder()

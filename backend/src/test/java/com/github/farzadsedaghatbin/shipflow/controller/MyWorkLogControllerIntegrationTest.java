...
/**
 * Integration tests for the "My Work Logs" feature (/api/worklogs/my endpoints)
...
class MyWorkLogControllerIntegrationTest {
  ...
  void setUp() {
    ...
    testPerson = Person.builder().name("Test User").email("testuser@example.com").isActive(true)
        .createdAt(LocalDateTime.now()).build();
    testPerson = personRepository.save(testPerson);

    // Create another person for ownership tests
    otherPerson = Person.builder().name("Other User").email("other@example.com").isActive(true)
        .createdAt(LocalDateTime.now()).build();
    otherPerson = personRepository.save(otherPerson);

    // Create user linked to testPerson
    testUser = User.builder().username("testuser").password(passwordEncoder.encode("password"))
        .email("testuser@example.com").role(UserRole.MEMBER).person(testPerson).isActive(true)
        .createdAt(LocalDateTime.now()).build();
    ...
    otherPitch = Pitch.builder().title("Other Pitch").description("Another Description").appetiteDays(4)
        .status(PitchStatus.IN_PROGRESS).cycle(testCycle).createdAt(LocalDateTime.now())
        .updatedAt(LocalDateTime.now()).build();
    otherPitch = pitchRepository.save(otherPitch);

    // Create work log for test user
    testWorkLog = WorkLog.builder().note("My Work").hoursSpent(new BigDecimal("4.00")).date(LocalDate.now())
        .person(testPerson).pitch(testPitch).build();
    testWorkLog = workLogRepository.save(testWorkLog);

    // Create work log for other person
    otherPersonWorkLog = WorkLog.builder().note("Other's Work").hoursSpent(new BigDecimal("3.00"))
        .date(LocalDate.now()).person(otherPerson).pitch(testPitch).build();
  ...
  void createMyWorkLog_WithValidData_ShouldCreateWorkLog() throws Exception {
    CreateWorkLogForSelfRequest request = CreateWorkLogForSelfRequest.builder().pitchId(otherPitch.getId())
        .date(LocalDate.now()).hoursSpent(new BigDecimal("2.50")).note("New self work log").build();

    mockMvc.perform(post("/api/worklogs/my").contentType(MediaType.APPLICATION_JSON)
        .content(objectMapper.writeValueAsString(request))).andExpect(status().isCreated())
        .andExpect(jsonPath("$.note", is("New self work log")))
        .andExpect(jsonPath("$.personName", is("Test User"))).andExpect(jsonPath("$.hoursSpent", is(2.50)));
  }

  @Test
  @WithMockUser(username = "testuser", roles = {"USER"})
  void createMyWorkLog_WithInvalidPitch_ShouldReturn400() throws Exception {
    CreateWorkLogForSelfRequest request = CreateWorkLogForSelfRequest.builder().pitchId(9999L).date(LocalDate.now())
        .hoursSpent(new BigDecimal("2.50")).note("Invalid pitch").build();
  ...
  void updateMyWorkLog_WhenOwned_ShouldUpdateWorkLog() throws Exception {
    CreateWorkLogForSelfRequest request = CreateWorkLogForSelfRequest.builder().pitchId(testPitch.getId())
        .date(LocalDate.now()).hoursSpent(new BigDecimal("6.00")).note("Updated my work").build();

    mockMvc.perform(put("/api/worklogs/my/{id}", testWorkLog.getId()).contentType(MediaType.APPLICATION_JSON)
        .content(objectMapper.writeValueAsString(request))).andExpect(status().isOk())
        .andExpect(jsonPath("$.note", is("Updated my work"))).andExpect(jsonPath("$.hoursSpent", is(6.00)));
  }
  ...
  void updateMyWorkLog_WhenNotOwned_ShouldReturn400() throws Exception {
    CreateWorkLogForSelfRequest request = CreateWorkLogForSelfRequest.builder().pitchId(testPitch.getId())
        .date(LocalDate.now()).hoursSpent(new BigDecimal("6.00")).note("Trying to update other's work").build();

    mockMvc.perform(put("/api/worklogs/my/{id}", otherPersonWorkLog.getId()).contentType(MediaType.APPLICATION_JSON)
        .content(objectMapper.writeValueAsString(request))).andExpect(status().isBadRequest());
  }
  ...
  void deleteMyWorkLog_WhenNotOwned_ShouldReturn400() throws Exception {
    mockMvc.perform(delete("/api/worklogs/my/{id}", otherPersonWorkLog.getId())).andExpect(status().isBadRequest());
  }
  ...
  void createMyWorkLog_WithoutAuthentication_ShouldReturn401() throws Exception {
    CreateWorkLogForSelfRequest request = CreateWorkLogForSelfRequest.builder().pitchId(testPitch.getId())
        .date(LocalDate.now()).hoursSpent(new BigDecimal("2.50")).note("Unauthorized").build();

    mockMvc.perform(post("/api/worklogs/my").contentType(MediaType.APPLICATION_JSON)
        .content(objectMapper.writeValueAsString(request))).andExpect(status().isFound()); // 302 redirect to
    // login
  }
}

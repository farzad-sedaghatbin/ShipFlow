...
class BettingTableControllerIntegrationTest {
    ...
    void getBettingTable_ShouldReturnBettingTableForCycle() throws Exception {
        mockMvc.perform(get("/api/betting/cycle/{cycleId}", testCycle.getId()))
    ...
    void getBettingTable_WhenCycleNotFound_ShouldReturn400() throws Exception {
        mockMvc.perform(get("/api/betting/cycle/{cycleId}", 9999L))
                .andExpect(status().isBadRequest());
    }

    @Test
    void getShapedPitches_ShouldReturnAvailablePitches() throws Exception {
        mockMvc.perform(get("/api/betting/shaped-pitches"))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
    ...
    void createSlot_WithValidData_ShouldCreateSlot() throws Exception {
        CreateBettingSlotRequest request = CreateBettingSlotRequest.builder()
        ...
        mockMvc.perform(post("/api/betting/slots")
    ...
    void assignPitchToSlot_ShouldAssignSuccessfully() throws Exception {
        mockMvc.perform(patch("/api/betting/slots/{slotId}/assign/{pitchId}", 
                        testSlot.getId(), shapedPitch.getId()))
                .andExpect(status().isOk())
    ...
    void assignPitchToSlot_WhenPitchTooLarge_ShouldReturn400() throws Exception {
        ...
        Pitch largePitch = Pitch.builder()
                .title("Huge Feature")
        ...
        // Create a small slot
        BettingSlot smallSlot = BettingSlot.builder()
                .cycle(testCycle)
                .team(testTeam)
                .position(2)
                .startDate(LocalDate.now())
        ...
        mockMvc.perform(patch("/api/betting/slots/{slotId}/assign/{pitchId}", 
                        smallSlot.getId(), largePitch.getId()))
                .andExpect(status().isBadRequest());
    }
    ...
    void removePitchFromSlot_ShouldRemoveSuccessfully() throws Exception {
        ...
        mockMvc.perform(patch("/api/betting/slots/{slotId}/unassign", testSlot.getId()))
                .andExpect(status().isOk())
    ...
    void deleteSlot_ShouldDeleteSuccessfully() throws Exception {
        mockMvc.perform(delete("/api/betting/slots/{slotId}", testSlot.getId()))
    ...
    void canPitchFitInSlot_WhenFits_ShouldReturnTrue() throws Exception {
        mockMvc.perform(get("/api/betting/slots/{slotId}/can-fit/{pitchId}", 
                        testSlot.getId(), shapedPitch.getId()))
                .andExpect(status().isOk())
    ...
    void canPitchFitInSlot_WhenDoesNotFit_ShouldReturnFalse() throws Exception {
        ...
        mockMvc.perform(get("/api/betting/slots/{slotId}/can-fit/{pitchId}", 
                        testSlot.getId(), largePitch.getId()))
                .andExpect(status().isOk())

...
class CustomMetricServiceTest {
  ...
  void setUp() {
    lenient().when(messageService.getMessage(anyString(), any(Object[].class))).thenAnswer(i -> {
      ...
      if (key.contains("metric.not.found"))
        return "Custom metric not found";
      if (key.contains("metric.formula.invalid"))
        return "Invalid formula: validation error";
      if (key.contains("metric.name.exists"))
    ...
    testMetric = CustomMetric.builder().id(1L).user(testUser).name("Test Metric").description("Test Description")
        .formula("SUM(hours)").dataSource(CustomMetric.DataSource.WORK_LOG)
  ...
  class CreateMetricTests {
    ...
    void shouldCreateMetric() {
      // Arrange
      CreateCustomMetricRequest request = CreateCustomMetricRequest.builder().name("New Metric")
          .description("Description").formula("10 + 20").dataSource(CustomMetric.DataSource.CUSTOM)
          .aggregationType(CustomMetric.AggregationType.SUM).displayFormat(CustomMetric.DisplayFormat.NUMBER)
          .isActive(true).build();
    ...
    void shouldThrowExceptionForInvalidFormula() {
      ...
      CreateCustomMetricRequest request = CreateCustomMetricRequest.builder().name("Invalid Metric")
          .formula("INVALID(formula").dataSource(CustomMetric.DataSource.CUSTOM).build();
    ...
    void shouldThrowExceptionForDuplicateName() {
      // Arrange
      CreateCustomMetricRequest request = CreateCustomMetricRequest.builder().name("Duplicate Metric")
          .formula("10 + 20").dataSource(CustomMetric.DataSource.CUSTOM).build();
    ...
    void shouldThrowExceptionWhenUserNotFound() {
      // Arrange
      CreateCustomMetricRequest request = CreateCustomMetricRequest.builder().name("Metric").formula("10")
          .dataSource(CustomMetric.DataSource.CUSTOM).build();
      ...
      // Act & Assert
      assertThrows(IllegalArgumentException.class, () -> customMetricService.createMetric(999L, request));
    }
  }
  ...
  class UpdateMetricTests {
    ...
    void shouldThrowExceptionWhenMetricNotFound() {
      // Arrange
      UpdateCustomMetricRequest request = UpdateCustomMetricRequest.builder().name("Name").build();

      when(customMetricRepository.findById(999L)).thenReturn(Optional.empty());

      // Act & Assert
      assertThrows(IllegalArgumentException.class, () -> customMetricService.updateMetric(1L, 999L, request));
    }
  ...
  class DeleteMetricTests {
    ...
    void shouldThrowExceptionWhenDeletingNonExistentMetric() {
      ...
      // Act & Assert
      assertThrows(IllegalArgumentException.class, () -> customMetricService.deleteMetric(1L, 999L));
    }
    ...
    void shouldThrowExceptionWhenNotAuthorizedToDelete() {
      ...
      // Act & Assert
      assertThrows(SecurityException.class, () -> customMetricService.deleteMetric(999L, 1L));
    }
  }
  ...
  class GetMetricTests {
    ...
    void shouldGetAllMetricsForUser() {
      // Arrange
      CustomMetric metric2 = CustomMetric.builder().id(2L).user(testUser).name("Metric 2").formula("20")
    ...
    void shouldThrowExceptionWhenGettingNonExistentMetric() {
      ...
      // Act & Assert
      assertThrows(IllegalArgumentException.class, () -> customMetricService.getMetric(1L, 999L));
    }
    ...
    void shouldThrowExceptionWhenNotAuthorizedToView() {
      ...
      // Act & Assert
      assertThrows(SecurityException.class, () -> customMetricService.getMetric(999L, 1L));
    }
  }
  ...
  class CalculateMetricTests {
    ...
    void shouldCalculateMetricValue() {
      ...
      when(metricDataPointRepository.save(any(MetricDataPoint.class)))
    ...
    void shouldThrowExceptionWhenCalculatingNonExistentMetric() {
      ...
      // Act & Assert
      assertThrows(IllegalArgumentException.class, () -> customMetricService.calculateMetricValue(1L, 999L));
    }
    ...
    void shouldThrowExceptionWhenNotAuthorizedToCalculate() {
      ...
      // Act & Assert
      assertThrows(SecurityException.class, () -> customMetricService.calculateMetricValue(999L, 1L));
    }
  }
  ...
  class MetricHistoryTests {
    ...
    void shouldGetMetricHistory() {
      ...
      List<MetricDataPoint> dataPoints = Arrays.asList(
          MetricDataPoint.builder().metric(testMetric).calculatedValue(new BigDecimal("100"))
              .calculationDate(LocalDateTime.now()).metadata("{}").build(),
          MetricDataPoint.builder().metric(testMetric).calculatedValue(new BigDecimal("150"))
              .calculationDate(LocalDateTime.now().minusDays(1)).metadata("{}").build());
  ...
  class FormulaValidationTests {
    ...
    void shouldReturnErrorForInvalidFormula() {
      // Arrange
      when(formulaParser.validateFormula("invalid("))
          .thenReturn(ValidationResult.error("Unbalanced parentheses"));
  ...
  class ScopeFeatureTests {
    ...
    void shouldCreateMetricWithCycleScope() {
      // Arrange
      CreateCustomMetricRequest request = CreateCustomMetricRequest.builder().name("Cycle Metric")
          .description("Metric for specific cycle").formula("10 + 20")
          .dataSource(CustomMetric.DataSource.CUSTOM).aggregationType(CustomMetric.AggregationType.SUM)
          .displayFormat(CustomMetric.DisplayFormat.NUMBER).cycleId(10L).isActive(true).build();
      ...
      verify(customMetricRepository)
          .save(argThat(metric -> metric.getCycle() != null && metric.getCycle().getId().equals(10L)));
    }
    ...
    void shouldCreateMetricWithPitchScope() {
      // Arrange
      CreateCustomMetricRequest request = CreateCustomMetricRequest.builder().name("Pitch Metric")
          .description("Metric for specific pitch").formula("COUNT(tasks)")
          .dataSource(CustomMetric.DataSource.TASK).aggregationType(CustomMetric.AggregationType.COUNT)
          .displayFormat(CustomMetric.DisplayFormat.NUMBER).pitchId(20L).isActive(true).build();
      ...
      verify(customMetricRepository)
          .save(argThat(metric -> metric.getPitch() != null && metric.getPitch().getId().equals(20L)));
    }
    ...
    void shouldCreateMetricWithTeamScope() {
      // Arrange
      CreateCustomMetricRequest request = CreateCustomMetricRequest.builder().name("Team Metric")
          .description("Metric for specific team").formula("AVG(velocity)")
          .dataSource(CustomMetric.DataSource.CUSTOM).aggregationType(CustomMetric.AggregationType.AVG)
          .displayFormat(CustomMetric.DisplayFormat.DECIMAL).teamId(30L).isActive(true).build();
      ...
      verify(customMetricRepository)
          .save(argThat(metric -> metric.getTeam() != null && metric.getTeam().getId().equals(30L)));
    }
    ...
    void shouldCreateMetricWithMultipleScopes() {
      // Arrange
      CreateCustomMetricRequest request = CreateCustomMetricRequest.builder().name("Multi-Scope Metric")
          .description("Metric with cycle, pitch, and team scope").formula("SUM(hours)")
          .dataSource(CustomMetric.DataSource.WORK_LOG).aggregationType(CustomMetric.AggregationType.SUM)
          .displayFormat(CustomMetric.DisplayFormat.NUMBER).cycleId(10L).pitchId(20L).teamId(30L)
          .isActive(true).build();
    ...
    void shouldThrowExceptionForInvalidCycleId() {
      // Arrange
      CreateCustomMetricRequest request = CreateCustomMetricRequest.builder().name("Invalid Cycle Metric")
          .formula("10 + 20").dataSource(CustomMetric.DataSource.CUSTOM).cycleId(999L).isActive(true).build();
    ...
    void shouldThrowExceptionForInvalidPitchId() {
      // Arrange
      CreateCustomMetricRequest request = CreateCustomMetricRequest.builder().name("Invalid Pitch Metric")
          .formula("COUNT(*)").dataSource(CustomMetric.DataSource.PITCH).pitchId(999L).isActive(true).build();
    ...
    void shouldThrowExceptionForInvalidTeamId() {
      // Arrange
      CreateCustomMetricRequest request = CreateCustomMetricRequest.builder().name("Invalid Team Metric")
          .formula("AVG(velocity)").dataSource(CustomMetric.DataSource.CUSTOM).teamId(999L).isActive(true)
          .build();
    ...
    void shouldCreateGlobalMetric() {
      // Arrange
      CreateCustomMetricRequest request = CreateCustomMetricRequest.builder().name("Global Metric")
          .description("Organization-wide metric").formula("SUM(all_hours)")
          .dataSource(CustomMetric.DataSource.WORK_LOG).aggregationType(CustomMetric.AggregationType.SUM)
          .displayFormat(CustomMetric.DisplayFormat.NUMBER).isActive(true).build();

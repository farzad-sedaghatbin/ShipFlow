 ...
 * Tests pattern matching, auto-linking, and auto-closing functionality
 */
...
class GitHubIntegrationServiceTest {
    ...
    void setUp() {
        ...
        testConfig = GitHubConfiguration.builder()
                .id(1L)
                .repository(testRepo)
    ...
    void processCommit_WithJiraStyleReference_ShouldAutoLinkToTask() {
        ...
        when(githubRepoRepository.findByFullName("testorg/testrepo"))
                .thenReturn(Optional.of(testRepo));
        when(commitRepository.findBySha(sha))
                .thenReturn(Optional.empty());
        when(configurationRepository.findByRepositoryId(1L))
                .thenReturn(Optional.of(testConfig));
        when(taskRepository.findById(42L))
                .thenReturn(Optional.of(testTask));
        when(taskLinkRepository.findByTaskId(42L))
                .thenReturn(List.of());
        when(commitRepository.save(any(GitHubCommit.class)))
        ...
        // When
        GitHubCommit result = service.processCommit(
                "testorg/testrepo", sha, commitMessage,
        ...
        verify(taskLinkRepository).save(argThat(link ->
                link.getTask().getId().equals(42L) &&
                link.getLinkType() == GitHubLinkType.COMMIT &&
                link.getAutoLinked()
        ));
    }
    ...
    void processCommit_WithMultipleTaskReferences_ShouldLinkToAll() {
        ...
        when(taskRepository.findById(100L))
                .thenReturn(Optional.of(testTask2));
        when(taskLinkRepository.findByTaskId(anyLong()))
                .thenReturn(List.of());
        when(commitRepository.save(any(GitHubCommit.class)))
                .thenAnswer(invocation -> {
                    GitHubCommit commit = invocation.getArgument(0);
    ...
    void processPullRequest_WithClosesKeyword_ShouldAutoCloseTaskWhenMerged() {
        ...
        when(githubRepoRepository.findByFullName("testorg/testrepo"))
                .thenReturn(Optional.of(testRepo));
        when(pullRequestRepository.findByRepositoryIdAndPrNumber(1L, 123))
                .thenReturn(Optional.empty());
        when(configurationRepository.findByRepositoryId(1L))
                .thenReturn(Optional.of(testConfig));
        when(taskRepository.findById(42L))
        ...
        when(taskLinkRepository.findByTaskId(42L))
                .thenReturn(List.of());
        when(pullRequestRepository.save(any(GitHubPullRequest.class)))
                .thenAnswer(invocation -> {
        ...
        // When
        service.processPullRequest(
                "testorg/testrepo", 123, prTitle, prDescription, "MERGED",
                "feature/proj-42", "main", "johndoe",
                "https://github.com/testorg/testrepo/pull/123",
                LocalDateTime.now().minusDays(1), mergedAt, mergedAt, "janedoe"
        );

        // Then
        verify(taskRepository).save(argThat(task ->
                task.getId().equals(42L) &&
    ...
    void processPullRequest_WithClosesKeywordButNotMerged_ShouldNotCloseTask() {
        ...
        service.processPullRequest(
                "testorg/testrepo", 123, prTitle, prDescription, "OPEN",
                "feature/proj-42", "main", "johndoe",
                "https://github.com/testorg/testrepo/pull/123",
    ...
    void processPullRequest_WithAutoLinkDisabled_ShouldNotAutoLink() {
        ...
        // When
        service.processPullRequest(
                "testorg/testrepo", 123, prTitle, null, "OPEN",
                "feature/proj-42", "main", "johndoe",
                "https://github.com/testorg/testrepo/pull/123",
                LocalDateTime.now(), null, null, null
        );
    ...
    void processPullRequest_WithAutoCloseDisabled_ShouldNotAutoClose() {
        ...
        // When
        service.processPullRequest(
                "testorg/testrepo", 123, "Fix bug", prDescription, "MERGED",
                "feature/fix", "main", "johndoe",
                "https://github.com/testorg/testrepo/pull/123",
                LocalDateTime.now().minusDays(1), LocalDateTime.now(), LocalDateTime.now(), "janedoe"
        );
    ...
    void processCommit_WhenExistingCommit_ShouldReturnExisting() {
        // Given
        String sha = "abc123";
        GitHubCommit existingCommit = GitHubCommit.builder()
                .id(1L)
                .repository(testRepo)
                .sha(sha)
                .message("Test")
        ...
        // When
        GitHubCommit result = service.processCommit(
                "testorg/testrepo", sha, "Test message",
                "John Doe", "john@example.com", "johndoe",
                LocalDateTime.now(), "main", "https://github.com/test/commit/abc123"
        );

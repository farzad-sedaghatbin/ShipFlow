...
class TaskDependencyControllerIntegrationTest {
    ...
    void addDependency_Success() throws Exception {
        ...
        CreateTaskDependencyRequest request = CreateTaskDependencyRequest.builder()
                .targetTaskId(task2.getId())
                .dependencyType(DependencyType.BLOCKS)
                .build();

        // When & Then
        mockMvc.perform(post("/api/tasks/{taskId}/dependencies", task1.getId())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(request)))
                .andExpect(status().isCreated())
                .andExpect(jsonPath("$.id").exists())
                .andExpect(jsonPath("$.sourceTaskId").value(task1.getId()))
    ...
    void addDependency_CircularDependency_ReturnsBadRequest() throws Exception {
        ...
        // When & Then
        mockMvc.perform(post("/api/tasks/{taskId}/dependencies", task2.getId())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(request)))
                .andExpect(status().isBadRequest())
    ...
    void addDependency_TaskNotFound_ReturnsBadRequest() throws Exception {
        ...
        // When & Then
        mockMvc.perform(post("/api/tasks/{taskId}/dependencies", 99999L)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(request)))
                .andExpect(status().isBadRequest())
    ...
    void getDependencies_Success() throws Exception {
        ...
        TaskDependency blocking = TaskDependency.builder()
                .sourceTask(task1)
                .targetTask(task2)
                .dependencyType(DependencyType.BLOCKS)
                .createdAt(LocalDateTime.now())
        ...
        TaskDependency blockedBy = TaskDependency.builder()
                .sourceTask(task3)
                .targetTask(task1)
                .dependencyType(DependencyType.BLOCKS)
                .createdAt(LocalDateTime.now())
        ...
        // When & Then
        mockMvc.perform(get("/api/tasks/{taskId}/dependencies", task1.getId()))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.blocking").isArray())
                .andExpect(jsonPath("$.blocking", hasSize(1)))
                .andExpect(jsonPath("$.blocking[0].targetTaskId").value(task2.getId()))
                .andExpect(jsonPath("$.blockedBy").isArray())
    ...
    void getBlockingDependencies_Success() throws Exception {
        ...
        // When & Then
        mockMvc.perform(get("/api/tasks/{taskId}/dependencies/blocking", task1.getId()))
                .andExpect(status().isOk())
    ...
    void getBlockedByDependencies_Success() throws Exception {
        ...
        // When & Then
        mockMvc.perform(get("/api/tasks/{taskId}/dependencies/blocked-by", task1.getId()))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$").isArray())
                .andExpect(jsonPath("$", hasSize(2)))
    ...
    void complexDependencyChain_PreventsCircular() throws Exception {
        ...
        // When & Then
        mockMvc.perform(post("/api/tasks/{taskId}/dependencies", task3.getId())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(request)))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.message").value(containsString("circular")));
    ...
    void multipleDependencies_DifferentTypes() throws Exception {
        // Given
        CreateTaskDependencyRequest blocksRequest = CreateTaskDependencyRequest.builder()
                .targetTaskId(task2.getId())
                .dependencyType(DependencyType.BLOCKS)
                .build();

        CreateTaskDependencyRequest dependsOnRequest = CreateTaskDependencyRequest.builder()
                .targetTaskId(task3.getId())
                .dependencyType(DependencyType.DEPENDS_ON)
                .build();
        ...
        mockMvc.perform(post("/api/tasks/{taskId}/dependencies", task1.getId())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(blocksRequest)))
                .andExpect(status().isCreated());

        mockMvc.perform(post("/api/tasks/{taskId}/dependencies", task1.getId())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(dependsOnRequest)))
                .andExpect(status().isCreated());
        ...
        mockMvc.perform(get("/api/tasks/{taskId}/dependencies", task1.getId()))
                ...
                .andExpect(jsonPath("$.blocking[*].dependencyType", 
                        containsInAnyOrder("BLOCKS", "DEPENDS_ON")));
    }
}

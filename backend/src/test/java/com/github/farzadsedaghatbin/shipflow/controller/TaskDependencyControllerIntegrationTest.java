...
class TaskDependencyControllerIntegrationTest {
  ...
  void addDependency_Success() throws Exception {
    ...
    CreateTaskDependencyRequest request =
        CreateTaskDependencyRequest.builder()
    ...
    mockMvc
        .perform(
            post("/api/tasks/{taskId}/dependencies", task1.getId())
  ...
  void addDependency_CircularDependency_ReturnsBadRequest() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/tasks/{taskId}/dependencies", task2.getId())
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(request)))
        .andExpect(status().isBadRequest())
  ...
  void addDependency_TaskNotFound_ReturnsBadRequest() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/tasks/{taskId}/dependencies", 99999L)
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(request)))
        .andExpect(status().isBadRequest())
  ...
  void getDependencies_Success() throws Exception {
    ...
    // When & Then
    mockMvc
        .perform(get("/api/tasks/{taskId}/dependencies", task1.getId()))
  ...
  void getBlockingDependencies_Success() throws Exception {
    ...
    // When & Then
    mockMvc
        .perform(get("/api/tasks/{taskId}/dependencies/blocking", task1.getId()))
        .andExpect(status().isOk())
  ...
  void getBlockedByDependencies_Success() throws Exception {
    ...
    // When & Then
    mockMvc
        .perform(get("/api/tasks/{taskId}/dependencies/blocked-by", task1.getId()))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$").isArray())
        .andExpect(jsonPath("$", hasSize(2)))
  ...
  void removeDependency_NotFound_ReturnsBadRequest() throws Exception {
    // When & Then
    mockMvc
        .perform(delete("/api/tasks/{taskId}/dependencies/{dependencyId}", task1.getId(), 99999L))
        .andExpect(status().isBadRequest())
  ...
  void complexDependencyChain_PreventsCircular() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/tasks/{taskId}/dependencies", task3.getId())
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(request)))
        .andExpect(status().isBadRequest())
        .andExpect(jsonPath("$.message").value(containsString("circular")));
  ...
  void multipleDependencies_DifferentTypes() throws Exception {
    // Given
    CreateTaskDependencyRequest blocksRequest =
        CreateTaskDependencyRequest.builder()
            .targetTaskId(task2.getId())
            .dependencyType(DependencyType.BLOCKS)
            .build();

    CreateTaskDependencyRequest dependsOnRequest =
        CreateTaskDependencyRequest.builder()
    ...
    mockMvc
        ...
        .andExpect(
            jsonPath("$.blocking[*].dependencyType", containsInAnyOrder("BLOCKS", "DEPENDS_ON")));
  }
}

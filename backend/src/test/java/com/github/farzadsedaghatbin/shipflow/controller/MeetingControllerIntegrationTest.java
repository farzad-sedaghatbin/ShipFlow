...
class MeetingControllerIntegrationTest {
  ...
  void setUp() {
    ...
    testPerson = Person.builder().name("Test Person").email("test@example.com").isActive(true)
        .createdAt(LocalDateTime.now()).build();
    testPerson = personRepository.save(testPerson);

    User testUser = User.builder().username("meeting-test-user").password("password").role(UserRole.MEMBER)
    ...
    testRetrospective = Retrospective.builder().title("Test Retrospective").status(RetroStatus.OPEN)
  ...
  void getMeetingById_WhenNotExists_ShouldReturn404() throws Exception {
    mockMvc.perform(get("/api/meetings/{id}", 9999L)).andExpect(status().isBadRequest());
  }
  ...
  void updateMeeting_WhenExists_ShouldUpdateMeeting() throws Exception {
    ...
    mockMvc.perform(put("/api/meetings/{id}", testMeeting.getId()).contentType(MediaType.APPLICATION_JSON)
  ...
  void deleteMeeting_WhenExists_ShouldDeleteMeeting() throws Exception {
    mockMvc.perform(delete("/api/meetings/{id}", testMeeting.getId())).andExpect(status().isNoContent());

    mockMvc.perform(get("/api/meetings/{id}", testMeeting.getId())).andExpect(status().isBadRequest());
  }

  @WithMockUser(username = "meeting-test-user", roles = "MEMBER")
  @Test
  void getMeetingsByPitch_ShouldReturnMeetingsForPitch() throws Exception {
    mockMvc.perform(get("/api/meetings/pitch/{pitchId}", testPitch.getId())).andExpect(status().isOk())
  ...
  void getMeetingsWithFilters_ByType_ShouldReturnFilteredMeetings() throws Exception {
    ...
    mockMvc.perform(get("/api/meetings/filter").param("types", "DEMO").param("page", "0").param("size", "10"))
        .andExpect(status().isOk()).andExpect(jsonPath("$.content[*].type", everyItem(is("DEMO"))));
  }
  ...
  void getMeetingsWithFilters_ByDateRange_ShouldReturnFilteredMeetings() throws Exception {
    ...
    mockMvc.perform(get("/api/meetings/filter").param("startDate", startDate.toString())
        .param("endDate", endDate.toString()).param("page", "0").param("size", "10")).andExpect(status().isOk())
  ...
  void getMeetingsWithFilters_ByDorReady_ShouldReturnFilteredMeetings() throws Exception {
    ...
    mockMvc.perform(get("/api/meetings/filter").param("dorReady", "true").param("page", "0").param("size", "10"))
        .andExpect(status().isOk()).andExpect(jsonPath("$.content[*].dorReady", everyItem(is(true))));
  }
  ...
  void createMeeting_WithActionItems_ShouldCreateMeetingWithActions() throws Exception {
    ...
    MeetingActionDTO action1 = MeetingActionDTO.builder().description("Complete documentation")
        .assignedToId(testPerson.getId()).status(ActionStatus.OPEN).dueDate(LocalDate.now().plusDays(7))
    ...
    mockMvc.perform(post("/api/meetings").contentType(MediaType.APPLICATION_JSON)
        ...
        .andExpect(jsonPath("$.attendees", is("Team A, Team B"))).andExpect(jsonPath("$.actions", hasSize(1)))
  ...
  void updateMeeting_WithActionItems_ShouldUpdateActions() throws Exception {
    List<MeetingActionDTO> actions = new ArrayList<>();
    MeetingActionDTO action1 = MeetingActionDTO.builder().description("Update tests")
        .assignedToId(testPerson.getId()).status(ActionStatus.IN_PROGRESS).dueDate(LocalDate.now().plusDays(3))
        .build();
    MeetingActionDTO action2 = MeetingActionDTO.builder().description("Review code").status(ActionStatus.OPEN)
  ...
  void createMeeting_WithMultipleActionStatuses_ShouldHandleAllStatuses() throws Exception {
    List<MeetingActionDTO> actions = new ArrayList<>();
    actions.add(MeetingActionDTO.builder().description("Task 1").status(ActionStatus.OPEN).build());
    actions.add(MeetingActionDTO.builder().description("Task 2").status(ActionStatus.IN_PROGRESS).build());
    ...
    mockMvc.perform(post("/api/meetings").contentType(MediaType.APPLICATION_JSON)
        .content(objectMapper.writeValueAsString(request))).andExpect(status().isCreated())
        .andExpect(jsonPath("$.actions", hasSize(4))).andExpect(jsonPath("$.actions[0].status", is("OPEN")))

...
class MeetingControllerIntegrationTest {
  ...
  @Autowired private RetrospectiveRepository retrospectiveRepository;
  ...
  void getMeetingById_WhenExists_ShouldReturnMeeting() throws Exception {
    mockMvc
        .perform(get("/api/meetings/{id}", testMeeting.getId()))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON))
  ...
  void updateMeeting_WhenExists_ShouldUpdateMeeting() throws Exception {
    ...
    mockMvc
        .perform(
            put("/api/meetings/{id}", testMeeting.getId())
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(request)))
        .andExpect(status().isOk())
  ...
  void deleteMeeting_WhenExists_ShouldDeleteMeeting() throws Exception {
    mockMvc
        .perform(delete("/api/meetings/{id}", testMeeting.getId()))
  ...
  void getMeetingsByPitch_ShouldReturnMeetingsForPitch() throws Exception {
    mockMvc
        .perform(get("/api/meetings/pitch/{pitchId}", testPitch.getId()))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON))
  ...
  void createMeeting_WithMultipleActionStatuses_ShouldHandleAllStatuses() throws Exception {
    ...
    actions.add(
        MeetingActionDTO.builder().description("Task 2").status(ActionStatus.IN_PROGRESS).build());
    actions.add(
        MeetingActionDTO.builder().description("Task 3").status(ActionStatus.COMPLETED).build());
    actions.add(
        MeetingActionDTO.builder().description("Task 4").status(ActionStatus.CANCELLED).build());

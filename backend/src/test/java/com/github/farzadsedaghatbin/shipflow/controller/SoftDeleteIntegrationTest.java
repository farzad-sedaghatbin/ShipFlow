...
class SoftDeleteIntegrationTest {
  ...
  void deletePitchShouldPerformSoftDelete() throws Exception {
    ...
    // When & Then
    mockMvc.perform(delete("/api/pitches/{id}", pitchId).contentType(MediaType.APPLICATION_JSON))
  ...
  void shouldReturn404WhenAccessingSoftDeletedPitch() throws Exception {
    ...
    // When & Then
    mockMvc.perform(get("/api/pitches/{id}", pitchId).contentType(MediaType.APPLICATION_JSON))
  ...
  void shouldNotIncludeSoftDeletedPitchesInList() throws Exception {
    ...
    // When & Then
    mockMvc.perform(get("/api/pitches").contentType(MediaType.APPLICATION_JSON)).andExpect(status().isOk())

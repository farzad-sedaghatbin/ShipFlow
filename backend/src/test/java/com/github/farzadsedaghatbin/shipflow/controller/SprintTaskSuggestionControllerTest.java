...
/**
 * Integration tests for {@link SprintTaskSuggestionController}. Tests REST API endpoints and
...
@ActiveProfiles("test")
@Transactional
class SprintTaskSuggestionControllerTest {

  @Autowired private MockMvc mockMvc;

  @MockBean private SprintTaskSuggestionService sprintTaskSuggestionService;
  ...
  void generate_AsAdmin_ShouldReturn200() throws Exception {
    TaskSuggestionResponseDTO response = TaskSuggestionResponseDTO.builder()
        ...
        .build();
    when(sprintTaskSuggestionService.suggestTasks(anyLong())).thenReturn(response);

    mockMvc.perform(post("/api/ai/sprint-task-suggestions/1/generate"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.suggestions[0].title").value("Build settings API"))
        .andExpect(jsonPath("$.suggestions[0].sourceContext").value("SPRINT"))
  ...
  void generate_AsMember_ShouldReturn200() throws Exception {
    ...
    mockMvc.perform(post("/api/ai/sprint-task-suggestions/1/generate")).andExpect(status().isOk());
  }

  @Test
  void status_NoAuth_ShouldReturn200() throws Exception {
    when(sprintTaskSuggestionService.isAvailable()).thenReturn(true);

    mockMvc.perform(get("/api/ai/sprint-task-suggestions/status"))
        .andExpect(status().isOk())

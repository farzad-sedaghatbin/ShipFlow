...
class EvidenceControllerIntegrationTest {
  ...
  void setUp() {
    ...
    User testUser = User.builder().username("evidence-test-user").password("password").role(UserRole.MEMBER)
    ...
    testEvidence = Evidence.builder().description("Test Evidence Description").date(LocalDate.now())
        .fileUrl("https://example.com/evidence").person(testPerson).pitch(testPitch).build();
  ...
  void getAllEvidence_ShouldReturnEvidence() throws Exception {
    mockMvc.perform(get("/api/evidences")).andExpect(status().isOk())
  ...
  void getEvidenceById_WhenExists_ShouldReturnEvidence() throws Exception {
    mockMvc.perform(get("/api/evidences/{id}", testEvidence.getId())).andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON))
  ...
  void getEvidenceById_WhenNotExists_ShouldReturn404() throws Exception {
    mockMvc.perform(get("/api/evidences/{id}", 9999L)).andExpect(status().isBadRequest());
  }

  @WithMockUser(username = "evidence-test-user", roles = "MEMBER")
  @Test
  void createEvidence_WithValidData_ShouldCreateEvidence() throws Exception {
    CreateEvidenceRequest request = CreateEvidenceRequest.builder().description("New Evidence Description")
        .date(LocalDate.now()).fileUrl("https://example.com/new-evidence").personId(testPerson.getId())
        .pitchId(testPitch.getId()).build();

    mockMvc.perform(post("/api/evidences").contentType(MediaType.APPLICATION_JSON)
  ...
  void updateEvidence_WhenExists_ShouldUpdateEvidence() throws Exception {
    CreateEvidenceRequest request = CreateEvidenceRequest.builder().description("Updated Evidence Description")
        .date(LocalDate.now()).fileUrl("https://example.com/updated-evidence").personId(testPerson.getId())
        .pitchId(testPitch.getId()).build();

    mockMvc.perform(put("/api/evidences/{id}", testEvidence.getId()).contentType(MediaType.APPLICATION_JSON)
  ...
  void deleteEvidence_WhenExists_ShouldDeleteEvidence() throws Exception {
    mockMvc.perform(delete("/api/evidences/{id}", testEvidence.getId())).andExpect(status().isNoContent());

    mockMvc.perform(get("/api/evidences/{id}", testEvidence.getId())).andExpect(status().isBadRequest());
  }

  @WithMockUser(username = "evidence-test-user", roles = "MEMBER")
  @Test
  void getEvidenceByPitch_ShouldReturnEvidenceForPitch() throws Exception {
    mockMvc.perform(get("/api/evidences/pitch/{pitchId}", testPitch.getId())).andExpect(status().isOk())

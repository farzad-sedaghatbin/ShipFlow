...
/**
 * Integration tests for Task History (Audit) endpoints.
 * Tests the Hibernate Envers auditing functionality for tasks.
 */
@SpringBootTest
...
class TaskHistoryIntegrationTest {
    ...
    void getTaskHistory_NonExistentTask_ReturnsEmptyPage() throws Exception {
        mockMvc.perform(get("/api/tasks/{id}/history", 99999L)
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.content").isArray())

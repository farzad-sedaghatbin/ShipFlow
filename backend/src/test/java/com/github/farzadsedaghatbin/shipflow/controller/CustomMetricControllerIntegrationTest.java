...
class CustomMetricControllerIntegrationTest {
    ...
    void createMetric_ShouldReturnCreatedMetric() throws Exception {
        ...
        mockMvc.perform(post("/api/metrics/custom")
    ...
    void getUserMetrics_ShouldReturnAllUserMetrics() throws Exception {
        // Create test metrics
        CustomMetric metric1 = CustomMetric.builder()
                .user(testUser)
                .name("Metric 1")
                .formula("10 + 5")
                .dataSource(CustomMetric.DataSource.CUSTOM)
    ...
    void getMetric_ShouldReturnMetric() throws Exception {
        ...
        mockMvc.perform(get("/api/metrics/custom/" + metric.getId()))
                .andExpect(status().isOk())
    ...
    void getMetric_WithInvalidId_ShouldReturnNotFound() throws Exception {
        mockMvc.perform(get("/api/metrics/custom/999999"))
                .andExpect(status().isBadRequest());
    }
    ...
    void updateMetric_ShouldReturnUpdatedMetric() throws Exception {
        ...
        mockMvc.perform(put("/api/metrics/custom/" + metric.getId())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(request)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id").value(metric.getId()))
    ...
    void deleteMetric_ShouldReturnNoContent() throws Exception {
        ...
        mockMvc.perform(delete("/api/metrics/custom/" + metric.getId()))
    ...
    void calculateMetric_ShouldReturnCalculatedValue() throws Exception {
        ...
        mockMvc.perform(post("/api/metrics/custom/" + metric.getId() + "/calculate"))
                .andExpect(status().isOk())
    ...
    void getMetricHistory_ShouldReturnHistoricalData() throws Exception {
        ...
        mockMvc.perform(get("/api/metrics/custom/" + metric.getId() + "/history")
                        .param("limit", "10"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$").isArray());
    }
    ...
    void createMetric_WithDuplicateName_ShouldReturnBadRequest() throws Exception {
        // Create first metric
        CustomMetric existingMetric = CustomMetric.builder()
                .user(testUser)
                .name("Unique Name")
                .formula("10")
                .dataSource(CustomMetric.DataSource.CUSTOM)

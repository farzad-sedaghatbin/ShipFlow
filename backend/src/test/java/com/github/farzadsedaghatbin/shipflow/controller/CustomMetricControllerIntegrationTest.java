...
class CustomMetricControllerIntegrationTest {
  ...
  @Autowired private CustomMetricRepository customMetricRepository;
  ...
  void createMetric_ShouldReturnCreatedMetric() throws Exception {
    CreateCustomMetricRequest request =
        CreateCustomMetricRequest.builder()
            ...
            .description("Measures team velocity")
            .formula("PITCH_COUNT")
    ...
    mockMvc
        ...
        .andExpect(jsonPath("$.displayFormat").value("NUMBER"))
  ...
  void getUserMetrics_ShouldReturnAllUserMetrics() throws Exception {
    ...
    CustomMetric metric2 =
        CustomMetric.builder()
            ...
            .displayFormat(CustomMetric.DisplayFormat.PERCENTAGE)
            .isActive(true)
            .createdAt(LocalDateTime.now())
            .updatedAt(LocalDateTime.now())
            .build();
    ...
    mockMvc
        .perform(get("/api/metrics/custom"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$").isArray())
        .andExpect(jsonPath("$.length()").value(2))
        .andExpect(jsonPath("$[0].name").value("Metric 1"))
  ...
  void getMetric_ShouldReturnMetric() throws Exception {
    ...
    mockMvc
        .perform(get("/api/metrics/custom/" + metric.getId()))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.id").value(metric.getId()))
  ...
  void updateMetric_ShouldReturnUpdatedMetric() throws Exception {
    ...
    mockMvc
        .perform(
            put("/api/metrics/custom/" + metric.getId())
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(request)))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.id").value(metric.getId()))
  ...
  void deleteMetric_ShouldReturnNoContent() throws Exception {
    ...
    mockMvc
        .perform(delete("/api/metrics/custom/" + metric.getId()))
  ...
  void calculateMetric_ShouldReturnCalculatedValue() throws Exception {
    ...
    mockMvc
        .perform(post("/api/metrics/custom/" + metric.getId() + "/calculate"))
        .andExpect(status().isOk())
  ...
  void getMetricHistory_ShouldReturnHistoricalData() throws Exception {
    ...
    mockMvc
        .perform(get("/api/metrics/custom/" + metric.getId() + "/history").param("limit", "10"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$").isArray());
  }
  ...
  void getMetric_AsOtherUser_ShouldReturnForbidden() throws Exception {
    // Create another user
    Person otherPerson =
        Person.builder().name("Other User").email("other@example.com").isActive(true).build();

...
class TestCaseSoftDeleteTest {
  ...
  void setUp() {
    // Setup test user
    testUser = User.builder().id(1L).username("testuser").email("test@example.com").role(UserRole.MEMBER)
        .isActive(true).build();

    // Setup test case
    testCase = TestCase.builder().id(1L).testCaseKey("TC-001").title("Test Case Title")
        .description("Test Case Description").type(TestCaseType.FUNCTIONAL).priority(TestCasePriority.HIGH)
  ...
  void shouldThrowExceptionWhenTestCaseNotFound() {
    ...
    // When & Then
    assertThatThrownBy(() -> testCaseService.deleteTestCase(999L)).isInstanceOf(RuntimeException.class)
  ...
  void shouldThrowExceptionWhenTestCaseAlreadyDeleted() {
    ...
    // When & Then
    assertThatThrownBy(() -> testCaseService.deleteTestCase(1L)).isInstanceOf(IllegalStateException.class)
  ...
  void shouldNotReturnDeletedTestCaseInGetTestCaseById() {
    ...
    // When & Then
    assertThatThrownBy(() -> testCaseService.getTestCaseById(1L)).isInstanceOf(RuntimeException.class)

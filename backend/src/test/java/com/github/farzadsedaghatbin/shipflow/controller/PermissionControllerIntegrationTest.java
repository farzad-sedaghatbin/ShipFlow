...
@ActiveProfiles("test")
@TestPropertySource(properties = {"app.security.rbac.enabled=true"})
@Transactional
class PermissionControllerIntegrationTest {
  ...
  void setUp() {
    ...
    // Create test persons
    Person adminPerson = Person.builder().name("Admin User").email("admin@example.com").skills("Admin")
        .isActive(true).createdAt(LocalDateTime.now()).build();
    adminPerson = personRepository.save(adminPerson);

    Person qaPerson = Person.builder().name("QA User").email("qa@example.com").skills("Testing").isActive(true)
        .createdAt(LocalDateTime.now()).build();
    qaPerson = personRepository.save(qaPerson);

    // Create test users
    adminUser = User.builder().username("admin").password(passwordEncoder.encode("admin123")).role(UserRole.ADMIN)
        .person(adminPerson).isActive(true).createdAt(LocalDateTime.now()).build();
    adminUser = userRepository.save(adminUser);

    qaUser = User.builder().username("qa").password(passwordEncoder.encode("qa123")).role(UserRole.MEMBER)
        .person(qaPerson).isActive(true).createdAt(LocalDateTime.now()).build();
    qaUser = userRepository.save(qaUser);

    // Create test permission
    testPermission = Permission.builder().role(UserRole.MEMBER).resourceType(ResourceType.BUG)
        .permissionType(PermissionType.CREATE).description("QA can create bugs").build();
  ...
  void getAllPermissions_AsAdmin_ShouldReturnAllPermissions() throws Exception {
    mockMvc.perform(get("/api/permissions")).andDo(print()).andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON))
        .andExpect(jsonPath("$", hasSize(greaterThanOrEqualTo(1))))
        .andExpect(jsonPath("$[0].role", is("MEMBER"))).andExpect(jsonPath("$[0].resourceType", is("BUG")))
  ...
  void getAllPermissions_AsNonAdmin_ShouldReturnForbidden() throws Exception {
    mockMvc.perform(get("/api/permissions")).andExpect(status().isForbidden());
  }

  @Test
  @WithMockUser(username = "admin", roles = {"ADMIN"})
  void getPermissionsByRole_AsAdmin_ShouldReturnRolePermissions() throws Exception {
    mockMvc.perform(get("/api/permissions/role/MEMBER")).andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON))
        .andExpect(jsonPath("$", hasSize(greaterThanOrEqualTo(1))))
  ...
  void getCurrentUserPermissions_AsQA_ShouldReturnOwnPermissions() throws Exception {
    mockMvc.perform(get("/api/permissions/my-permissions")).andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON))
        .andExpect(jsonPath("$.username", is("qa"))).andExpect(jsonPath("$.role", is("MEMBER")))
  ...
  void hasPermission_WithValidPermission_ShouldReturnTrue() throws Exception {
    mockMvc.perform(
        get("/api/permissions/has-permission").param("resourceType", "BUG").param("permissionType", "CREATE"))
  ...
  void hasPermission_WithInvalidPermission_ShouldReturnFalse() throws Exception {
    mockMvc.perform(get("/api/permissions/has-permission").param("resourceType", "SYSTEM").param("permissionType",
        "MANAGE")).andExpect(status().isOk()).andExpect(content().contentType(MediaType.APPLICATION_JSON))
  ...
  void createPermission_AsAdmin_ShouldCreateNewPermission() throws Exception {
    ...
    mockMvc.perform(post("/api/permissions").contentType(MediaType.APPLICATION_JSON)
        .content(objectMapper.writeValueAsString(request))).andExpect(status().isCreated())
        .andExpect(jsonPath("$.role", is("MEMBER"))).andExpect(jsonPath("$.resourceType", is("PITCH")))
    ...
    assertThat(permissions).anyMatch(
        p -> p.getResourceType() == ResourceType.PITCH && p.getPermissionType() == PermissionType.APPROVE);
  }
  ...
  void updatePermission_AsAdmin_ShouldUpdateDescription() throws Exception {
    ...
    mockMvc.perform(put("/api/permissions/" + testPermission.getId()).contentType(MediaType.APPLICATION_JSON)
  ...
  void updatePermission_WithNonExistentId_ShouldReturnBadRequest() throws Exception {
    ...
    mockMvc.perform(put("/api/permissions/999").contentType(MediaType.APPLICATION_JSON)
        .content(objectMapper.writeValueAsString(request))).andExpect(status().isBadRequest());
  }
  ...
  void deletePermission_AsAdmin_ShouldDeletePermission() throws Exception {
    ...
    mockMvc.perform(delete("/api/permissions/" + permissionId)).andExpect(status().isNoContent());
  ...
  void deletePermission_AsNonAdmin_ShouldReturnForbidden() throws Exception {
    mockMvc.perform(delete("/api/permissions/" + testPermission.getId())).andExpect(status().isForbidden());
  }

  @Test
  @WithMockUser(username = "admin", roles = {"ADMIN"})
  void deletePermission_WithNonExistentId_ShouldReturnBadRequest() throws Exception {
    mockMvc.perform(delete("/api/permissions/999")).andExpect(status().isBadRequest());
  }
  ...
  void createBulkPermissions_AsAdmin_ShouldCreateMultiplePermissions() throws Exception {
    ...
    mockMvc.perform(post("/api/permissions/bulk").contentType(MediaType.APPLICATION_JSON)
        .content(objectMapper.writeValueAsString(requests))).andExpect(status().isCreated())
  ...
  void createBulkPermissions_AsNonAdmin_ShouldReturnForbidden() throws Exception {
    ...
    mockMvc.perform(post("/api/permissions/bulk").contentType(MediaType.APPLICATION_JSON)
        .content(objectMapper.writeValueAsString(List.of(request)))).andExpect(status().isForbidden());
  }
  ...
  void deleteBulkPermissions_AsAdmin_ShouldDeleteMultiplePermissions() throws Exception {
    ...
    Permission permission2 = Permission.builder().role(UserRole.MEMBER).resourceType(ResourceType.BUG)
        .permissionType(PermissionType.UPDATE).description("QA can update bugs").build();
    permission2 = permissionRepository.save(permission2);

    List<Long> ids = Arrays.asList(testPermission.getId(), permission2.getId());

    mockMvc.perform(delete("/api/permissions/bulk").contentType(MediaType.APPLICATION_JSON)
        .content(objectMapper.writeValueAsString(ids))).andExpect(status().isNoContent());
  ...
  void unauthenticated_ShouldRedirectToLogin() throws Exception {
    mockMvc.perform(get("/api/permissions")).andExpect(status().isFound()); // 302 redirect to login
  }
}

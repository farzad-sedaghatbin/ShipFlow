...
class UserControllerIntegrationTest {
  ...
  void setUp() {
    ...
    testUser = User.builder().username("user-ctrl-test-user").password(passwordEncoder.encode("password123"))
        .role(UserRole.MEMBER).person(testPerson).isActive(true).build();
    testUser = userRepository.save(testUser);

    adminUser = User.builder().username("user-ctrl-admin").password(passwordEncoder.encode("admin123"))
  ...
  void updateUserRole_AsAdmin_ShouldUpdateRole() throws Exception {
    mockMvc.perform(put("/api/users/{id}/role", testUser.getId()).param("role", "MANAGER"))
        .andExpect(status().isOk()).andExpect(jsonPath("$.role", is("MANAGER")));
  }

  @Test
  @WithMockUser(username = "user-ctrl-test-user", roles = {"MEMBER"})
  void updateUserRole_AsNonAdmin_ShouldReturn403() throws Exception {
    mockMvc.perform(put("/api/users/{id}/role", testUser.getId()).param("role", "ADMIN"))
  ...
  void deactivateUser_AsAdmin_ShouldDeactivate() throws Exception {
    mockMvc.perform(put("/api/users/{id}/deactivate", testUser.getId())).andExpect(status().isOk())

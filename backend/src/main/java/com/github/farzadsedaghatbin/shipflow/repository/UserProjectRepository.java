...
public interface UserProjectRepository extends JpaRepository<UserProject, Long> {
    ...
    /**
     * Find all projects accessible to a user through ANY access path:
     * 1. Direct assignment via user_projects
     * 2. Project ownership
     * 3. Team membership (via team_assignments → teams → cycles → projects)
     * 
    ...
    /**
     * Get the effective role a user has in a project
     * Returns the highest privilege role from direct assignment
     */
    @Query("SELECT up.projectRole FROM UserProject up WHERE up.user.id = :userId AND up.project.id = :projectId")
    Optional<ProjectRole> findProjectRoleByUserIdAndProjectId(@Param("userId") Long userId, @Param("projectId") Long projectId);

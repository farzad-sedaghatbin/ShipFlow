...
public interface BettingSlotRepository extends JpaRepository<BettingSlot, Long> {
  ...
  @Query(
      "SELECT bs FROM BettingSlot bs WHERE bs.cycle.id = :cycleId AND bs.team.id = :teamId AND bs.position = :position")
  ...
  @Query(
      "SELECT COUNT(bs) FROM BettingSlot bs WHERE bs.pitch IS NOT NULL AND bs.cycle.id = :cycleId")
  long countAssignedByCycleId(@Param("cycleId") Long cycleId);

  @Query(
      "SELECT COALESCE(MAX(bs.position), -1) FROM BettingSlot bs WHERE bs.cycle.id = :cycleId AND bs.team.id = :teamId")
  Integer findMaxPositionByCycleAndTeam(
      @Param("cycleId") Long cycleId, @Param("teamId") Long teamId);

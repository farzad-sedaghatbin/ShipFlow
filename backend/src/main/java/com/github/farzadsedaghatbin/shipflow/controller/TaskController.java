...
@Tag(
    name = "Tasks",
    description = "Task management APIs for tracking independent work during cycles")
public class TaskController {
  ...
  @Operation(
      ...
      description =
          "Returns all tasks assigned to the currently authenticated user (as assignee or pair) with pagination")
  @ApiResponses({
  ...
  @Operation(
      summary = "Get current user's tasks by cycle",
  ...
  public ResponseEntity<?> searchTasks(
    ...
    if (q == null || q.trim().length() < 3) {
      return ResponseEntity.badRequest()
          .body(Map.of("error", "Search query must be at least 3 characters"));
    }
    Sort.Direction direction =
        sortOrder.equalsIgnoreCase("asc") ? Sort.Direction.ASC : Sort.Direction.DESC;
    Pageable pageable = PageRequest.of(page, size, Sort.by(direction, sortBy));
  ...
  @Operation(
      summary = "Get tasks with multi-selection filters",
      description =
          "Filter tasks by multiple statuses, priorities, assignees, and category with optional exclusion")
  public ResponseEntity<Page<TaskDTO>> getTasksWithFilters(
    ...
    return ResponseEntity.ok(
        taskService.getTasksWithFilters(
            cycleId, statuses, priorities, assigneeIds, category, exclude, pageable));
  }

  @GetMapping("/cycle/{cycleId}/category/{category}")
  @Operation(
      summary = "Get tasks by cycle ID and category",
      description =
          "Returns tasks for a cycle filtered by category (PITCH_SCOPE or DEBT_IMPROVEMENT)")
  ...
  public ResponseEntity<Page<TaskDTO>> getTasksByProjectIdAndCategory(
    ...
    Pageable pageable = PageRequest.of(page, size, sort);
    return ResponseEntity.ok(
        taskService.getTasksByProjectIdAndCategory(projectId, category, pageable));
  }

  @GetMapping("/project/{projectId}/statistics")
  @Operation(summary = "Get task statistics for a project")
  public ResponseEntity<TaskStatisticsDTO> getTaskStatisticsByProjectId(
      @PathVariable Long projectId) {
  ...
  @Operation(
      summary = "Get task statistics for a cycle",
  ...
  public ResponseEntity<TaskDTO> updateTaskPriority(
      @PathVariable Long id, @RequestBody Map<String, String> priorityUpdate) {

...
public class CustomMetricService {
  ...
  public CustomMetricDTO createMetric(Long userId, CreateCustomMetricRequest request) {
    ...
    if (!validation.isValid()) {
      throw new IllegalArgumentException(
          messageService.getMessage("error.metric.formula.invalid", validation.getErrorMessage()));
    }

    // Check for duplicate name
    if (customMetricRepository.existsByUserIdAndName(userId, request.getName())) {
      throw new IllegalArgumentException(
          messageService.getMessage("error.metric.name.exists", request.getName()));
    }
    ...
    CustomMetric metric = CustomMetric.builder().user(user).cycle(cycle).pitch(pitch).team(team)
        .name(request.getName()).description(request.getDescription()).formula(request.getFormula())
        .dataSource(request.getDataSource()).aggregationType(request.getAggregationType())
        .filters(request.getFilters())
        .displayFormat(request.getDisplayFormat() != null
            ? request.getDisplayFormat()
            : CustomMetric.DisplayFormat.NUMBER)
  ...
  public CustomMetricDTO updateMetric(Long userId, Long metricId, UpdateCustomMetricRequest request) {
    ...
    CustomMetric metric = customMetricRepository.findById(metricId).orElseThrow(
        () -> new IllegalArgumentException(messageService.getMessage("error.metric.not.found", metricId)));
  ...
  public List<CustomMetricDTO> getUserMetrics(Long userId) {
    return customMetricRepository.findByUserIdOrderByNameAsc(userId).stream().map(this::toDTO)
  ...
  public MetricValueDTO calculateMetricValue(Long userId, Long metricId) {
    ...
    // Save data point
    MetricDataPoint dataPoint = MetricDataPoint.builder().metric(metric).calculatedValue(value)
        .calculationDate(LocalDateTime.now()).metadata(buildMetadata(dataContext)).build();

    metricDataPointRepository.save(dataPoint);

    return MetricValueDTO.builder().metricId(metric.getId()).metricName(metric.getName()).value(value)
        .displayValue(formatValue(value, metric.getDisplayFormat())).calculatedAt(LocalDateTime.now())
        .metadata(buildMetadata(dataContext)).build();
  }
  ...
  public List<MetricValueDTO> getMetricHistory(Long userId, Long metricId, int limit) {
    ...
    return metricDataPointRepository.findTopNByMetricId(metricId, limit).stream()
        .map(dp -> MetricValueDTO.builder().metricId(metric.getId()).metricName(metric.getName())
            .value(dp.getCalculatedValue())
            .displayValue(formatValue(dp.getCalculatedValue(), metric.getDisplayFormat()))
            .calculatedAt(dp.getCalculationDate()).metadata(dp.getMetadata()).build())
        .collect(Collectors.toList());
  }
  ...
  private Map<String, Object> buildDataContext(CustomMetric metric) {
    ...
    switch (metric.getDataSource()) {
      case PITCH :
        ...
        context.put("completedPitches",
            pitches.stream().filter(p -> p.getStatus() == PitchStatus.DONE).count());
        context.put("inProgressPitches",
            pitches.stream().filter(p -> p.getStatus() == PitchStatus.IN_PROGRESS).count());
        break;
  ...
  private String buildMetadata(Map<String, Object> dataContext) {
    ...
    return dataContext.entrySet().stream().map(e -> "\"" + e.getKey() + "\":" + e.getValue())
  ...
  private CustomMetricDTO toDTO(CustomMetric metric) {
    return CustomMetricDTO.builder().id(metric.getId()).name(metric.getName()).description(metric.getDescription())
        .formula(metric.getFormula()).dataSource(metric.getDataSource())
        .aggregationType(metric.getAggregationType()).filters(metric.getFilters())
        .displayFormat(metric.getDisplayFormat()).isActive(metric.getIsActive())
        ...
        .teamName(metric.getTeam() != null ? metric.getTeam().getName() : null).createdAt(metric.getCreatedAt())
        .updatedAt(metric.getUpdatedAt()).build();
  }
}

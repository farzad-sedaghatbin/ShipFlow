...
public class PersonService {
  ...
  public PersonDTO getPersonByEmail(String email) {
    Person person =
        personRepository
            .findByEmail(email)
            .orElseThrow(
                () -> new EntityNotFoundException("Person not found with email: " + email));
    return mapToDTO(person);
  }
  ...
  public List<PersonDTO> getActivePersons() {
    return personRepository.findByIsActiveTrue().stream()
        .map(this::mapToDTO)
  ...
  public List<PersonDTO> searchPersons(String query) {
    return personRepository.searchByNameOrEmail(query).stream()
        .map(this::mapToDTO)
        .collect(Collectors.toList());
  }
  ...
  public List<PersonDTO> getPersonsByTeam(Long teamId) {
    return personRepository.findByCurrentTeam(teamId).stream()
        .map(this::mapToDTO)
        .collect(Collectors.toList());
  }
  ...
  public List<PersonDTO> getPersonsByCycle(Long cycleId) {
    return personRepository.findByCurrentCycle(cycleId).stream()
        .map(this::mapToDTO)
        .collect(Collectors.toList());
  }
  ...
  public PersonDTO deactivatePerson(Long id) {
    ...
    List<TeamAssignment> activeAssignments =
        teamAssignmentRepository.findByPersonIdAndIsActiveTrue(id);
  ...
  public TeamAssignmentDTO assignToTeam(CreateTeamAssignmentRequest request) {
    ...
    Team team =
        teamRepository
            ...
            .orElseThrow(
                () ->
                    new EntityNotFoundException("Team not found with id: " + request.getTeamId()));

    // Check if already assigned to this team
    if (teamAssignmentRepository.existsActiveAssignment(
        request.getPersonId(), request.getTeamId())) {
      throw new IllegalArgumentException(
          messageService.getMessage("error.person.already.assigned"));
    }
  ...
  public TeamAssignmentDTO updateAssignment(
      Long assignmentId, CreateTeamAssignmentRequest request) {
    TeamAssignment assignment =
        teamAssignmentRepository
            .findById(assignmentId)
            .orElseThrow(
                () -> new EntityNotFoundException("Assignment not found with id: " + assignmentId));
  ...
  public List<TeamAssignmentDTO> getPersonAssignments(Long personId) {
    return teamAssignmentRepository.findByPersonIdWithTeamAndCycle(personId).stream()
  ...
  public List<TeamAssignmentDTO> getTeamAssignments(Long teamId) {
    return teamAssignmentRepository.findByTeamIdAndIsActiveTrue(teamId).stream()
        .map(this::mapToAssignmentDTO)
        .collect(Collectors.toList());
  }
  ...
  private PersonDTO mapToDTO(Person person) {
    List<TeamAssignment> assignments =
        teamAssignmentRepository.findByPersonIdWithTeamAndCycle(person.getId());

    List<TeamAssignmentDTO> currentAssignments =
        assignments.stream()
            .filter(TeamAssignment::getIsActive)
            .map(this::mapToAssignmentDTO)
            .collect(Collectors.toList());

    List<TeamAssignmentDTO> pastAssignments =
        assignments.stream()

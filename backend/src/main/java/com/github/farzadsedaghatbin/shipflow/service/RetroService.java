...
public class RetroService {
  ...
  public List<RetroDTO> getAllRetrosByProject(Long projectId) {
    ...
    return retroRepository.findByProjectIdOrderByCreatedAtDesc(projectId).stream()
        .map(this::toDTO)
  ...
  public List<RetroDTO> getRetrosByCycle(Long cycleId) {
    ...
    return retroRepository.findByCycleIdOrderByCreatedAtDesc(cycleId).stream()
        .map(this::toDTO)
        .collect(Collectors.toList());
  }
  ...
  public RetroDTO getRetroById(Long id) {
    Retrospective retro =
        retroRepository
            ...
            .orElseThrow(
                () -> new ResourceNotFoundException("Retrospective not found with id: " + id));
  ...
  public List<RetroItemDTO> getItemsByRetro(Long retroId) {
    Retrospective retro =
        retroRepository
            .findById(retroId)
            .orElseThrow(
                () -> new ResourceNotFoundException("Retrospective not found with id: " + retroId));

    if (retro.getProject() != null) {
      validateRetrospectivesEnabled(retro.getProject().getId());
    }

    return retroItemRepository.findByRetrospectiveIdOrderByCreatedAtAsc(retroId).stream()
        .map(this::toItemDTO)
        .collect(Collectors.toList());
  }
  ...
  public RetroItemDTO createRetroItem(CreateRetroItemRequest request) {
    ...
    if (retro.getStatus() == RetroStatus.CLOSED) {
      throw new IllegalStateException(
          localizationService.getMessage("retro.cannot.add.items.closed"));
    }
  ...
  public RetroItemDTO updateRetroItem(Long itemId, String content) {
    RetroItem item =
        retroItemRepository
            .findById(itemId)
            .orElseThrow(
                () -> new ResourceNotFoundException("Retro item not found with id: " + itemId));
    ...
    if (retro.getStatus() == RetroStatus.CLOSED) {
      throw new IllegalStateException(
          localizationService.getMessage("retro.cannot.update.items.closed"));
    }
  ...
  public void deleteRetroItem(Long itemId) {
    ...
    if (retro.getStatus() == RetroStatus.CLOSED) {
      throw new IllegalStateException(
          localizationService.getMessage("retro.cannot.delete.items.closed"));
    }
  ...
  public RetroItemDTO toggleVote(Long itemId) {
    ...
    boolean hasVoted =
        retroItemVoteRepository.existsByRetroItemIdAndUserId(itemId, currentUser.getId());
  ...
  public RetroItemDTO mergeItems(Long targetItemId, Long sourceItemId) {
    RetroItem targetItem =
        retroItemRepository
            ...
            .orElseThrow(
                () ->
                    new ResourceNotFoundException(
                        "Target item not found with id: " + targetItemId));

    RetroItem sourceItem =
        retroItemRepository
            .findById(sourceItemId)
            .orElseThrow(
                () ->
                    new ResourceNotFoundException(
                        "Source item not found with id: " + sourceItemId));

    // Validate same retrospective
    if (!targetItem.getRetrospective().getId().equals(sourceItem.getRetrospective().getId())) {
      throw new IllegalStateException(
          messageService.getMessage("error.retro.merge.different.retros"));
    }

    // Validate same column
    if (targetItem.getColumnType() != sourceItem.getColumnType()) {
      throw new IllegalStateException(
          messageService.getMessage("error.retro.merge.different.columns"));
    }
    ...
    if (sourceItem.getMergedInto() != null) {
      throw new IllegalStateException(
          messageService.getMessage("error.retro.merge.already.merged"));
    }
    ...
    for (RetroItemVote vote : sourceVotes) {
      ...
      if (!retroItemVoteRepository.existsByRetroItemIdAndUserId(
          targetItemId, vote.getUser().getId())) {
        RetroItemVote newVote =
            RetroItemVote.builder().retroItem(targetItem).user(vote.getUser()).build();
  ...
  private RetroDTO toDTO(Retrospective retro) {
    RetroDTO.RetroDTOBuilder builder =
        RetroDTO.builder()
            .id(retro.getId())
    ...
    if (retro.getProject() != null) {
      builder
          .projectId(retro.getProject().getId())
    ...
    if (retro.getCreatedBy() != null) {
      builder
          .createdById(retro.getCreatedBy().getId())
  ...
  private RetroDTO toDTOWithItems(Retrospective retro) {
    ...
    List<RetroItemDTO> items =
        retro.getItems().stream().map(this::toItemDTO).collect(Collectors.toList());
  ...
  private RetroItemDTO toItemDTO(RetroItem item) {
    ...
    RetroItemDTO.RetroItemDTOBuilder builder =
        RetroItemDTO.builder()
            .id(item.getId())
    ...
    if (currentUser != null) {
      boolean hasVoted =
          retroItemVoteRepository.existsByRetroItemIdAndUserId(item.getId(), currentUser.getId());
    ...
    // Get merged item IDs
    List<Long> mergedItemIds =
        retroItemRepository.findByMergedIntoId(item.getId()).stream()
            .map(RetroItem::getId)
            .collect(Collectors.toList());

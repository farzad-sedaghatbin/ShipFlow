 ...
 * - Initiating GitHub App authorization (organization-wide consent)
 * - Handling OAuth callbacks from GitHub
 * - Managing GitHub App installations
 * - Syncing repositories from all installations
 * 
 * Benefits over manual repository registration:
 * - Single authorization grants access to all organization repositories
 * - Automatic webhook configuration for all repositories
 * - New repositories automatically accessible (if "all" is selected)
 * - No need to add repositories one by one
 */
@RestController
...
public class GitHubAppOAuthController {
    ...
    @Operation(summary = "Get GitHub App configuration status",
    ...
    @Operation(summary = "Initiate GitHub App authorization",
               description = "Generate authorization URL for installing the GitHub App on an organization. " +
    ...
    public ResponseEntity<GitHubOAuthUrlResponse> initiateAuthorization(
        ...
        if (!oauthService.isAppConfigured()) {
            return ResponseEntity.status(HttpStatus.SERVICE_UNAVAILABLE)
                    .body(GitHubOAuthUrlResponse.builder()
    ...
    @GetMapping("/callback")
    @Operation(summary = "Handle GitHub App OAuth callback",
               description = "This endpoint receives the callback from GitHub after app installation. " +
    ...
    @PreAuthorize("hasAnyRole('ADMIN', 'MANAGER')")
    @Operation(summary = "Get all GitHub App installations",
    ...
    @PreAuthorize("hasAnyRole('ADMIN', 'MANAGER')")
    @Operation(summary = "Get a specific GitHub App installation",
    ...
    public ResponseEntity<GitHubAppInstallationDTO> getInstallation(@PathVariable Long id) {
        return oauthService.getInstallation(id)
    ...
    @Operation(summary = "Remove a GitHub App installation",
               description = "Deactivate a GitHub App installation (repositories will remain but won't receive updates)")
    ...
    @PreAuthorize("hasAnyRole('ADMIN', 'MANAGER')")
    @Operation(summary = "Sync repositories for an installation",
    ...
    @PreAuthorize("hasRole('ADMIN')")
    @Operation(summary = "Sync all repositories from all installations",
               description = "Perform a full sync of all repositories from all active GitHub App installations. " +

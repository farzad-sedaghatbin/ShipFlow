...
/**
 * Interface for vector store provider plugins. Implement this interface to add
 * support for a new vector store.
 *
 * <p>
 ...
 * <li>Create and return a properly configured EmbeddingStore via
 * {@link #createStore(VectorStoreProviderConfig)}
 ...
 * 			return MyStore.builder().url(config.getUrl()).collectionName(config.getCollectionName()).build();
 * 		}
 * 	}
 * }
 * </pre>
...
public interface VectorStoreProvider {
   ...
   * provider. Called automatically by the factory before creating the store.
   *
   * @param config
  ...
  /**
   * Indicates whether this provider requires an API key. Providers can override
   * this to indicate their authentication requirements.
   *
  ...
  /**
   * Indicates whether this provider requires a base URL. Most remote vector
   * stores require a URL.
   *

 ...
 * Estimates tokens and truncates context intelligently.
 */
@Component
@Slf4j
public class ContextWindowManager {
    ...
    public ContextResult buildManagedContext(
        ...
        int availableTokens = (maxTokens > 0 ? maxTokens : DEFAULT_MAX_TOKENS) 
                               - PROMPT_OVERHEAD_TOKENS 
                               - questionTokens;
        
        log.debug("Building context with {} available tokens (max: {}, question: {}, overhead: {})",
                availableTokens, maxTokens, questionTokens, PROMPT_OVERHEAD_TOKENS);
        ...
        if (wasTruncated) {
            log.warn("Context truncated: included {}/{} documents, {} tokens used of {} available",
                    documentsIncluded, matches.size(), usedTokens, availableTokens);
        } else {
            log.debug("Context built: {}/{} documents, {} tokens used of {} available",
                    documentsIncluded, matches.size(), usedTokens, availableTokens);
        }
    ...
    public static class ContextResult {
        ...
        public int getDocumentsIncluded() { return documentsIncluded; }

...
/**
 * AOP Aspect for enforcing @RequirePermission annotations.
 * Intercepts method calls annotated with @RequirePermission and checks if the current user
 * has the required permission before allowing the method to execute.
 */
@Aspect
@Component
...
public class PermissionAspect {
    ...
    public void checkPermission(JoinPoint joinPoint) throws Throwable {
        ...
        log.debug("Permission granted for method: {}.{}", 
                joinPoint.getTarget().getClass().getSimpleName(), 
                method.getName());
    }
}

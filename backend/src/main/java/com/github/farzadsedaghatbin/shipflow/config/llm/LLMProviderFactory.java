 ...
 * 
 * <p>This factory automatically discovers all registered {@link LLMProvider} implementations
 * and creates the appropriate model based on configuration.
 * 
 * <p>Usage:
 * <pre>
 * {@code
 ...
 * <ol>
 *   <li>Add the provider type to {@link LLMProviderType}</li>
 *   <li>Create a provider implementation of {@link LLMProvider}</li>
...
public class LLMProviderFactory {
    ...
    /**
     * Constructor with auto-discovery of all LLMProvider implementations.
     * Spring will inject all beans implementing LLMProvider.
     */
    public LLMProviderFactory(List<LLMProvider> providerList) {
        ...
        for (LLMProvider provider : providerList) {
            ...
            if (providers.containsKey(type)) {
                log.warn("Duplicate LLM provider registration for type {}: {} will be overwritten by {}",
                        type, providers.get(type).getClass().getSimpleName(),
                        provider.getClass().getSimpleName());
            }
            providers.put(type, provider);
            log.info("Registered LLM provider: {} -> {}", 
                    type.getConfigValue(), provider.getClass().getSimpleName());
        }
        
        log.info("LLM Provider Factory initialized with {} providers: {}", 
    ...
    public ChatLanguageModel createModel(LLMProviderType providerType, LLMProviderConfig config) {
        ...
        log.info("Creating ChatLanguageModel using provider: {} ({})", 
                providerType.getConfigValue(), provider.getDisplayName());

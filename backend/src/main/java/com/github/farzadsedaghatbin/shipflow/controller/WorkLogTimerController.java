...
public class WorkLogTimerController {
    ...
    @PostMapping("/start")
    @Operation(summary = "Start a new timer",
               description = "Starts tracking time for a pitch or task. Only one active timer per user allowed.")
    @ApiResponses({
    ...
    @GetMapping("/active")
    @Operation(summary = "Get active timer",
               description = "Returns the currently running timer for the authenticated user, if any")
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "Active timer retrieved (or null if no timer)"),
        @ApiResponse(responseCode = "401", description = "User not authenticated")
    })
    ...
    @Operation(summary = "Cancel the active timer",

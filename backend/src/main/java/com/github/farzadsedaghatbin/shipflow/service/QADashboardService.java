...
public class QADashboardService {
  ...
  public QATestManagementStatusDTO getStatus() {
    ...
    long aiGeneratedTestCases =
        testManagementEnabled ? testCaseRepository.findAllAiGenerated().size() : 0;
  ...
  public TestCoverageDTO getTestCoverageByPitch(Long pitchId) {
    ...
    long approvedTestCases =
        testCaseRepository.countByPitchIdAndStatus(pitchId, TestCaseStatus.APPROVED);
    ...
    long criticalBugs =
        bugReportRepository.countByPitchIdAndSeverity(pitchId, BugSeverity.CRITICAL);
    ...
    double coverageScore =
        calculateCoverageScore(totalTestCases, totalRuns, passedRuns, openBugs, blockerBugs);
  ...
  public QADashboardDTO getQADashboardByCycle(Long cycleId) {
    ...
    long draftTestCases =
        testCases.stream().filter(tc -> tc.getStatus() == TestCaseStatus.DRAFT).count();
    long readyTestCases =
        testCases.stream().filter(tc -> tc.getStatus() == TestCaseStatus.READY).count();
    long approvedTestCases =
        testCases.stream().filter(tc -> tc.getStatus() == TestCaseStatus.APPROVED).count();
    long deprecatedTestCases =
        testCases.stream().filter(tc -> tc.getStatus() == TestCaseStatus.DEPRECATED).count();
    long aiGeneratedTestCases =
        testCases.stream().filter(tc -> tc.getAiGenerated() != null && tc.getAiGenerated()).count();
  ...
  private double calculateCoverageScore(
      long totalTestCases, long totalRuns, long passedRuns, long openBugs, long blockerBugs) {
    if (totalTestCases == 0) return 0;

    // Base score from test execution
    double executionCoverage =
        totalRuns > 0 ? Math.min(100, (double) totalRuns / totalTestCases * 100) : 0;

...
public class QADashboardService {
  ...
  public QATestManagementStatusDTO getStatus() {
    ...
    long aiGeneratedTestCases = testManagementEnabled ? testCaseRepository.findAllAiGenerated().size() : 0;

    return QATestManagementStatusDTO.builder().testManagementEnabled(testManagementEnabled)
        .aiTestGenerationEnabled(aiTestGenerationEnabled).totalTestCases(totalTestCases)
        .totalBugReports(totalBugReports).totalTestRuns(totalTestRuns)
        .aiGeneratedTestCases(aiGeneratedTestCases).build();
  }
  ...
  public TestCoverageDTO getTestCoverageByPitch(Long pitchId) {
    ...
    double testCaseCoverage = totalTestCases > 0
        ? (double) (readyTestCases + approvedTestCases) / totalTestCases * 100
        : 0;
    ...
    return TestCoverageDTO.builder().pitchId(pitchId).pitchTitle(pitch.getTitle())
        .cycleId(pitch.getCycle() != null ? pitch.getCycle().getId() : null)
        .cycleName(pitch.getCycle() != null ? pitch.getCycle().getName() : null).totalTestCases(totalTestCases)
        .draftTestCases(draftTestCases).readyTestCases(readyTestCases).approvedTestCases(approvedTestCases)
        .totalRuns(totalRuns).passedRuns(passedRuns).failedRuns(failedRuns).blockedRuns(blockedRuns)
        .skippedRuns(skippedRuns).testCaseCoverage(testCaseCoverage).passRate(passRate)
        .coverageScore(coverageScore).totalBugs(totalBugs).openBugs(openBugs).criticalBugs(criticalBugs)
        .blockerBugs(blockerBugs).build();
  }
  ...
  public QADashboardDTO getQADashboardByCycle(Long cycleId) {
    ...
    long draftTestCases = testCases.stream().filter(tc -> tc.getStatus() == TestCaseStatus.DRAFT).count();
    long readyTestCases = testCases.stream().filter(tc -> tc.getStatus() == TestCaseStatus.READY).count();
    long approvedTestCases = testCases.stream().filter(tc -> tc.getStatus() == TestCaseStatus.APPROVED).count();
    long deprecatedTestCases = testCases.stream().filter(tc -> tc.getStatus() == TestCaseStatus.DEPRECATED).count();
    long aiGeneratedTestCases = testCases.stream().filter(tc -> tc.getAiGenerated() != null && tc.getAiGenerated())
        .count();
    ...
    long openBugs = bugs.stream().filter(b -> b.getStatus() == BugStatus.OPEN).count();
    long inProgressBugs = bugs.stream().filter(b -> b.getStatus() == BugStatus.IN_PROGRESS).count();
    long resolvedBugs = bugs.stream().filter(b -> b.getStatus() == BugStatus.RESOLVED).count();
    long verifiedBugs = bugs.stream().filter(b -> b.getStatus() == BugStatus.VERIFIED).count();
    long closedBugs = bugs.stream().filter(b -> b.getStatus() == BugStatus.CLOSED).count();

    // Bug severity breakdown
    long trivialBugs = bugs.stream().filter(b -> b.getSeverity() == BugSeverity.TRIVIAL).count();
    long minorBugs = bugs.stream().filter(b -> b.getSeverity() == BugSeverity.MINOR).count();
    long majorBugs = bugs.stream().filter(b -> b.getSeverity() == BugSeverity.MAJOR).count();
    long criticalBugs = bugs.stream().filter(b -> b.getSeverity() == BugSeverity.CRITICAL).count();
    long blockerBugs = bugs.stream().filter(b -> b.getSeverity() == BugSeverity.BLOCKER).count();
    ...
    return QADashboardDTO.builder().cycleId(cycleId).cycleName(cycle.getName()).totalTestCases(totalTestCases)
        .draftTestCases(draftTestCases).readyTestCases(readyTestCases).approvedTestCases(approvedTestCases)
        .deprecatedTestCases(deprecatedTestCases).aiGeneratedTestCases(aiGeneratedTestCases)
        .totalRuns(totalRuns).passedRuns(passedRuns).failedRuns(failedRuns).blockedRuns(blockedRuns)
        .skippedRuns(skippedRuns).pendingRuns(pendingRuns).totalBugs(totalBugs).openBugs(openBugs)
        .inProgressBugs(inProgressBugs).resolvedBugs(resolvedBugs).verifiedBugs(verifiedBugs)
        .closedBugs(closedBugs).trivialBugs(trivialBugs).minorBugs(minorBugs).majorBugs(majorBugs)
        .criticalBugs(criticalBugs).blockerBugs(blockerBugs).overallPassRate(overallPassRate)
        .overallCoverage(overallCoverage).pitchCoverage(pitchCoverage).build();
  }

  private double calculateCoverageScore(long totalTestCases, long totalRuns, long passedRuns, long openBugs,
      long blockerBugs) {
    if (totalTestCases == 0)
      return 0;

    // Base score from test execution
    double executionCoverage = totalRuns > 0 ? Math.min(100, (double) totalRuns / totalTestCases * 100) : 0;

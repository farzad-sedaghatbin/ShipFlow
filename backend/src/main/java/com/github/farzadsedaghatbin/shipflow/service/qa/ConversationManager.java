...
/**
 * Manages conversation contexts for multi-turn Q&A.
 * Uses Redis when configured via app.ai.cache.provider=redis, otherwise in-memory.
 */
@Component
@Slf4j
...
public class ConversationManager {
    ...
    /**
     * Initialize Redis connection for distributed conversation management.
    ...
    public void addTurn(String conversationId, String question, String answer) {
        ...
        if (context != null) {
            ...
            log.debug("Added turn to conversation {}, total turns: {}", 
                    conversationId, context.getHistory().size());
        }
    }
    ...
    /**
     * Get the most recent context (contextType and contextId) from conversation history.
     * Useful for inferring context when user asks follow-up questions.
     */
    public ConversationContext.ContextInfo getMostRecentContext(String conversationId) {
        ConversationContext context = conversations.get(conversationId);
        if (context != null) {
            return new ConversationContext.ContextInfo(

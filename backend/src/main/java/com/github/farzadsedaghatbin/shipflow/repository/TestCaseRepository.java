...
public interface TestCaseRepository extends JpaRepository<TestCase, Long> {
  ...
  long countByPitchIdAndStatus(
      @Param("pitchId") Long pitchId, @Param("status") TestCaseStatus status);
  ...
  @Query(
      "SELECT MAX(CAST(SUBSTRING(tc.testCaseKey, 4) AS int)) FROM TestCase tc WHERE tc.testCaseKey LIKE 'TC-%'")
  ...
  // Multi-filter query
  @Query(
      "SELECT tc FROM TestCase tc WHERE 1=1 "
  ...
  List<TestCase> findByScopeIdAndStatus(
      @Param("scopeId") Long scopeId, @Param("status") TestCaseStatus status);

  @Query("SELECT tc FROM TestCase tc WHERE tc.task.id = :taskId AND tc.status = :status")
  List<TestCase> findByTaskIdAndStatus(
      @Param("taskId") Long taskId, @Param("status") TestCaseStatus status);

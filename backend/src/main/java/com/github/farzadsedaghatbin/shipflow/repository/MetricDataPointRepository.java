...
public interface MetricDataPointRepository extends JpaRepository<MetricDataPoint, Long> {
  ...
  @Query(
      "SELECT mdp FROM MetricDataPoint mdp WHERE mdp.metric.id = :metricId "
          + "AND mdp.calculationDate BETWEEN :startDate AND :endDate "
          + "ORDER BY mdp.calculationDate DESC")
  List<MetricDataPoint> findByMetricIdAndDateRange(
  ...
  List<MetricDataPoint> findTopNByMetricId(
      @Param("metricId") Long metricId, @Param("limit") int limit);

...
public class CommentService {
  ...
  public CommentDTO createComment(CreateCommentRequest request, Long userId) {
    User author = userRepository.findById(userId)
        .orElseThrow(() -> new IllegalArgumentException(messageService.getMessage("comment.user.not.found")));

    // Validate entity exists
    validateEntityExists(request.getEntityType(), request.getEntityId());

    Comment comment = Comment.builder().content(request.getContent()).entityType(request.getEntityType())
        .entityId(request.getEntityId()).author(author).build();

    comment = commentRepository.save(comment);
    log.info("Created comment {} on {} {} by user {}", comment.getId(), request.getEntityType(),
        request.getEntityId(), userId);
  ...
  public CommentDTO updateComment(Long commentId, UpdateCommentRequest request, Long userId) {
    Comment comment = commentRepository.findByIdNotDeleted(commentId)
        .orElseThrow(() -> new IllegalArgumentException(messageService.getMessage("comment.not.found")));

    // Check if user can edit
    if (!canUserEdit(comment, userId)) {
      throw new IllegalArgumentException(messageService.getMessage("comment.edit.unauthorized"));
    }
  ...
  public void deleteComment(Long commentId, Long userId) {
    ...
    if (!canUserDelete(comment, user)) {
      throw new IllegalArgumentException(messageService.getMessage("comment.delete.unauthorized"));
    }
  ...
  public List<CommentDTO> getComments(CommentEntityType entityType, Long entityId, Long userId) {
    ...
    return comments.stream().map(c -> toDTO(c, userId)).collect(Collectors.toList());
  }
   ...
   */
  @Transactional(readOnly = true)
  public Page<CommentDTO> getCommentsPaginated(CommentEntityType entityType, Long entityId, Long userId,
      Pageable pageable) {
    Page<Comment> comments = commentRepository.findByEntityTypeAndEntityIdNotDeleted(entityType, entityId,
        pageable);
  ...
  public CommentDTO toggleReaction(Long commentId, ReactionRequest request, Long userId) {
    ...
    if (existingReaction.isPresent()) {
      ...
      log.info("Removed reaction {} from comment {} by user {}", request.getReactionType(), commentId, userId);
    } else {
      // Add new reaction
      CommentReactionEntity reaction = CommentReactionEntity.builder().comment(comment).user(user)
          .reactionType(request.getReactionType()).build();
      reactionRepository.save(reaction);
      log.info("Added reaction {} to comment {} by user {}", request.getReactionType(), commentId, userId);
    }
  ...
  public List<Map<String, String>> getAvailableReactions() {
    return Arrays.stream(CommentReaction.values()).map(r -> Map.of("type", r.name(), "emoji", r.getEmoji()))
    ...
    switch (entityType) {
      case TASK :
        if (!taskRepository.existsById(entityId)) {
          throw new IllegalArgumentException(messageService.getMessage("comment.task.not.found"));
        }
        break;
      case BUG_REPORT :
  ...
  private CommentDTO toDTO(Comment comment, Long currentUserId) {
    ...
    return CommentDTO.builder().id(comment.getId()).content(comment.getContent())
        .entityType(comment.getEntityType()).entityId(comment.getEntityId()).authorId(author.getId())
        .authorName(authorName).authorUsername(author.getUsername()).isEdited(comment.getIsEdited())
        ...
        .canDelete(currentUser != null && canUserDelete(comment, currentUser)).build();
  }
  ...
  private ReactionSummaryDTO buildReactionSummary(Long commentId, Long currentUserId) {
    ...
    List<CommentReaction> userReactionTypes = reactionRepository.findUserReactionsByCommentIdAndUserId(commentId,
        currentUserId);
    ...
    return ReactionSummaryDTO.builder().reactionCounts(reactionCounts).userReactions(userReactions)
        .totalReactions(totalReactions).build();
  }
  ...
  private void processMentions(String content, User author, CommentEntityType entityType, Long entityId) {
    ...
    for (User mentionedUser : mentionedUsers) {
      try {
        notificationService.notifyCommentMention(mentionedUser, author, entityType.name(), entityId, content);
      } catch (Exception e) {
        log.error("Failed to send mention notification to user {}: {}", mentionedUser.getUsername(),
            e.getMessage());
      }
    }

    log.info("Processed {} mentions in comment on {} {}", mentionedUsers.size(), entityType, entityId);
  }

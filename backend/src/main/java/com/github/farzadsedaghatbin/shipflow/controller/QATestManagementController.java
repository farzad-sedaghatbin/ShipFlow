...
/**
 * REST controller for QA Test Management endpoints.
 * Provides CRUD operations for test cases, bug reports, and test runs.
 */
@RestController
...
public class QATestManagementController {
    ...
    @Operation(summary = "Get test cases with multi-selection filters",
    ...
    @Operation(summary = "Get bug reports with multi-selection filters",
               description = "Filter bug reports by cycle, pitch, multiple statuses, severities, and assignees with optional exclusion")
    public ResponseEntity<Page<BugReportDTO>> getBugReportsWithFilters(
        ...
        Sort.Direction direction = sortOrder.equalsIgnoreCase("asc") ? Sort.Direction.ASC : Sort.Direction.DESC;
        Pageable pageable = PageRequest.of(page, size, Sort.by(direction, sortBy));
        return ResponseEntity.ok(bugReportService.getBugReportsWithFilters(cycleId, pitchId, statuses, severities, assigneeIds, exclude, pageable));
    }
    ...
    @PreAuthorize("hasAnyRole('QA', 'TEAM_LEAD', 'ADMIN')")

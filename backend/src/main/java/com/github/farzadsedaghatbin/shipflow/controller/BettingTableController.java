...
@Tag(
    name = "Betting Table",
    description = "Shape Up Betting Table management for assigning shaped pitches to cycle slots")
public class BettingTableController {
  ...
  @Operation(
      summary = "Get betting table for a cycle",
      description =
          "Returns the full betting table view including shaped pitches, team tracks, and assigned slots")
  ...
  @Operation(
      summary = "Get shaped pitches available for betting",
  ...
  @PostMapping("/slots")
  @Operation(
      summary = "Create a new betting slot",
      description =
          "Creates a new slot for a team in a cycle. Can optionally assign a pitch immediately.")
  public ResponseEntity<BettingSlotDTO> createSlot(
      @Valid @RequestBody CreateBettingSlotRequest request) {
  ...
  @Operation(
      summary = "Auto-generate slots for all teams in a cycle",
      description =
          "Creates default slots spanning the entire cycle for each team that doesn't have slots yet")
  public ResponseEntity<List<BettingSlotDTO>> generateSlots(@PathVariable Long cycleId) {
    return ResponseEntity.status(HttpStatus.CREATED)
        .body(bettingTableService.generateSlotsForCycle(cycleId));
  }

  @PatchMapping("/slots/{slotId}/assign/{pitchId}")
  @Operation(
      summary = "Assign a pitch to a slot",
      description =
          "The main betting action - assigns a shaped pitch to a team's slot. "
              + "Validates that the pitch fits within the slot's duration.")
  public ResponseEntity<BettingSlotDTO> assignPitchToSlot(
      @PathVariable Long slotId, @PathVariable Long pitchId) {
    return ResponseEntity.ok(bettingTableService.assignPitchToSlot(slotId, pitchId));
  }

  @PatchMapping("/slots/{slotId}/unassign")
  @Operation(
      summary = "Remove pitch from a slot",
  ...
  @Operation(
      summary = "Update slot dates",
      description =
          "Resize a slot by changing its start and end dates. "
  ...
  @Operation(
      summary = "Delete a betting slot",
      description =
          "Removes the slot. If a pitch is assigned, it will be returned to SHAPED status.")
  ...
  @Operation(
      summary = "Check if pitch fits in slot",
      description =
          "Validates if a pitch's appetite fits within the slot's duration. "
  ...
  @Operation(
      summary = "Get team performance history",
      description = "Returns historical betting success rates and performance metrics for a team")
  public ResponseEntity<TeamPerformanceHistoryDTO> getTeamPerformanceHistory(
  ...
  @Operation(
      summary = "Get pitch comparison analysis",
      description =
          "Returns shaped pitches with team fit analysis and capacity warnings for betting meetings")

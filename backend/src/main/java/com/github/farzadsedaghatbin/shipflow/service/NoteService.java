...
public class NoteService {
    ...
    public List<NoteDTO> getNotesByContext(String contextType, Long contextId) {
        return noteRepository.findByContextTypeAndContextId(contextType.toLowerCase(), contextId)
                .stream()
                .map(this::toDTO)
    ...
    public List<NoteDTO> getNotesByPitch(Long pitchId) {
        return noteRepository.findByPitchId(pitchId)
                .stream()
                .map(this::toDTO)
                .collect(Collectors.toList());
    }
    ...
    public List<NoteDTO> getNotesByCycle(Long cycleId) {
        return noteRepository.findByCycleId(cycleId)
                .stream()
                .map(this::toDTO)
                .collect(Collectors.toList());
    }
    ...
    private NoteDTO toDTO(ManualNote note) {
        ...
        if (note.getAuthorId() != null) {
            authorName = userRepository.findById(note.getAuthorId())

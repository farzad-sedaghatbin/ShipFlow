...
public class ReportService {
    ...
    public String exportCycleReportCsv(Long cycleId) {
        ...
        csv.append("Task Estimate (hours),").append(String.format("%.2f", report.getTotalTaskEstimateHours())).append("\n");
        csv.append("Task Actual (hours),").append(String.format("%.2f", report.getTotalTaskActualHours())).append("\n\n");

        // Pitch details
        csv.append("Pitch Reports\n");
        csv.append("Pitch,Team,Status,Appetite Days,Appetite Hours,Actual Hours,Variance Hours,Variance %,Over Budget\n");
        for (PitchReportDTO pitch : report.getPitchReports()) {
            csv.append(escapeCsv(pitch.getPitchTitle())).append(",")
                    .append(escapeCsv(pitch.getTeamName())).append(",")
                    .append(pitch.getStatus()).append(",")
                    .append(pitch.getAppetiteDays()).append(",")
                    .append(pitch.getAppetiteHours()).append(",")
                    .append(String.format("%.2f", pitch.getActualHours())).append(",")
                    .append(String.format("%.2f", pitch.getVarianceHours())).append(",")
        ...
        for (MemberWorkReportDTO member : report.getMemberReports()) {
            csv.append(escapeCsv(member.getMemberName())).append(",")
                    .append(member.getRole()).append(",")
                    .append(escapeCsv(member.getTeamName())).append(",")
                    .append(String.format("%.2f", member.getTotalHours())).append(",")
                    .append(member.getWorkDays()).append(",")
                    .append(String.format("%.2f", member.getAvgHoursPerDay())).append("\n");
        }
    ...
    public byte[] exportCycleReportPdf(Long cycleId) {
        ...
        try {
            ...
            // Title
            Paragraph title = new Paragraph("Cycle Report: " + report.getCycleName())
                    .setFontSize(20)
                    .setBold()
            ...
            addSummaryRow(summaryTable, "Total Appetite (hours)", String.format("%.2f", report.getTotalAppetiteHours()));
            ...
            Paragraph taskHeader = new Paragraph("Out-of-Scope Work (Tasks)")
                    .setFontSize(16)
                    .setBold();
            ...
            addSummaryRow(taskTable, "Task Estimate (hours)", String.format("%.2f", report.getTotalTaskEstimateHours()));
            addSummaryRow(taskTable, "Task Actual (hours)", String.format("%.2f", report.getTotalTaskActualHours()));
            document.add(taskTable);
            document.add(new Paragraph(" "));

            // Pitch Reports Section
            Paragraph pitchHeader = new Paragraph("Pitch Reports")
                    .setFontSize(16)
                    .setBold();
            document.add(pitchHeader);

            Table pitchTable = new Table(UnitValue.createPercentArray(new float[]{3, 2, 2, 1.5f, 1.5f, 1.5f, 1.5f, 1.5f, 1.5f}));
            ...
            for (PitchReportDTO pitch : report.getPitchReports()) {
                ...
                pitchTable.addCell(new Cell().add(new Paragraph(String.valueOf(pitch.getAppetiteDays())).setFontSize(9)));
                pitchTable.addCell(new Cell().add(new Paragraph(String.format("%.1f", pitch.getAppetiteHours())).setFontSize(9)));
                pitchTable.addCell(new Cell().add(new Paragraph(String.format("%.1f", pitch.getActualHours())).setFontSize(9)));
                pitchTable.addCell(new Cell().add(new Paragraph(String.format("%.1f", pitch.getVarianceHours())).setFontSize(9)));
                pitchTable.addCell(new Cell().add(new Paragraph(String.format("%.1f", pitch.getVariancePercentage())).setFontSize(9)));
                pitchTable.addCell(new Cell().add(new Paragraph(pitch.getIsOverBudget() ? "Yes" : "No").setFontSize(9)));
            }
            document.add(pitchTable);
            document.add(new Paragraph(" "));

            // Member Reports Section
            Paragraph memberHeader = new Paragraph("Member Work Summary")
                    .setFontSize(16)
                    .setBold();
            ...
            for (MemberWorkReportDTO member : report.getMemberReports()) {
                ...
                memberTable.addCell(new Cell().add(new Paragraph(String.format("%.1f", member.getTotalHours())).setFontSize(9)));
                memberTable.addCell(new Cell().add(new Paragraph(String.valueOf(member.getWorkDays())).setFontSize(9)));
                memberTable.addCell(new Cell().add(new Paragraph(String.format("%.1f", member.getAvgHoursPerDay())).setFontSize(9)));
            }
    ...
    private MemberWorkReportDTO buildMemberReport(Long personId, List<WorkLog> workLogs, Long cycleId) {
        ...
        Set<java.time.LocalDate> workDays = workLogs.stream()
                .map(WorkLog::getDate)
        ...
        List<MemberWorkReportDTO.PitchWorkSummary> pitchWork = pitchHours.entrySet().stream()
                .map(entry -> {

...
public class ReportService {
  ...
  public CycleReportDTO getCycleReport(Long cycleId) {
    ...
    List<PitchReportDTO> pitchReports =
        pitches.stream().map(p -> buildPitchReport(p, workLogs)).collect(Collectors.toList());

    // Calculate member reports
    Map<Long, List<WorkLog>> personWorkLogs =
        workLogs.stream().collect(Collectors.groupingBy(wl -> wl.getPerson().getId()));

    List<MemberWorkReportDTO> memberReports =
        personWorkLogs.entrySet().stream()
            .map(entry -> buildMemberReport(entry.getKey(), entry.getValue(), cycleId))
            .collect(Collectors.toList());

    // Calculate totals
    int completedPitches =
        (int) pitches.stream().filter(p -> p.getStatus() == PitchStatus.DONE).count();
    int inProgressPitches =
        (int)
            pitches.stream()
                .filter(
                    p ->
    ...
    double totalAppetiteHours =
        pitches.stream().mapToDouble(p -> p.getAppetiteDays() * HOURS_PER_DAY).sum();

    double totalActualHours =
        workLogs.stream().mapToDouble(wl -> wl.getHoursSpent().doubleValue()).sum();
  ...
  public String exportCycleReportCsv(Long cycleId) {
    ...
    csv.append("Efficiency %,")
        .append(String.format("%.2f", report.getEfficiencyPercentage()))
        .append("\n\n");
    ...
    csv.append("Task Estimate (hours),")
        .append(String.format("%.2f", report.getTotalTaskEstimateHours()))
        .append("\n");
    csv.append("Task Actual (hours),")
        .append(String.format("%.2f", report.getTotalTaskActualHours()))
        .append("\n\n");
    ...
    for (PitchReportDTO pitch : report.getPitchReports()) {
      csv.append(escapeCsv(pitch.getPitchTitle()))
          .append(",")
          .append(escapeCsv(pitch.getTeamName()))
          .append(",")
          .append(pitch.getStatus())
          .append(",")
          .append(pitch.getAppetiteDays())
          .append(",")
          .append(pitch.getAppetiteHours())
          .append(",")
          .append(String.format("%.2f", pitch.getActualHours()))
          .append(",")
          .append(String.format("%.2f", pitch.getVarianceHours()))
          .append(",")
          .append(String.format("%.2f", pitch.getVariancePercentage()))
          .append(",")
          .append(pitch.getIsOverBudget())
          .append("\n");
    }
    ...
    for (MemberWorkReportDTO member : report.getMemberReports()) {
      csv.append(escapeCsv(member.getMemberName()))
          .append(",")
          .append(member.getRole())
          .append(",")
          .append(escapeCsv(member.getTeamName()))
          .append(",")
          .append(String.format("%.2f", member.getTotalHours()))
          .append(",")
          .append(member.getWorkDays())
          .append(",")
          .append(String.format("%.2f", member.getAvgHoursPerDay()))
          .append("\n");
    }
  ...
  public byte[] exportCycleReportPdf(Long cycleId) {
    ...
    try {
      ...
      // Title
      Paragraph title =
          new Paragraph("Cycle Report: " + report.getCycleName())
              .setFontSize(20)
              .setBold()
      ...
      addSummaryRow(
          summaryTable,
          "Total Appetite (hours)",
          String.format("%.2f", report.getTotalAppetiteHours()));
      addSummaryRow(
          summaryTable,
          "Total Actual (hours)",
          String.format("%.2f", report.getTotalActualHours()));
      addSummaryRow(
          summaryTable, "Efficiency %", String.format("%.2f%%", report.getEfficiencyPercentage()));
      ...
      addSummaryRow(
          taskTable,
          "Task Actual (hours)",
          String.format("%.2f", report.getTotalTaskActualHours()));
      ...
      Table pitchTable =
          new Table(
              UnitValue.createPercentArray(
                  new float[] {3, 2, 2, 1.5f, 1.5f, 1.5f, 1.5f, 1.5f, 1.5f}));
      ...
      for (PitchReportDTO pitch : report.getPitchReports()) {
        ...
        pitchTable.addCell(
            new Cell().add(new Paragraph(pitch.getStatus().toString()).setFontSize(9)));
        pitchTable.addCell(
            new Cell().add(new Paragraph(String.valueOf(pitch.getAppetiteDays())).setFontSize(9)));
        pitchTable.addCell(
            new Cell()
                .add(
                    new Paragraph(String.format("%.1f", pitch.getAppetiteHours())).setFontSize(9)));
        pitchTable.addCell(
            new Cell()
                .add(new Paragraph(String.format("%.1f", pitch.getActualHours())).setFontSize(9)));
        pitchTable.addCell(
            new Cell()
                .add(
                    new Paragraph(String.format("%.1f", pitch.getVarianceHours())).setFontSize(9)));
        pitchTable.addCell(
            new Cell()
                .add(
                    new Paragraph(String.format("%.1f", pitch.getVariancePercentage()))
                        .setFontSize(9)));
        pitchTable.addCell(
            new Cell().add(new Paragraph(pitch.getIsOverBudget() ? "Yes" : "No").setFontSize(9)));
      }
      ...
      for (MemberWorkReportDTO member : report.getMemberReports()) {
        ...
        memberTable.addCell(
            new Cell().add(new Paragraph(member.getRole().toString()).setFontSize(9)));
        memberTable.addCell(new Cell().add(new Paragraph(member.getTeamName()).setFontSize(9)));
        memberTable.addCell(
            new Cell()
                .add(new Paragraph(String.format("%.1f", member.getTotalHours())).setFontSize(9)));
        memberTable.addCell(
            new Cell().add(new Paragraph(String.valueOf(member.getWorkDays())).setFontSize(9)));
        memberTable.addCell(
            new Cell()
                .add(
                    new Paragraph(String.format("%.1f", member.getAvgHoursPerDay()))
                        .setFontSize(9)));
      }
  ...
  private PitchReportDTO buildPitchReport(Pitch pitch, List<WorkLog> allWorkLogs) {
    double actualHours =
        allWorkLogs.stream()
            .filter(wl -> wl.getPitch().getId().equals(pitch.getId()))
  ...
  @SuppressWarnings("null")
  private MemberWorkReportDTO buildMemberReport(
      Long personId, List<WorkLog> workLogs, Long cycleId) {
    Person person =
        personRepository
            .findById(personId)
    ...
    List<TeamAssignment> assignments =
        teamAssignmentRepository.findByPersonAndCycle(personId, cycleId);
    ...
    Set<java.time.LocalDate> workDays =
        workLogs.stream().map(WorkLog::getDate).collect(Collectors.toSet());

    Map<Long, Double> pitchHours =
        workLogs.stream()
            .collect(
                Collectors.groupingBy(
                    wl -> wl.getPitch().getId(),
                    Collectors.summingDouble(wl -> wl.getHoursSpent().doubleValue())));

    List<MemberWorkReportDTO.PitchWorkSummary> pitchWork =
        pitchHours.entrySet().stream()
            .map(
                entry -> {
                  WorkLog sample =
                      workLogs.stream()
                          .filter(wl -> wl.getPitch().getId().equals(entry.getKey()))
                          .findFirst()
                          .orElse(null);
                  return MemberWorkReportDTO.PitchWorkSummary.builder()
                      .pitchId(entry.getKey())
                      .pitchTitle(sample != null ? sample.getPitch().getTitle() : "Unknown")
                      .hoursSpent(entry.getValue())
                      .build();
                })
            .collect(Collectors.toList());

    return MemberWorkReportDTO.builder()
        ...
        .avgHoursPerDay(workDays.isEmpty() ? 0 : totalHours / workDays.size())
        .pitchWork(pitchWork)
        .build();
  }
  ...
  public EnhancedCycleReportDTO getEnhancedCycleReport(Long cycleId) {
                ...
                .filter(
                    p ->
                        p.getStatus() == PitchStatus.IN_PROGRESS
                            || p.getStatus() == PitchStatus.STARTED)
                .count();
    int notStartedPitches =
        (int)
            pitches.stream()
                .filter(
                    p ->
                        p.getStatus() == PitchStatus.PENDING || p.getStatus() == PitchStatus.SHAPED)
                .count();
    ...
    double variancePercentage =
        totalAppetiteHours > 0 ? (varianceHours / totalAppetiteHours) * 100 : 0;
    double efficiency = totalAppetiteHours > 0 ? (totalActualHours / totalAppetiteHours) * 100 : 0;
    ...
    double maxHoursPerMember =
        memberReports.stream().mapToDouble(MemberWorkReportDTO::getTotalHours).max().orElse(0);
    double minHoursPerMember =
        memberReports.stream().mapToDouble(MemberWorkReportDTO::getTotalHours).min().orElse(0);

    // Identify top performers (members with above-average hours and efficiency)
    List<String> topPerformers =
        memberReports.stream()
  ...
  private RiskDistributionDTO calculateRiskDistribution(List<Pitch> pitches) {
    ...
    List<PitchRiskDTO> riskAnalyses =
        pitches.stream()
            .map(p -> riskAnalysisService.analyzePitchRisk(p, false))
            .collect(Collectors.toList());

    int lowRiskCount =
        (int) riskAnalyses.stream().filter(r -> r.getRiskLevel() == RiskLevel.LOW).count();
    int mediumRiskCount =
        (int) riskAnalyses.stream().filter(r -> r.getRiskLevel() == RiskLevel.MEDIUM).count();
    int highRiskCount =
        (int) riskAnalyses.stream().filter(r -> r.getRiskLevel() == RiskLevel.HIGH).count();
    int criticalRiskCount =
        (int) riskAnalyses.stream().filter(r -> r.getRiskLevel() == RiskLevel.CRITICAL).count();

    double averageRiskScore =
        riskAnalyses.stream().mapToDouble(PitchRiskDTO::getRiskScore).average().orElse(0);
    double maxRiskScore =
        riskAnalyses.stream().mapToDouble(PitchRiskDTO::getRiskScore).max().orElse(0);
    double minRiskScore =
        riskAnalyses.stream().mapToDouble(PitchRiskDTO::getRiskScore).min().orElse(0);

...
public class SlackIntegrationService implements NotificationProvider {
  ...
  public List<SlackConfigurationDTO> getAllConfigurations() {
    return slackConfigRepository.findAll().stream().map(this::toDTO).collect(Collectors.toList());
  }

  /** Get active Slack configuration */
  @Transactional(readOnly = true)
  public Optional<SlackConfigurationDTO> getActiveConfiguration() {
    return slackConfigRepository.findFirstByIsEnabledTrue().map(this::toDTO);
  }

...
/**
 * Service for querying entity revision history using Hibernate Envers.
 * Provides methods to retrieve change history with computed field-level diffs.
 */
@Service
...
public class AuditService {
     ...
     */
    private <T> Page<EntityHistoryDTO> getEntityHistory(
            Class<T> entityClass, 
            Long entityId, 
            Pageable pageable,
        ...
        for (Object[] revisionData : chronologicalRevisions) {
            ...
            EntityHistoryDTO historyDTO = EntityHistoryDTO.builder()
                    ...
                    .changes(computeFieldChanges(previousEntity, entity, fieldExtractor, enversRevisionType))
                    .build();
     ...
     */
    private <T> List<FieldChangeDTO> computeFieldChanges(
            T previousEntity, 
            T currentEntity,
            FieldValueExtractor<T> fieldExtractor,
    ...
    private List<String> getAuditedFieldNames(Class<?> entityClass) {
        if (entityClass.equals(Task.class)) {
            ...
            return List.of("title", "description", "status", "team", "isCircuitBreakerTriggered", "circuitBreakerReason");
    ...
    private String getPitchFieldValue(Pitch pitch, String fieldName) {
        ...
        return switch (fieldName) {
            ...
            case "isCircuitBreakerTriggered" -> pitch.getIsCircuitBreakerTriggered() != null 
                    ? pitch.getIsCircuitBreakerTriggered().toString() : null;

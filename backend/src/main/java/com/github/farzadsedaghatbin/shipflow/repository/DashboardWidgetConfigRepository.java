...
public interface DashboardWidgetConfigRepository extends JpaRepository<DashboardWidgetConfig, Long> {
  ...
  /**
   * Delete all widgets for a dashboard (cascade handled by DB, but useful for
   * manual cleanup)
   */
  void deleteByDashboardId(Long dashboardId);

  /** Get maximum display order for a dashboard (for appending new widgets) */
  @Query("SELECT COALESCE(MAX(w.displayOrder), 0) FROM DashboardWidgetConfig w WHERE w.dashboard.id = :dashboardId")

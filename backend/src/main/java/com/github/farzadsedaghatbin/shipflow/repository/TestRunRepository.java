...
public interface TestRunRepository extends JpaRepository<TestRun, Long> {
    ...
    @Query("SELECT tr FROM TestRun tr WHERE tr.pitch.id = :pitchId ORDER BY tr.executedAt DESC")
    List<TestRun> findByPitchIdOrderByExecutedAtDesc(@Param("pitchId") Long pitchId);

    @Query("SELECT tr FROM TestRun tr WHERE tr.cycle.id = :cycleId ORDER BY tr.executedAt DESC")
    ...
    @Query("SELECT tr FROM TestRun tr WHERE tr.executedAt >= :fromDate AND tr.executedAt <= :toDate ORDER BY tr.executedAt DESC")
    List<TestRun> findByExecutedAtBetween(@Param("fromDate") LocalDateTime fromDate, @Param("toDate") LocalDateTime toDate);

    @Query("SELECT tr FROM TestRun tr WHERE tr.pitch.id = :pitchId AND tr.status = 'PASSED' ORDER BY tr.executedAt DESC")
    List<TestRun> findPassedByPitchId(@Param("pitchId") Long pitchId);

    @Query("SELECT tr FROM TestRun tr WHERE tr.pitch.id = :pitchId AND tr.status = 'FAILED' ORDER BY tr.executedAt DESC")

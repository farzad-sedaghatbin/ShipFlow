...
public class TestCaseQualityMetrics {
    ...
    public Double calculateOverallScore() {
        ...
        if (requirementCoverage != null) { sum += requirementCoverage; count++; }
        if (scenarioDiversity != null) { sum += scenarioDiversity; count++; }
        if (stepClarity != null) { sum += stepClarity; count++; }
        if (actionability != null) { sum += actionability; count++; }
        if (consistency != null) { sum += consistency; count++; }
        if (completeness != null) { sum += completeness; count++; }

 ...
 * 
 * <p>This factory automatically discovers all registered {@link VectorStoreProvider} implementations
 * and creates the appropriate store based on configuration.
 * 
 * <p>Usage:
 * <pre>
 * {@code
 ...
 * <ol>
 *   <li>Add the provider type to {@link VectorStoreProviderType}</li>
 *   <li>Create a provider implementation of {@link VectorStoreProvider}</li>
 *   <li>Add required dependency to pom.xml</li>
 *   <li>Annotate with @Component for auto-discovery</li>
 * </ol>
 */
@Component
@Slf4j
public class VectorStoreProviderFactory {
    ...
    public VectorStoreProviderFactory(List<VectorStoreProvider> providerList) {
        ...
        for (VectorStoreProvider provider : providerList) {
            ...
            if (providers.containsKey(type)) {
                log.warn("Duplicate vector store provider registration for type {}: {} will be overwritten by {}",
                        type, providers.get(type).getClass().getSimpleName(),
                        provider.getClass().getSimpleName());
            }
            providers.put(type, provider);
            log.info("Registered vector store provider: {} -> {}", 
                    type.getConfigValue(), provider.getClass().getSimpleName());
        }
        
        log.info("Vector Store Provider Factory initialized with {} providers: {}", 
     ...
     */
    public EmbeddingStore<TextSegment> createStore(VectorStoreProviderType providerType, VectorStoreProviderConfig config) {
        ...
        log.info("Creating vector store using provider: {} ({})", 
                providerType.getConfigValue(), provider.getClass().getSimpleName());
        
        // Validate configuration
        try {
            provider.validateConfig(config);
        } catch (Exception e) {
            log.error("Configuration validation failed for provider {}: {}", 
        ...
        try {
            ...
            log.info("Successfully created vector store: {} with collection '{}'", 
                    providerType.getConfigValue(), config.getCollectionName());
            return store;
        } catch (Exception e) {
            log.error("Failed to create vector store for provider {}: {}", 

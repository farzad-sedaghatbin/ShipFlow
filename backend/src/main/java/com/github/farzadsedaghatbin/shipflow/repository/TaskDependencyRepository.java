...
public interface TaskDependencyRepository extends JpaRepository<TaskDependency, Long> {
  ...
  /** Find all dependencies where the given task is the source (it blocks or depends on others). */
  List<TaskDependency> findBySourceTaskId(Long sourceTaskId);

  /**
   * Find all dependencies where the given task is the target (it is blocked by or depended upon by
   * others).
   */
  ...
  /**
   * Find all tasks that are blocking the given task (dependencies where this task is the source and
   * type is BLOCKS).
   */
  @Query(
      "SELECT td FROM TaskDependency td WHERE td.sourceTask.id = :taskId AND td.dependencyType = 'BLOCKS'")
  List<TaskDependency> findBlockingDependenciesByTaskId(@Param("taskId") Long taskId);

  /**
   * Find all tasks that are blocked by the given task (dependencies where this task is the target
   * and type is BLOCKS).
   */
  @Query(
      "SELECT td FROM TaskDependency td WHERE td.targetTask.id = :taskId AND td.dependencyType = 'BLOCKS'")
  ...
  @Query(
      "SELECT CASE WHEN COUNT(td) > 0 THEN true ELSE false END FROM TaskDependency td "

...
public class TeamsIntegrationService {
  ...
  public List<TeamsConfigurationDTO> getAllConfigurations() {
    return teamsConfigRepository.findAll().stream().map(this::toDTO).collect(Collectors.toList());
  }

  /** Get active Teams configuration */
  @Transactional(readOnly = true)
  public Optional<TeamsConfigurationDTO> getActiveConfiguration() {
    return teamsConfigRepository.findFirstByIsEnabledTrue().map(this::toDTO);
  }
  ...
  public void sendTestNotification(Long configId, TestTeamsNotificationRequest request) {
    ...
    try {
      ...
      log.error(
          "❌ Network error sending Teams notification - this usually indicates connection timeout, DNS issues, or Teams service unavailability: {}",
          e.getMessage());
      throw new RuntimeException(
          "Connection failed to Teams webhook. Please check: 1) Internet connectivity, 2) Webhook URL is correct, 3) Teams service is available. Error: "
              + e.getMessage());
      ...
      // Check for traditional Logic App URL mistakenly used instead of Teams webhook
        ...
        throw new RuntimeException(
            "Teams rejected the webhook payload (400 Bad Request). The webhook URL might be expired or the message format is invalid.");
      } else if (errorMsg.contains("404")) {
        throw new RuntimeException(
            "Teams webhook not found (404). Please check if the webhook URL is correct and still active.");
      } else if (errorMsg.contains("401")) {
        throw new RuntimeException(
            "Authentication failed (401). The webhook URL might be expired or invalid. Please recreate your Teams incoming webhook.");
      } else {
  ...
  private void validateWebhookUrl(String webhookUrl) {
    ...
    if (!isPowerAutomateUrl && !isTraditionalTeamsUrl) {
      log.warn(
          "⚠️  Webhook URL does not match expected Teams webhook or Power Automate patterns. Please verify it's correct: {}",
          webhookUrl.substring(0, Math.min(60, webhookUrl.length())) + "...");
      log.info(
          "💡 Supported URLs: Teams webhooks (https://outlook.office.com/webhook/...) or Power Automate flows (https://...powerplatform.com/...)");
    }

    if (isPowerAutomateUrl) {
      log.info(
          "🔄 Detected Power Automate URL - make sure your flow is configured to 'Accept from anyone' for anonymous access");
    }
  }
}

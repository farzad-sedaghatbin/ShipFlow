...
/**
 * Server-side caching service for AI responses.
 * Provides intelligent caching to reduce AI API calls across all users.
 * 
 * Features:
 * - TTL-based cache expiration (0 = infinite until invalidation or restart)
 * - Automatic invalidation when data changes
 * - Fuzzy matching for similar Q&A questions
 * - Feature flags for enabling/disabling caching per feature
 * - Thread-safe concurrent access
 * - Supports in-memory (dev) and Redis (production) providers
 * 
...
public class AICacheService {
     ...
     * Note: This is a placeholder - actual implementation would use Jedis or Lettuce.
     */
    ...
    public static class CacheEntry<T> {
         ...
         * Returns false if expiresAt is null (infinite TTL).
         */
    ...
    public void cachePitchRisk(Long pitchId, boolean withAI, PitchRiskDTO riskData, String dataHash) {
        ...
        CacheEntry<PitchRiskDTO> entry = CacheEntry.<PitchRiskDTO>builder()
                .data(riskData)
    ...
    public void cacheCycleRisk(Long cycleId, boolean withAI, CycleRiskOverviewDTO riskData) {
        ...
        CacheEntry<CycleRiskOverviewDTO> entry = CacheEntry.<CycleRiskOverviewDTO>builder()
                .data(riskData)
                .createdAt(LocalDateTime.now())
                .expiresAt(expiresAt)
                .hitCount(0)
    ...
    public void cacheQAResponse(
        ...
        // TTL = 0 means infinite (null expiresAt)
        LocalDateTime expiresAt = ttlHours > 0 
                ? LocalDateTime.now().plusHours(ttlHours) 
                : null;
        
        CacheEntry<QAResponse> entry = CacheEntry.<QAResponse>builder()
                .data(response)
                .createdAt(LocalDateTime.now())
                .expiresAt(expiresAt)
                .hitCount(0)
    ...
    public void cleanupExpiredEntries() {
        ...
        for (Iterator<Map.Entry<String, CacheEntry<PitchRiskDTO>>> it = pitchRiskCache.entrySet().iterator(); it.hasNext();) {
        ...
        for (Iterator<Map.Entry<String, CacheEntry<CycleRiskOverviewDTO>>> it = cycleRiskCache.entrySet().iterator(); it.hasNext();) {
            if (it.next().getValue().isExpired()) {
                it.remove();
                cycleRemoved++;
            }
        ...
        if (pitchRemoved + cycleRemoved + qaRemoved > 0) {
            log.info("Cache cleanup: removed {} pitch, {} cycle, {} Q&A expired entries", 
                    pitchRemoved, cycleRemoved, qaRemoved);
        }
    }

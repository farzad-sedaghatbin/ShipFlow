...
/**
 * Re-ranks retrieved documents for better relevance ordering.
 * Uses score-based and metadata-based re-ranking since we don't have a cross-encoder.
 */
@Component
@Slf4j
public class DocumentReranker {
    ...
    public List<EmbeddingMatch<TextSegment>> rerank(
        ...
        List<ScoredMatch> scoredMatches = matches.stream()
                .map(match -> scoreMatch(match, query))
        ...
        // Return top K
        List<EmbeddingMatch<TextSegment>> reranked = scoredMatches.stream()
                .limit(topK)
                .map(ScoredMatch::getMatch)
                .collect(Collectors.toList());
        
        log.debug("Re-ranking improved avg score from {:.3f} to {:.3f}",
    ...
    private ScoredMatch scoreMatch(EmbeddingMatch<TextSegment> match, String query) {
        ...
        double compositeScore = 
                embeddingScore * 0.70 +    // Embedding similarity is most important
    ...
    /**
     * Penalize documents that are too short or too long.
     * Sweet spot: 100-1000 characters.
     */

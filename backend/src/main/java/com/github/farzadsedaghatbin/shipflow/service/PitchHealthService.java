 ...
 *
 * <p>AUTOMATED RISK DETECTION: This service implements sophisticated automated risk detection that
 * analyzes multiple data points to calculate health status WITHOUT requiring manual assignment:
 *
 * <p>1. Budget Analysis (configurable weight, default 25%): - Compares actual hours spent vs.
 * appetite (budget) - Flags when spending is ahead of schedule or over budget - Considers burn rate
 * acceleration
 *
 * <p>2. Bug Analysis (configurable weight, default 30%): - Tracks critical/blocker bugs (highest
 * priority) - Monitors high-severity bug counts - Analyzes bug density and resolution rates -
 * Detects recent bug influx (regression indicators)
 *
 * <p>3. Scope Completion Analysis (configurable weight, default 25%): - Uses hill chart positions
 * to track understanding and progress - Compares expected progress vs. actual scope completion -
 * Identifies stagnant scopes (no recent movement) - Flags scopes stuck at decision points
 *
 * <p>4. Time & Status Analysis (configurable weight, default 20%): - Evaluates status
 * appropriateness for time remaining - Detects deadline pressure situations - Checks for delayed
 * starts or slow progress
 *
 * <p>WEIGHT CUSTOMIZATION: - Weights are configurable per organization via settings - Preset
 * profiles available: balanced, conservative, aggressive, quality_focused, time_critical - Weights
 * must sum to 100%
 *
 * <p>MODES: - Fast Mode (default): Rule-based calculation for instant results - AI Mode (optional):
 * Enhanced with AI-powered risk analysis (slower)
 *
 * <p>The service provides both pitch-level and cycle-level health summaries, with automatic sorting
 * and visual indicators for critical items.
 */
@Service
@Slf4j
...
public class PitchHealthService {
   ...
   *
   * <p>This value (0.76 = 76%) determines the balance between weighted average risk and maximum
   * individual risk factor when calculating final risk scores. It ensures that a single severe
   * factor can trigger appropriate risk levels even when other factors would dilute the weighted
   * average.
   *
   * <p>WHY 76%? - Must be > 60% to ensure a risk factor score of 80 (severe) crosses the MEDIUM
   * threshold (60) - 0.75 (75%) results in exactly 60 (80 × 0.75 = 60), which fails boundary
   * conditions - 0.76 (76%) yields 60.8 (80 × 0.76 = 60.8), safely exceeding the MEDIUM threshold -
   * Prevents weighted averaging from masking critical single-factor risks - Example: 150% budget
   * overrun (risk=80) → 80×0.76=60.8 → HIGH risk level
   *
   * <p>NOTE: This constant is based on default risk thresholds (MEDIUM=25-49, HIGH=50-69). If
   * organization settings customize these thresholds significantly, this multiplier may need
   * adjustment to maintain the intended behavior.
   */
  ...
  private com.github.farzadsedaghatbin.shipflow.dto.admin.OrganizationSettingsDTO.RiskThresholds
      getThresholds() {
    ...
    return new com.github.farzadsedaghatbin.shipflow.dto.admin.OrganizationSettingsDTO
        .RiskThresholds();
  }

  /** Get risk weights from organization settings, or use defaults if not available. */
  private com.github.farzadsedaghatbin.shipflow.dto.admin.OrganizationSettingsDTO.RiskWeights
      getWeights() {
    if (organizationSettingsService != null) {
      try {
        var settings = organizationSettingsService.getSettings();
    ...
    // Return defaults if service not available or settings not configured
    return com.github.farzadsedaghatbin.shipflow.dto.admin.OrganizationSettingsDTO.RiskWeights
        .builder()
        .budgetWeight(
            com.github.farzadsedaghatbin.shipflow.dto.admin.OrganizationSettingsDTO.RiskWeights
  ...
  public CycleHealthSummaryDTO getCycleHealthSummary(Long cycleId, boolean includeAI) {
    ...
    List<PitchHealthDTO> pitchHealthList =
        pitches.stream()
    ...
    double cycleProgress =
        totalDays > 0 ? Math.min(100, (double) elapsedDays / totalDays * 100) : 0;

    // Risk breakdown
    int healthyCount =
        (int) pitchHealthList.stream().filter(p -> p.getRiskLevel() == RiskLevel.LOW).count();
    int atRiskCount =
        (int) pitchHealthList.stream().filter(p -> p.getRiskLevel() == RiskLevel.MEDIUM).count();
    int criticalCount =
        (int)
            pitchHealthList.stream()
                .filter(
                    p ->
                        p.getRiskLevel() == RiskLevel.HIGH
                            || p.getRiskLevel() == RiskLevel.CRITICAL)
                .count();

    // Budget calculations
    double totalAppetiteHours =
        pitchHealthList.stream()
    ...
    int pendingCount =
        (int)
            pitchHealthList.stream()
                ...
                .count();
    int inProgressCount =
        (int) pitchHealthList.stream().filter(p -> "IN_PROGRESS".equals(p.getStatus())).count();
    int testingCount =
        (int) pitchHealthList.stream().filter(p -> "TESTING".equals(p.getStatus())).count();
    int doneCount =
        (int) pitchHealthList.stream().filter(p -> "DONE".equals(p.getStatus())).count();
  ...
  private PitchHealthDTO buildPitchHealth(
      Pitch pitch, boolean includeAI, Map<Long, Double> hoursMap) {
  ...
  /**
   * Fast rule-based risk level calculation without AI. Includes automated detection based on: -
   * Budget vs time progress (weighted 25%) - Critical/blocker bug counts (weighted 30%) - Scope
   * completion status (weighted 25%) - Time remaining and status (weighted 20%)
   *
   * <p>This provides immediate, data-driven risk assessment without manual input.
   */
  private RiskLevel calculateRuleBasedRiskLevel(Pitch pitch, Map<Long, Double> hoursMap) {
    ...
    double maxIndividualRisk =
        Math.max(Math.max(budgetRisk, bugsRisk), Math.max(scopeRisk, timeRisk));
  ...
  private double calculateBudgetRisk(
      double appetiteUsed,
      double cycleProgress,
      com.github.farzadsedaghatbin.shipflow.dto.admin.OrganizationSettingsDTO.RiskThresholds
          thresholds) {
    double riskScore = 0;
    ...
    if (appetiteUsed > thresholds.getBudgetCritical()) {
      ...
      double warningLevel =
          (appetiteUsed - thresholds.getBudgetWarning())
  ...
  private double calculateBugsRisk(
      Pitch pitch,
      long daysElapsed,
      long totalCycleDays,
      com.github.farzadsedaghatbin.shipflow.dto.admin.OrganizationSettingsDTO.RiskThresholds
    ...
    long criticalBugs =
        bugs.stream()
            .filter(
                b ->
                    (b.getSeverity() == BugSeverity.CRITICAL
                            || b.getSeverity() == BugSeverity.BLOCKER)
                        && b.getStatus() != BugStatus.RESOLVED
                        && b.getStatus() != BugStatus.CLOSED)
            .count();
    ...
    // Major severity bugs
    long majorSeverityBugs =
        bugs.stream()
            .filter(
                b ->
                    b.getSeverity() == BugSeverity.MAJOR
                        && b.getStatus() != BugStatus.RESOLVED
                        && b.getStatus() != BugStatus.CLOSED)
            .count();

    if (majorSeverityBugs > thresholds.getMajorBugsHigh()
    ...
    if (openBugs > thresholds.getOpenBugsCritical() && daysLeft < thresholds.getDaysWarning()) {
      ...
      riskScore += 25; // Too many open bugs with little time
        ...
        && daysLeft < thresholds.getDaysUrgent()) {
    ...
    if (bugResolutionRate < thresholds.getBugResolutionRateMin()
        && openBugs > thresholds.getMajorBugsThreshold()
        && daysLeft < thresholds.getDaysWarning()) {
  ...
  private double calculateScopeRisk(
      Pitch pitch,
      double cycleProgress,
      com.github.farzadsedaghatbin.shipflow.dto.admin.OrganizationSettingsDTO.RiskThresholds
          thresholds) {
    ...
    if (!scopes.isEmpty()) {
      double avgPosition =
          scopes.stream().mapToInt(HillChartPoint::getPosition).average().orElse(0);
      ...
      if (cycleProgress > thresholds.getCycleFinalQuarter()
          && avgPosition < thresholds.getScopeUphillMax()) {
        riskScore += 60; // Critical: Still figuring things out in the final quarter
      } else if (cycleProgress > thresholds.getCycleLatePhase()
          && avgPosition < thresholds.getScopeMidPhase()) {
        riskScore += 45; // High risk: Past 60% but scopes still mostly uphill
      } else if (cycleProgress > thresholds.getCycleMidpoint()
          && avgPosition < thresholds.getScopeEarlyPhase()) {
      ...
      long stagnantScopes =
          scopes.stream()
              .filter(
                  s ->
                      s.getPosition() < thresholds.getScopeUphillMax()
                          && s.getUpdatedAt()
                              .isBefore(
                                  LocalDateTime.now()
                                      .minusDays(thresholds.getScopeStagnationDays())))
              .count();
      ...
      long peakStuckScopes =
          scopes.stream()
              .filter(
                  s ->
                      s.getPosition() >= thresholds.getScopePeakMin()
                          && s.getPosition() <= thresholds.getScopePeakMax()
                          && s.getUpdatedAt()
                              .isBefore(
                                  LocalDateTime.now().minusDays(thresholds.getPeakStuckDays())))
              .count();
  ...
  private double calculateTimeRisk(
    ...
    if (daysLeft <= thresholds.getDaysUrgent()
  ...
  /**
   * Calculate risk trend based on recent changes. Analyzes if conditions are improving, stable, or
   * worsening.
   */
  private String calculateRiskTrend(Pitch pitch, double appetiteUsed, int daysLeft) {
    ...
    double recentHours =
        recentLogs.stream()
            .mapToDouble(
                log -> log.getHoursSpent() != null ? log.getHoursSpent().doubleValue() : 0.0)
            .sum();

    // If spending is accelerating and already over budget, that's worsening
    if (appetiteUsed > thresholds.getAppetiteHighUsage()
        && recentHours > thresholds.getRecentWorkHighHours()) {
      trendScore -= 2; // Worsening
    }

    // 2. Check hill chart movement
    List<HillChartPoint> scopes =
        hillChartPointRepository.findByPitchIdOrderByUpdatedAtDesc(pitch.getId());
    if (!scopes.isEmpty()) {
      ...
      boolean hasRecentProgress =
          scopes.stream().anyMatch(s -> s.getUpdatedAt().isAfter(sevenDaysAgo));
    ...
    List<BugReport> recentBugs =
        bugReportRepository.findByPitchId(pitch.getId()).stream()
            .filter(b -> b.getCreatedAt().isAfter(threeDaysAgoTime))
            .collect(Collectors.toList());

    long recentCriticalBugs =
        recentBugs.stream()
            .filter(
                b ->
                    b.getSeverity() == BugSeverity.CRITICAL
    ...
    if (daysLeft <= 5
        && pitch.getStatus() != PitchStatus.DONE
        && pitch.getStatus() != PitchStatus.TESTING) {
  ...
  private String determineQAStatus(Pitch pitch) {
    ...
    boolean hasQAActivity =
        meetings.stream()
            .anyMatch(
                m ->
                    m.getType() == MeetingType.DEMO
  ...
  /**
   * Generate human-readable status summary with automated risk indicators. Provides context-aware
   * messages based on budget, time, and QA status.
   */
  private String generateStatusSummary(
      Pitch pitch, double appetiteUsed, int daysLeft, String qaStatus) {
    var thresholds = getThresholds();
    ...
    switch (status) {
      ...
      case IN_PROGRESS:
        if (appetiteUsed > thresholds.getBudgetCritical()) {
          summary
              .append("Significantly over budget (")
              .append(String.format("%.0f", appetiteUsed))
              .append("%)");
        } else if (appetiteUsed > thresholds.getBudgetOverrun()) {

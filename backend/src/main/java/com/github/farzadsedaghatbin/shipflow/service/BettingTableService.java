...
public class BettingTableService {
  ...
  public BettingTableDTO getBettingTable(Long cycleId) {
    ...
    // Group slots by team
    Map<Long, List<BettingSlot>> slotsByTeam =
        allSlots.stream().collect(Collectors.groupingBy(slot -> slot.getTeam().getId()));
    ...
    // Build team tracks
    List<BettingTableDTO.TeamTrackDTO> teamTracks =
        teams.stream()
            .map(
                team ->
                    buildTeamTrack(
                        team,
                        slotsByTeam.getOrDefault(team.getId(), new ArrayList<>()),
                        cycleDurationWeeks))
            .collect(Collectors.toList());

    // Calculate totals
    int totalCapacity =
        teamTracks.stream().mapToInt(BettingTableDTO.TeamTrackDTO::getTotalCapacityWeeks).sum();
    int usedCapacity =
        teamTracks.stream().mapToInt(BettingTableDTO.TeamTrackDTO::getUsedCapacityWeeks).sum();
  ...
  public List<PitchDTO> getShapedPitchesForBetting(Long projectId) {
    ...
    if (projectId != null) {
      ...
      pitches =
          pitchRepository.findAll().stream()
              .filter(
                  p ->
                      p.getStatus() == PitchStatus.SHAPED
                          && cycleIds.contains(p.getCycle().getId()))
              .collect(Collectors.toList());
    } else {
      pitches =
          pitchRepository.findAll().stream()
    ...
    Set<Long> assignedIds =
        bettingSlotRepository.findAll().stream()
            .filter(slot -> slot.getPitch() != null)
            .map(slot -> slot.getPitch().getId())
            .collect(Collectors.toSet());
  ...
  public BettingSlotDTO createSlot(CreateBettingSlotRequest request) {
    ...
    bettingSlotRepository
        .findByCycleIdAndTeamIdAndPosition(
            request.getCycleId(), request.getTeamId(), request.getPosition())
        .ifPresent(
            existing -> {
              throw new IllegalArgumentException(
                  messageService.getMessage(
                      "error.betting.slot.position.exists", request.getPosition(), team.getName()));
            });
  ...
  public List<BettingSlotDTO> generateSlotsForCycle(Long cycleId) {
    ...
    for (Team team : teams) {
      ...
      List<BettingSlot> existingSlots =
          bettingSlotRepository.findByCycleIdAndTeamId(cycleId, team.getId());
  ...
  /**
   * Assign a pitch to a slot (the main betting action) This will resize the slot to match the
   * pitch's appetite and create a new empty slot for remaining time
   */
  public BettingSlotDTO assignPitchToSlot(Long slotId, Long pitchId) {
    BettingSlot slot =
        bettingSlotRepository
            .findById(slotId)
            .orElseThrow(
                () -> new IllegalArgumentException("Betting slot not found with id: " + slotId));
        ...
        .ifPresent(
            existingSlot -> {
              if (!existingSlot.getId().equals(slotId)) {
                throw new IllegalArgumentException(
                    messageService.getMessage("error.betting.pitch.already.assigned"));
              }
            });
  ...
  /** Remove a pitch from a slot (unassign) This will merge the slot with any adjacent empty slot */
  public BettingSlotDTO removePitchFromSlot(Long slotId) {
    ...
    List<BettingSlot> teamSlots =
        bettingSlotRepository.findByCycleIdAndTeamId(
            slot.getCycle().getId(), slot.getTeam().getId());
  ...
  public BettingSlotDTO updateSlotDates(Long slotId, LocalDate startDate, LocalDate endDate) {
    ...
    if (startDate.isBefore(cycle.getStartDate()) || endDate.isAfter(cycle.getEndDate())) {
      throw new IllegalArgumentException(
          messageService.getMessage(
              "error.betting.slot.dates.invalid", cycle.getStartDate(), cycle.getEndDate()));
    }
    ...
    if (slot.getPitch() != null) {
      ...
      if (slot.getPitch().getAppetiteDays() > newDays) {
        throw new IllegalArgumentException(
            messageService.getMessage(
                "error.betting.slot.resize.invalid", slot.getPitch().getAppetiteDays(), newDays));
      }
    }
  ...
  private void validatePitchFits(Pitch pitch, BettingSlot slot) {
    if (!slot.canFitPitch(pitch.getAppetiteDays())) {
      ...
      throw new IllegalArgumentException(
          messageService.getMessage(
              "error.betting.pitch.doesnt.fit",
              pitch.getTitle(),
              pitch.getAppetiteDays(),
              slotDays));
    }
  }

  private BettingTableDTO.TeamTrackDTO buildTeamTrack(
      Team team, List<BettingSlot> slots, int cycleDurationWeeks) {
    List<BettingSlotDTO> slotDTOs =
        slots.stream()
  ...
  private PitchDTO toPitchDTO(Pitch pitch) {
    ...
    return PitchDTO.builder()
        ...
        .projectId(
            pitch.getCycle().getProject() != null ? pitch.getCycle().getProject().getId() : null)
        .projectName(
            pitch.getCycle().getProject() != null ? pitch.getCycle().getProject().getName() : null)
        .projectKey(
            pitch.getCycle().getProject() != null
                ? pitch.getCycle().getProject().getProjectKey()
                : null)
  ...
  public TeamPerformanceHistoryDTO getTeamPerformanceHistory(Long teamId) {
    ...
    // Group by cycle
    Map<Long, List<BettingSlot>> slotsByCycle =
        allHistoricalSlots.stream()
            .filter(slot -> slot.getPitch() != null)
    ...
    if (!cycleIds.isEmpty()) {
      ...
      lastCycleCompletedBets =
          (int)
              lastCycleSlots.stream()
                  .filter(slot -> slot.getPitch().getStatus() == PitchStatus.DONE)
                  .count();
    }

    Double lastCycleCompletionRate =
        lastCycleTotalBets > 0 ? (lastCycleCompletedBets * 100.0 / lastCycleTotalBets) : 0.0;
    ...
    return TeamPerformanceHistoryDTO.builder()
        ...
        .avgBetsPerCycle(
            totalCycles > 0 ? Math.round((totalBets * 10.0 / totalCycles)) / 10.0 : 0.0)
  ...
  public CapacityAnalysisDTO getCapacityAnalysis(Long cycleId) {
    ...
    for (BettingTableDTO.TeamTrackDTO track : bettingTable.getTeamTracks()) {
      ...
      if (utilization > 100) {
        ...
        warnings.add(
            CapacityAnalysisDTO.CapacityWarning.builder()
                .severity("CRITICAL")
                .teamId(track.getTeamId())
                .teamName(track.getTeamName())
                .message(
                    String.format(
                        "Team is over-allocated by %d weeks (%.0f%% capacity)",
        ...
        warnings.add(
            CapacityAnalysisDTO.CapacityWarning.builder()
                ...
                .message(
                    String.format(
                        "Team has significant available capacity: %.0f%% utilized", utilization))
                .utilizationRate(utilization)
                .type("UNDER_UTILIZED")
                .build());
      }
    ...
    if (isOverAllocated) {
      recommendations.add(
          "⚠️ Some teams are over-allocated. Consider redistributing work or reducing scope.");
    }
    ...
    if (totalAvailable > 0 && bettingTable.getShapedPitches().size() > 0) {
      recommendations.add(
          String.format(
              "✅ %d weeks of capacity available for %d shaped pitches",
              totalAvailable, bettingTable.getShapedPitches().size()));
    }

    double overallUtilization =
        bettingTable.getTotalCapacityWeeks() > 0
  ...
  private PitchComparisonDTO buildPitchComparison(Pitch pitch, List<Team> teams, Cycle cycle) {
    List<String> risks =
        pitch.getRisks() != null && !pitch.getRisks().isEmpty()
            ? Arrays.asList(pitch.getRisks().split("\\n"))
            : new ArrayList<>();

    List<String> rabbitHoles =
        pitch.getRabbitHoles() != null && !pitch.getRabbitHoles().isEmpty()
    ...
    List<PitchComparisonDTO.TeamFitAnalysis> teamFitScores =
        teams.stream().map(team -> analyzeTeamFit(team, pitch, cycle)).collect(Collectors.toList());
    ...
    return PitchComparisonDTO.builder()
        ...
        .estimatedBusinessValue(
            null) // NOTE: Not currently calculated - intentionally returned as null until business
        // value logic is implemented
        .urgency(
            null) // NOTE: Not currently calculated - intentionally returned as null until urgency
  ...
  private PitchComparisonDTO.TeamFitAnalysis analyzeTeamFit(Team team, Pitch pitch, Cycle cycle) {
    ...
    List<BettingSlot> teamSlots =
        bettingSlotRepository.findByCycleIdAndTeamId(cycle.getId(), team.getId());
  ...
  private String calculateTrend(List<Long> cycleIds, Map<Long, List<BettingSlot>> slotsByCycle) {
    ...
    for (int i = 0; i < Math.min(3, cycleIds.size()); i++) {
      ...
      long completed =
          slots.stream().filter(s -> s.getPitch().getStatus() == PitchStatus.DONE).count();
  ...
  /**
   * Calculate average time overrun percentage from work log analysis Compares actual hours spent vs
   * appetite hours for completed bets
   */
  private Double calculateAvgTimeOverrun(Map<Long, List<BettingSlot>> slotsByCycle) {
    ...
    double avgOverrun =
        overrunPercentages.stream().mapToDouble(Double::doubleValue).average().orElse(0.0);

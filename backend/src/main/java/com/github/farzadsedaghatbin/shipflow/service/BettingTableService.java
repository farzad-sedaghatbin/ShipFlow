...
public class BettingTableService {
  ...
  public BettingTableDTO getBettingTable(Long cycleId) {
    ...
    Set<Long> assignedPitchIds = allSlots.stream().filter(slot -> slot.getPitch() != null)
        .map(slot -> slot.getPitch().getId()).collect(Collectors.toSet());

    List<PitchDTO> availablePitches = shapedPitches.stream()
        .filter(pitch -> !assignedPitchIds.contains(pitch.getId())).map(this::toPitchDTO)
    ...
    // Build team tracks
    List<BettingTableDTO.TeamTrackDTO> teamTracks = teams.stream().map(team -> buildTeamTrack(team,
        slotsByTeam.getOrDefault(team.getId(), new ArrayList<>()), cycleDurationWeeks))
        .collect(Collectors.toList());
    ...
    return BettingTableDTO.builder().cycleId(cycle.getId()).cycleName(cycle.getName())
        ...
        .cycleStartDate(cycle.getStartDate()).cycleEndDate(cycle.getEndDate())
        .cycleDurationWeeks(cycleDurationWeeks).isCycleActive(cycle.getIsActive())
        .shapedPitches(availablePitches).teamTracks(teamTracks).totalShapedPitches(shapedPitches.size())
        .totalAssignedPitches(assignedPitchIds.size()).totalCapacityWeeks(totalCapacity)
        .usedCapacityWeeks(usedCapacity).build();
  }
  ...
  public List<PitchDTO> getShapedPitchesForBetting(Long projectId) {
    ...
    if (projectId != null) {
      ...
      pitches = pitchRepository.findAll().stream().filter(p -> p.getStatus() == PitchStatus.SHAPED)
          .collect(Collectors.toList());
    }
    ...
    return pitches.stream().filter(p -> !assignedIds.contains(p.getId())).map(this::toPitchDTO)
        .collect(Collectors.toList());
  }
  ...
  public BettingSlotDTO createSlot(CreateBettingSlotRequest request) {
    ...
    bettingSlotRepository
        .findByCycleIdAndTeamIdAndPosition(request.getCycleId(), request.getTeamId(), request.getPosition())
        .ifPresent(existing -> {
          throw new IllegalArgumentException(messageService.getMessage("error.betting.slot.position.exists",
              request.getPosition(), team.getName()));
        });

    BettingSlot slot = BettingSlot.builder().cycle(cycle).team(team).position(request.getPosition())
        .startDate(request.getStartDate()).endDate(request.getEndDate()).notes(request.getNotes()).build();
  ...
  public List<BettingSlotDTO> generateSlotsForCycle(Long cycleId) {
    ...
    for (Team team : teams) {
      ...
      BettingSlot slot = BettingSlot.builder().cycle(cycle).team(team).position(0).startDate(cycle.getStartDate())
  ...
  /**
   * Assign a pitch to a slot (the main betting action) This will resize the slot
   * to match the pitch's appetite and create a new empty slot for remaining time
   */
  public BettingSlotDTO assignPitchToSlot(Long slotId, Long pitchId) {
    ...
    if (newSlotEndDate.isBefore(originalSlotEndDate)) {
      ...
      Integer maxPosition = bettingSlotRepository.findMaxPositionByCycleAndTeam(slot.getCycle().getId(),
          slot.getTeam().getId());
      if (maxPosition == null)
        maxPosition = -1;

      // Create new empty slot for remaining capacity
      BettingSlot remainingSlot = BettingSlot.builder().cycle(slot.getCycle()).team(slot.getTeam())
          .position(maxPosition + 1).startDate(newSlotEndDate).endDate(originalSlotEndDate).build();
  ...
  /**
   * Remove a pitch from a slot (unassign) This will merge the slot with any
   * adjacent empty slot
   */
  public BettingSlotDTO removePitchFromSlot(Long slotId) {
    ...
    List<BettingSlot> teamSlots = bettingSlotRepository.findByCycleIdAndTeamId(slot.getCycle().getId(),
        slot.getTeam().getId());
  ...
  public BettingSlotDTO updateSlotDates(Long slotId, LocalDate startDate, LocalDate endDate) {
    ...
    if (startDate.isBefore(cycle.getStartDate()) || endDate.isAfter(cycle.getEndDate())) {
      throw new IllegalArgumentException(messageService.getMessage("error.betting.slot.dates.invalid",
    ...
    if (slot.getPitch() != null) {
      ...
      if (slot.getPitch().getAppetiteDays() > newDays) {
        throw new IllegalArgumentException(messageService.getMessage("error.betting.slot.resize.invalid",
            slot.getPitch().getAppetiteDays(), newDays));
      }
    }
  ...
  private BettingSlotDTO toDTO(BettingSlot slot) {
    return BettingSlotDTO.builder().id(slot.getId()).cycleId(slot.getCycle().getId())
        .cycleName(slot.getCycle().getName()).teamId(slot.getTeam().getId()).teamName(slot.getTeam().getName())
        ...
        .position(slot.getPosition()).startDate(slot.getStartDate()).endDate(slot.getEndDate())
        .durationWeeks(slot.getDurationWeeks()).notes(slot.getNotes()).createdAt(slot.getCreatedAt())
        .updatedAt(slot.getUpdatedAt()).canFitPitch(true) // Default, will be recalculated in frontend
        .build();
  }
  ...
  public TeamPerformanceHistoryDTO getTeamPerformanceHistory(Long teamId) {
    ...
    // Group by cycle
    Map<Long, List<BettingSlot>> slotsByCycle = allHistoricalSlots.stream().filter(slot -> slot.getPitch() != null)
    ...
    int totalBets = allHistoricalSlots.stream().filter(slot -> slot.getPitch() != null)
        .filter(slot -> !slot.getCycle().getIsActive()).mapToInt(slot -> 1).sum();

    int totalCompletedBets = (int) allHistoricalSlots.stream().filter(slot -> slot.getPitch() != null)
        .filter(slot -> !slot.getCycle().getIsActive())
        .filter(slot -> slot.getPitch().getStatus() == PitchStatus.DONE).count();
    ...
    Double lastCycleCompletionRate = lastCycleTotalBets > 0
        ? (lastCycleCompletedBets * 100.0 / lastCycleTotalBets)
        : 0.0;
    Double overallCompletionRate = totalBets > 0 ? (totalCompletedBets * 100.0 / totalBets) : 0.0;
    ...
    return TeamPerformanceHistoryDTO.builder().teamId(team.getId()).teamName(team.getName())
        .lastCycleTotalBets(lastCycleTotalBets).lastCycleCompletedBets(lastCycleCompletedBets)
        .lastCycleCompletionRate(Math.round(lastCycleCompletionRate * 10.0) / 10.0).totalCycles(totalCycles)
        .totalBets(totalBets).totalCompletedBets(totalCompletedBets)
        ...
        .avgWeeksPerBet(avgWeeksPerBet).avgTimeOverrun(avgTimeOverrun).trend(trend)
        .performanceRating(performanceRating).build();
  }
  ...
  public CapacityAnalysisDTO getCapacityAnalysis(Long cycleId) {
    ...
    for (BettingTableDTO.TeamTrackDTO track : bettingTable.getTeamTracks()) {
      double utilization = track.getTotalCapacityWeeks() > 0
          ? (track.getUsedCapacityWeeks() * 100.0 / track.getTotalCapacityWeeks())
          : 0.0;

      if (utilization > 100) {
        isOverAllocated = true;
        warnings.add(CapacityAnalysisDTO.CapacityWarning.builder().severity("CRITICAL")
            .teamId(track.getTeamId()).teamName(track.getTeamName())
            ...
            .utilizationRate(utilization).type("OVER_ALLOCATED").build());
      } else if (utilization > 85) {
        warnings.add(CapacityAnalysisDTO.CapacityWarning.builder().severity("WARNING").teamId(track.getTeamId())
            .teamName(track.getTeamName())
            .message(String.format("Team capacity is tight: %.0f%% utilized", utilization))
            .utilizationRate(utilization).type("TIGHT_SCHEDULE").build());
      } else if (utilization < 50) {
        warnings.add(CapacityAnalysisDTO.CapacityWarning.builder().severity("INFO").teamId(track.getTeamId())
            .teamName(track.getTeamName())
            .message(String.format("Team has significant available capacity: %.0f%% utilized", utilization))
            .utilizationRate(utilization).type("UNDER_UTILIZED").build());
      }
    }
    ...
    if (totalAvailable > 0 && bettingTable.getShapedPitches().size() > 0) {
      recommendations.add(String.format("✅ %d weeks of capacity available for %d shaped pitches", totalAvailable,
          bettingTable.getShapedPitches().size()));
    }

    double overallUtilization = bettingTable.getTotalCapacityWeeks() > 0
        ? (bettingTable.getUsedCapacityWeeks() * 100.0 / bettingTable.getTotalCapacityWeeks())
        : 0.0;
    ...
    return CapacityAnalysisDTO.builder().cycleId(cycleId).cycleName(bettingTable.getCycleName())
        .totalTeams(bettingTable.getTeamTracks().size())
        .totalCapacityWeeks(bettingTable.getTotalCapacityWeeks())
        .allocatedWeeks(bettingTable.getUsedCapacityWeeks()).remainingWeeks(totalAvailable)
        .utilizationRate(overallUtilization / 100.0) // Convert to decimal (0.0 to 1.0)
        .warnings(warnings).isOverAllocated(isOverAllocated).hasConflicts(false)
  ...
  private PitchComparisonDTO buildPitchComparison(Pitch pitch, List<Team> teams, Cycle cycle) {
    ...
    List<PitchComparisonDTO.TeamFitAnalysis> teamFitScores = teams.stream()
        .map(team -> analyzeTeamFit(team, pitch, cycle)).collect(Collectors.toList());

    return PitchComparisonDTO.builder().pitch(toPitchDTO(pitch)).appetiteDays(pitch.getAppetiteDays())
        .complexityLevel(complexityLevel).risks(risks.isEmpty() ? List.of("No risks identified") : risks)
        .rabbitHoles(rabbitHoles.isEmpty() ? List.of("No rabbit holes identified") : rabbitHoles)
        .teamFitScores(teamFitScores).estimatedBusinessValue(null) // NOTE: Not currently calculated -
        // intentionally returned as null until
        // business
        // value logic is implemented
        .urgency(null) // NOTE: Not currently calculated - intentionally returned as null until urgency
  ...
  private PitchComparisonDTO.TeamFitAnalysis analyzeTeamFit(Team team, Pitch pitch, Cycle cycle) {
    ...
    return PitchComparisonDTO.TeamFitAnalysis.builder().teamId(team.getId()).teamName(team.getName())
        .availableCapacityWeeks(availableWeeks).canFit(canFit).capacityStatus(capacityStatus)
        .recommendation(recommendation).warnings(warnings).build();
  }
  ...
  private String calculateTrend(List<Long> cycleIds, Map<Long, List<BettingSlot>> slotsByCycle) {
    ...
    for (int i = 0; i < Math.min(3, cycleIds.size()); i++) {
      ...
      long completed = slots.stream().filter(s -> s.getPitch().getStatus() == PitchStatus.DONE).count();
      completionRates.add(total > 0 ? (completed * 100.0 / total) : 0.0);
    }

    if (completionRates.size() < 2)
      return "STABLE";
  ...
  /**
   * Calculate average time overrun percentage from work log analysis Compares
   * actual hours spent vs appetite hours for completed bets
   */
  private Double calculateAvgTimeOverrun(Map<Long, List<BettingSlot>> slotsByCycle) {
    ...
    if (overrunPercentages.isEmpty())
      return null;

    // Calculate average overrun
    double avgOverrun = overrunPercentages.stream().mapToDouble(Double::doubleValue).average().orElse(0.0);

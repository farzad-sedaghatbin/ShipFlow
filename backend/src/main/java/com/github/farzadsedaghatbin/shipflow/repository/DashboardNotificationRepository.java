...
public interface DashboardNotificationRepository
  ...
  List<DashboardNotification> findByUserIdAndIsReadOrderByCreatedAtDesc(
  ...
  List<DashboardNotification> findActiveNotifications(
      @Param("userId") Long userId, @Param("now") LocalDateTime now);

  /** Find expired notifications */
  @Query(
      "SELECT n FROM DashboardNotification n WHERE n.expiresAt IS NOT NULL "
          + "AND n.expiresAt <= :now")
  List<DashboardNotification> findExpiredNotifications(@Param("now") LocalDateTime now);

  /** Delete old read notifications */
  @Query(
      "DELETE FROM DashboardNotification n WHERE n.isRead = true " + "AND n.readAt < :cutoffDate")

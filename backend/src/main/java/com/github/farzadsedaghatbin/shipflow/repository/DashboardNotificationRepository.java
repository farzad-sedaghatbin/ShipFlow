...
public interface DashboardNotificationRepository extends JpaRepository<DashboardNotification, Long> {
  ...
  List<DashboardNotification> findActiveNotifications(@Param("userId") Long userId, @Param("now") LocalDateTime now);

  /** Find expired notifications */
  @Query("SELECT n FROM DashboardNotification n WHERE n.expiresAt IS NOT NULL " + "AND n.expiresAt <= :now")

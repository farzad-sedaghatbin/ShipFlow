...
/**
 * Service for saving risk history records. Separated to ensure proper transaction handling with
...
public class RiskHistoryService {
  ...
  /**
   * Save risk analysis result to history for trend tracking. Uses REQUIRES_NEW with readOnly=false
   * to ensure this runs in its own writable transaction, avoiding issues with parent transaction
   * read-only mode.
   *
  ...
  public void saveRiskHistory(
      Pitch pitch, PitchRiskDTO riskDTO, PitchRiskHistory.TriggerType triggerType) {
    try {
      log.debug(
          "Saving risk history for pitch {} in new transaction (readOnly=false)", pitch.getId());
      ...
      log.debug(
          "Saved risk history for pitch {} with score {} (trigger: {})",
          pitch.getId(),
          riskDTO.getRiskScore(),
          triggerType);

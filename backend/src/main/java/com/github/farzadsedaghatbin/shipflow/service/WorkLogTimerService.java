...
public class WorkLogTimerService {
    ...
    if (request.getPitchId() != null) {
      pitch =
          pitchRepository
              ...
              .orElseThrow(
                  () ->
                      new BadRequestException("Pitch not found with id: " + request.getPitchId()));
    }
    ...
    if (request.getTaskId() != null) {
      task =
          taskRepository
              ...
              .orElseThrow(
                  () -> new BadRequestException("Task not found with id: " + request.getTaskId()));
    }
  ...
  public StopTimerResponse stopTimer() {
    ...
    WorkLog workLog =
        WorkLog.builder()
            .person(person)
            .pitch(timer.getPitch())
            .task(timer.getTask())
            .date(LocalDate.now())
            .hoursSpent(BigDecimal.valueOf(hours).setScale(2, RoundingMode.HALF_UP))
            .note(timer.getNote())
            .build();
    ...
    log.info(
        "Stopped timer {} and created work log {} for {} hours",
        timer.getId(),
        savedWorkLog.getId(),
        hours);

    return StopTimerResponse.builder()
  ...
  private WorkLogTimerDTO toDTO(WorkLogTimer timer) {
    ...
    return WorkLogTimerDTO.builder()
        .id(timer.getId())
        .personId(timer.getPerson().getId())
        ...
        .taskTitle(timer.getTask() != null ? timer.getTask().getTitle() : null)

...
/**
 * Creates default dashboard templates after admin user is created.
 * This runs AFTER DefaultAdminInitializer (Order=2 vs Order=1).
 * Templates are system-wide and available to all users.
 */
@Component
...
public class DashboardTemplateInitializer implements CommandLineRunner {
     ...
     * Uses the unique constraint on (user_id, name) indirectly by checking templates.
     */
    private boolean templateExists(String templateName) {
        return customDashboardRepository.findByIsTemplateOrderByTemplateCategoryAsc(true)
            .stream()
    ...
    private void createExecutiveSummaryTemplate(User owner) {
        CustomDashboard dashboard = CustomDashboard.builder()
            ...
            .isTemplate(true)
            .isDefault(false)
            .templateCategory(TemplateCategory.EXECUTIVE)
    ...
    private void createDeveloperDashboardTemplate(User owner) {
        CustomDashboard dashboard = CustomDashboard.builder()
            ...
            .description("Task-focused view with my tasks, team workload, and recent activity")
            .isTemplate(true)
            .isDefault(false)
            .templateCategory(TemplateCategory.DEVELOPER)
            .layoutConfig(DEFAULT_LAYOUT_CONFIG)
            .createdBy(owner)
            .build();
        dashboard = customDashboardRepository.save(dashboard);
    ...
    private void createManagerOverviewTemplate(User owner) {
        CustomDashboard dashboard = CustomDashboard.builder()
            ...
            .description("Team performance, cycle health, budget tracking, and delivery metrics")
            .isTemplate(true)
            .isDefault(false)
            .templateCategory(TemplateCategory.MANAGER)
            .layoutConfig(DEFAULT_LAYOUT_CONFIG)
            .createdBy(owner)
            .build();
        dashboard = customDashboardRepository.save(dashboard);
    ...
    private void createQADashboardTemplate(User owner) {
        CustomDashboard dashboard = CustomDashboard.builder()
            .user(owner)
            .name("QA Dashboard")
            .description("Bug tracking, test coverage, and quality metrics")
            .isTemplate(true)
            .isDefault(false)
            .templateCategory(TemplateCategory.QA)
            .layoutConfig(DEFAULT_LAYOUT_CONFIG)
            .createdBy(owner)
            .build();
        dashboard = customDashboardRepository.save(dashboard);
    ...
    private void createWidget(CustomDashboard dashboard, String widgetType, 
                              int posX, int posY, int width, int height, 
                              int displayOrder, String settings) {
        DashboardWidgetConfig widget = DashboardWidgetConfig.builder()
            .dashboard(dashboard)

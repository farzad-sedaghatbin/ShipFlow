...
/**
 * Creates default dashboard templates after admin user is created. This runs AFTER
 * DefaultAdminInitializer (Order=2 vs Order=1). Templates are system-wide and available to all
 * users.
 */
@Component
...
public class DashboardTemplateInitializer implements CommandLineRunner {
  ...
  /**
   * Check if a template with the given name already exists. Uses the unique constraint on (user_id,
   * name) indirectly by checking templates.
   */

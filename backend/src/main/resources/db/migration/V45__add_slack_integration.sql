...
CREATE TABLE slack_configuration (
    ...
    CONSTRAINT uk_slack_workspace UNIQUE (workspace_name)
);
...
CREATE TABLE slack_channel_config (
    ...
    created_at TIMESTAMP NOT NULL,
    updated_at TIMESTAMP NOT NULL,
    CONSTRAINT fk_slack_channel_config FOREIGN KEY (slack_config_id) REFERENCES slack_configuration(id) ON DELETE CASCADE,
    CONSTRAINT uk_slack_channel UNIQUE (slack_config_id, channel_name)
);
...
CREATE INDEX idx_slack_history_sent_at ON slack_notification_history(sent_at);
CREATE INDEX idx_slack_history_entity ON slack_notification_history(entity_type, entity_id);
CREATE INDEX idx_slack_history_type ON slack_notification_history(notification_type);

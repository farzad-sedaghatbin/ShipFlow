...
-- Valid TestCaseStatus values: DRAFT, READY, APPROVED, DEPRECATED, ARCHIVED
-- ===========================================
...
(1, 'TC-001', 'User Login Success', 'Verify user can login with valid credentials', 'User account exists', '1. Navigate to login\n2. Enter valid email\n3. Enter valid password\n4. Click login', 'User is logged in and redirected to dashboard', 14, 1, NULL, 'E2E', 'CRITICAL', 'READY', 'auth,login', 5, 6, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
(2, 'TC-002', 'User Login Invalid Password', 'Verify error for invalid password', 'User account exists', '1. Navigate to login\n2. Enter valid email\n3. Enter wrong password\n4. Click login', 'Error message displayed, user not logged in', 14, 1, NULL, 'E2E', 'HIGH', 'READY', 'auth,login,negative', 3, 6, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
(3, 'TC-003', 'Password Reset Flow', 'Verify password reset email flow', 'User account exists', '1. Click forgot password\n2. Enter email\n3. Check email\n4. Click reset link\n5. Enter new password', 'Password is changed, user can login', 14, 1, NULL, 'E2E', 'HIGH', 'READY', 'auth,password-reset', 10, 6, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),

-- GitHub Integration test cases
(4, 'TC-004', 'GitHub OAuth Connect', 'Verify GitHub OAuth connection flow', 'GitHub OAuth app configured', '1. Go to integrations\n2. Click Connect GitHub\n3. Authorize on GitHub\n4. Verify redirect back', 'GitHub account is connected, token stored', 1, 4, 1, 'INTEGRATION', 'CRITICAL', 'READY', 'github,oauth', 8, 6, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
(5, 'TC-005', 'GitHub Webhook Receive', 'Verify webhook events are received', 'GitHub integration connected', '1. Trigger PR event on GitHub\n2. Check webhook logs', 'Webhook event is logged and processed', 1, 4, 1, 'INTEGRATION', 'HIGH', 'READY', 'github,webhook', 5, 6, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
...
(7, 'TC-007', 'Slack Channel Config', 'Verify Slack channel configuration', 'Slack app installed', '1. Go to Slack settings\n2. Select channel\n3. Save configuration', 'Channel configuration is saved', 2, 4, 1, 'E2E', 'HIGH', 'READY', 'slack,config', 5, 6, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
(8, 'TC-008', 'Slack Notification Send', 'Verify notifications sent to Slack', 'Channel configured', '1. Trigger notification event\n2. Check Slack channel', 'Message appears in Slack channel', 2, 4, 1, 'INTEGRATION', 'CRITICAL', 'READY', 'slack,notification', 5, 6, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),

-- Dashboard test cases
(9, 'TC-009', 'Widget Drag and Drop', 'Verify widgets can be repositioned', 'Dashboard with widgets exists', '1. Open dashboard\n2. Drag widget\n3. Drop in new position', 'Widget moves to new position and persists', 3, 4, 2, 'E2E', 'HIGH', 'READY', 'dashboard,dnd', 5, 6, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
(10, 'TC-010', 'Chart Widget Data', 'Verify chart displays correct data', 'Chart widget on dashboard', '1. View chart widget\n2. Compare with source data', 'Chart shows accurate data with correct labels', 3, 4, 2, 'E2E', 'MEDIUM', 'READY', 'dashboard,charts', 8, 6, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),

-- Performance test cases
(11, 'TC-011', 'Page Load Performance', 'Verify dashboard loads under 3s', 'User logged in', '1. Navigate to dashboard\n2. Measure load time', 'Dashboard loads in under 3 seconds', NULL, 4, NULL, 'PERFORMANCE', 'MEDIUM', 'READY', 'performance', 5, 6, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
(12, 'TC-012', 'API Response Time', 'Verify API responds under 500ms', 'Server running', '1. Make API request\n2. Measure response time', 'Response received in under 500ms', NULL, 4, NULL, 'PERFORMANCE', 'LOW', 'READY', 'performance,api', 10, 6, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);
...
-- Valid BugStatus values: OPEN, IN_PROGRESS, RESOLVED, VERIFIED, CLOSED, REOPENED, WONT_FIX, DUPLICATE
-- ===========================================
...
(2, 'BUG-002', 'GitHub webhook timeout', 'Webhook events timeout after 30 seconds', '1. Configure webhook\n2. Trigger large PR event\n3. Check logs', 'Event should process within 10s', 'Event times out after 30s', 'Production', 1, 4, 1, 5, 'MAJOR', 'RESOLVED', 'github,performance', 6, 3, 'Increased timeout and added async processing', '2025-10-27', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 1),
(3, 'BUG-003', 'Dashboard widget overlap', 'Widgets overlap when browser resized quickly', '1. Open dashboard\n2. Rapidly resize browser\n3. Observe widgets', 'Widgets should reflow smoothly', 'Widgets temporarily overlap', 'Chrome 120', 3, 4, 2, 8, 'TRIVIAL', 'OPEN', 'dashboard,ui', 6, NULL, NULL, NULL, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 1),
(4, 'BUG-004', 'Slack notification missing pitch title', 'Pitch title not included in notification', '1. Trigger pitch update notification\n2. Check Slack message', 'Message should include pitch title', 'Title shows as "undefined"', 'Staging', 2, 4, 1, NULL, 'MINOR', 'RESOLVED', 'slack,notification', 6, 2, 'Fixed template variable name', '2025-10-29', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 1),
